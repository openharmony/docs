...
使用router.replace、router.back或router.clear接口进行页面跳转时，原页面将被销毁，该页面上的所有节点将被标记为InDestroying，后续不再触发布局和绘制流程。由于命令式节点没有清除该标志位的能力，导致新页面上复用老页面上的命令式节点时，无法显示在新页面中更新的内容。
