...
## IPC&RPC API支持异常处理方式和支持传入布尔值选择同步或异步方式发送信息变更
1. 这里是列表文本软总线IPC&RPC部分接口使用业务逻辑返回值表示错误信息，不符合Openharmony接口错误规范；
2. 这里是列表文本支持传入布尔值选择同步或异步方式发送信息。
#### 变更影响

此版本兼容之前的应用开发，不需要适配，后续可调用新增接口支持异常处理并返回错误码。
