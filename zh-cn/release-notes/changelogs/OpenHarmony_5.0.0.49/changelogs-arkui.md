...
当前行为：应用窗口处于横屏状态时，应用设置状态栏显隐不生效，状态栏始终处于隐藏状态；

预期行为：若应用设置过状态栏显隐，状态栏的显隐状态按应用的设置来生效；

**变更影响**

该变更为不兼容变更。

可能产生的影响：应用预期在横屏时隐藏状态栏，该接口行为变更后应用横屏时状态栏变为显示状态。

产生影响的原因：应用没有主动设置过状态栏显隐属性。

涉及适配修改的场景：应用的预期为横屏时隐藏状态栏，且应用至少设置过一次状态栏显示，当应用旋转到横屏时没有主动设置过状态栏隐藏。
