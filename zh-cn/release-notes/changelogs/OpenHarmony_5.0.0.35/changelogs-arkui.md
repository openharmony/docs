...
## cl.arkui.14 RichEditor收起键盘后，选中区、手柄菜单状态变更
...
变更前：RichEditor非用户手动点击按钮收起键盘时，触发组件失焦，复位选中区，关闭手柄菜单。

变更后：RichEditor非用户手动点击按钮收起键盘时，不触发组件失焦，不复位选中区，不关闭手柄菜单。
...
## cl.arkui.18 Contextmenu组件hoverScale接口过度动效默认行为变更
**访问级别**
