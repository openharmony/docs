...
- 支持可信设备列表查询：提供可信设备的列表查询、提供本地设备信息查询。

- 支持可信设备的上下线监听：提供注册/去注册设备上下线行为监听、支持设备可用和可信的上下线。

- 支持周边不可信设备的发现 ：提供指定条件发现周边的设备、支持设备被发现开关，关闭则其他设备无法发现本设备、设备发现结果通过回调返回给应用、支持停止设备发现流程。

- 支持设备PIN码认证（不依赖账号） ：提供业务级可信认证能力，支持PIN码认证 、提供认证解绑的能力，支持用户操作和超出信任生命周期时解绑。
...
- 支持应用文件的端云同步（仅限应用的Cloud目录）。

- 支持文件增量备份到云空间。

- 提供备份应用获取备份数据量详情的接口，打通备份应用与待备份应用间的感知通路，支持备份恢复过程中的异常事件上报。
...
- 基于NDK的C API新增支持自绘UI组件。

- 优化拉起键盘时回调时序，系统先回调编辑框属性、后显示键盘，键盘可根据输入框信息及时调整键盘显示内容。
