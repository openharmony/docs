...
|API跨平台权限变更|类名：WorkerEventTarget；<br>API声明：addEventListener(type: string, listener: WorkerEventListener): void;<br>差异内容：NA|类名：WorkerEventTarget；<br>API声明：addEventListener(type: string, listener: WorkerEventListener): void;<br>差异内容：crossplatform|api/@ohos.worker.d.ts|
