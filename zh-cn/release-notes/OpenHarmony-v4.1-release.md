...
- 子系统规格能力持续增强，如：支持P2Pv2协商、支持按业务策略的被动发布、选路增加连接可靠性确认能力；支持感知资源情况动态设置连接参数等；

- 子系统DFX机制增强，如：支持互联雷达打点、支持上层分布式业务统计、支持对组网监控事件进行业务告警、支持基础安全审计能力；

- 实现Mini/Standard设备软总线能力互通能力打样。
...
- 蓝牙提供BLE广播动态启停接口，支持广播暂停以及配置广播持续时间。

- 蓝牙支持在MDM管控场景下，MDM调用蓝牙接口强制关闭蓝牙功能，并可限制开启蓝牙。

- 蓝牙提供设备发现状态查询接口，可获得当前蓝牙是否处于设备发现状态。

- 蓝牙提供配对状态查询接口，可根据设备地址获取相应的配对状态。
