...
3. 调用该对象的on()方法，传入type和callback，订阅关心的事件调用该对象的[on()](../application-dev/reference/apis-network-kit/js-apis-net-connection.md#onnetavailable)方法，传入type和callback，订阅关心的事件。
