...
> 2. **标题层级**：文档标题为一级标题，使用`#`；介绍环境要求、调试工具命令列表、具体命令为二级标题，使用`##`。
...
使用本工具前，开发者需要先获取[hdc工具](../dfx/hdc.md)，执行hdc shell。
...
>2. 命令列表：必须按照"xx xxxx help/-h"的返回值去介绍**命令所支持的每一个参数,并说明每一个参数是否必选以及参数适用的场景。"help/-h"在首位。
