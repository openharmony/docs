# DFX概述<a name="ZH-CN_TOPIC_0000001075771948"></a>
...
-   Hilog流水日志，轻量系统类设备（参考内存≥128KB）、小型系统类设备（参考内存≥1MB）、大型系统类设备（参考内存≥1GB）适用。
...
在一个分布式系统中，一次业务的发起往往会经历多个软件模块，通过进程内、进程间、设备间的通信接口进行控制和数据传递，为了便于开发人员对这类复杂流程的理解和问题跟踪定界，DFX提供了一个分布式跟踪框架。
