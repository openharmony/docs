...
| buffer | 共享内存的信息，包含共享内存的文件描述符和空间大小，SharedBuffer定义请查看[SharedBuffer](_shared_buffer_v10.md)。 | 
...
| 名称 | 描述 | 
| -------- | -------- |
| status | 设备当前状态，DeviceStatus枚举定义了可选的设备状态，详细定义请查看[DeviceStatus](_n_n_rt_v10.md#devicestatus)。 | 
...
| 名称 | 描述 | 
| -------- | -------- |
| deviceType | 设备类型，DeviceType枚举定义了可选的设备类型，详细定义请查看[DeviceType](_n_n_rt_v10.md#devicetype)。 | 
...
| 名称 | 描述 | 
| -------- | -------- |
| model | AI模型，模型结构由Model定义，详细定义请查看[Model](_model_v10.md)。  | 
...
```
**描述**
查询设备是否支持性能偏好设置，性能偏好的定义可以参考[PerformanceMode](_n_n_rt_v10.md#performancemode)。
...
```
**描述**
查询设备是否支持任务优先级设置，优先级的定义可以参考[Priority](_n_n_rt_v10.md#priority)。
...
| 名称 | 描述 | 
| -------- | -------- |
| model | 需要编译的模型，Model定义请查看[Model](_model_v10.md)。  | 
| config | 编译模型的配置，ModelConfig定义请查看[ModelConfig](_model_config_v10.md)。  | 
| preparedModel | 编译好的模型对象，用于后续的运算，IPreparedModel定义请查看[IPreparedModel](interface_i_prepared_model_v10.md)。 | 
...
| 名称 | 描述 | 
| -------- | -------- |
| modelCache | 模型缓存文件的数组，数组顺序与导出时的数组顺序一致，数组元素类型请查看SharedBuffer定义[SharedBuffer](_shared_buffer_v10.md)。  | 
| config | 加载模型缓存的配置，配置参数的详细定义请参考[ModelConfig](_model_config_v10.md)。  | 
| preparedModel | 加载缓存得到的模型对象，用于后续的运算，IPreparedModel定义请查看[IPreparedModel](interface_i_prepared_model_v10.md)。 | 
