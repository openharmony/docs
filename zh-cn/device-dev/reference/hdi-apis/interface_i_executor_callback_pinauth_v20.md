...
定义异步API接口回调，用于返回异步接口的请求处理结果和获取信息。使用细节见[IAllInOneExecutor](interface_i_all_in_one_executor_pinauth_v20.md)。
...
| [OnTip](#ontip) ([in] int result, [in] unsigned char[] extraInfo) | 返回操作的过程交互信息。 | 
...
```
IExecutorCallback::OnTip ([in] int result, [in] unsigned char[] extraInfo)
```
**描述**
返回操作的过程交互信息。
