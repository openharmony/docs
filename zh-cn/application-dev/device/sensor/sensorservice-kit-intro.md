...
- 设备中除了本地传感器，还有可插拔的动态传感器，模块能力可以更细化的控制每个动态传感器。可通过on、off等接口监听到动态传感器的状态变化（连接、断开）。

- 动态振动器: 设备中的多个马达支持独立控制，且可以根据设备的连接、振动器的状态等信息进行灵活的管理。动态马达主要包含外接设备（如手柄、外接震动器等）可能的多个马达。
