...
Driver Development Kit（驱动开发套件）为开发者提供高效、安全、便捷的外设扩展驱动开发解决方案，为消费者带来即插即用的极致体验。
...
- 扩展外设应用：查询驱动并绑定驱动，自定义驱动与设备之间的通信方式以及数据处理方式等，即扩展[外设管理客户端](externaldevice-guidelines.md)。
- 外设扩展驱动（应用）：基于DDK能力开发的专业专用外设扩展驱动或外设扩展增强驱动，即扩展[外设管理服务端](driverextensionability.md)。
- 扩展外设管理服务（Driver Extension SA）：扩展设备管理，驱动包全生命周期管理等。
- AMS ：用于拉起和关闭扩展驱动能力DriverExtensionAbility。

### 流程说明

用户插入设备时，设备与驱动的匹配流程如图2所示。

**图2** 外设设备驱动匹配时序图

![时序图1](figures/timeSeries2.png)

用户使用扩展外设应用时，外设客户端与驱动的匹配流程如图3所示。
