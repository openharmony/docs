...
## FormExtensionAbility基本概念

FormExtensionAbility是基于Form模板的ExtensionAbility（以下简称“Form”）。ExtensionAbility是stage模型中新增的扩展组件基类，详见[Stage模型综述](stage-brief.md)。卡片提供方开发者可以通过FormExtensionAbility控制其自身卡片实际显示的内容及点击事件等。
...
1. 创建FormExtensionAbility
   ...
   开发者在创建的TS文件中自定义类继承FormExtensionAbility，重写基类回调函数，代码示例如下：
