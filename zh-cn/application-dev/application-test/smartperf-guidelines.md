...
基础采集，通过（-N）设置采集次数（采集频率为一秒一次），指定需要采集的应用包名（-PKG）或者进程ID（-PID），可以深入到应用的子线程（-threads）和文件描述符（-fds）。此外，还可以监测CPU（-c）,GPU（-g），cpu指令数（-ci），内存（-r），网络流量（-net）等相关的性能指标，以及屏幕截图（-snapshot），屏幕刷新率（-f）和温度（-t）等视觉及环境信息。对于更细致的分析，还可以设置图层（-VIEW）和分段采集（-sections），以至于更好的帮助开发者了解设备或者应用的运行情况，
