...
- DDR：每秒读取一次设备节点下各DDR的频点信息。
- FPS：衡量游戏画面的流畅度，帧率越高，流畅度越高。应用界面每秒真实的刷新次数。
- POWER：每秒读取一次设备节点下的电流信息、电池的电压信息。
- TEMP： 每秒读取一次设备的soc温度等信息，异常的温度不仅带来不好的用户体验，还会存在其它安全隐患。
- RAM：衡量应用的内存占比情况。每秒读取一次应用进程的实际物理内存。
- TRACE：当帧绘制时间超过100ms以上会自动抓取trace，1min内抓取1次。
- 屏幕截图：每秒截取一张应用界面截图。
