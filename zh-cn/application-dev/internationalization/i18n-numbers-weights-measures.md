...
度量衡包括长度单位、面积单位、体积单位和容量单位等，通过[I18NUtil](../reference/apis-localization-kit/js-apis-i18n.md#i18nutil9)类的[unitConvert](../reference/apis-localization-kit/js-apis-i18n.md#unitconvert9)接口实现单位转换和格式化。具体开发步骤如下：
...
2. 将一个单位（fromUnit）转换为另一个单位（toUnit）。

   将单位从fromUnit转换到toUnit，数值为value，并根据区域和风格格式化。style取不同值时，显示不同效果，详情见表1。
   ```ts
