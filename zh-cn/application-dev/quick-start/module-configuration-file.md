...
| deliveryWithInstall | 标识当前Module是否在用户主动安装的时候安装，即该Module对应的HAP/HSP是否跟随应用一起安装。<br/>-&nbsp;true：跟随应用一起安装。<br/>-&nbsp;false：不跟随应用一起安装，通过[按需分发](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/store-moduleinstall_arkts)的方式安装。 | 布尔值 | 该标签不可缺省。 |
