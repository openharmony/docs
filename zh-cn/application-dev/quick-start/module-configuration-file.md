...
| [routerMap](#routermap标签) | 标识当前HAP配置的路由表路径。 | 字符串 | 该标签可缺省，缺省值为空。 |
...
| 属性名称 | 含义 | 数据类型 | 是否可缺省 |
| -------- | -------- | -------- | -------- |
| url           | 标识跳转页面的名称。 | 字符串  | 该标签不可缺省。       |
| module        | 标识页面所在的模块名称。 | 字符串 | 该标签可缺省，缺省值为空。 |
| path          | 标识页面在模块内的路径。 | 字符串 | 该标签不可缺省。  |
...
此标签用于支持在路由表中配置自定义数据。
data对象内部，可以填入任意的自定义数据。

**注：** 该字段中仅支持字符串类型的自定义数据。
