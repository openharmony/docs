...
| icon | 标识快捷方式的图标，取值为资源文件的索引。<br/>**说明：**<br/>图标分为单层图标和分层图标，单层图标包含一个图片，分层图标包含前景图和背景图，推荐使用如下配置的分层图标：<br/>1.前景层：图标显示大小为450\*450px，资源大小为1024\*1024px的透明图层。<br/>2.背景层：大小为1024\*1024px。<br/> | 字符串 | 该标签可缺省，缺省值为空。 |
