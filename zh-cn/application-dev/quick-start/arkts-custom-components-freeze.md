...
页面路由和TabContent混用时，之所以会解锁TabContent的标签，是因为回到页面时会触发了页面的生命周期：OnPageShow。OnPageShow会将Page中的节点设置为active状态，TabContent作为页面的子节点，会被设置为active状态。在屏幕灭屏和屏幕亮屏时会分别触发页面的生命周期：OnPageHide和OnPageShow，因此页面中使用LazyForEach时，手动灭屏和亮屏也能实现页面路由一样的效果，如以下示例代码：
