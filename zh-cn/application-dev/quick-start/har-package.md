...
- HAR不支持在配置文件中声明[ExtensionAbility](../application-models/extensionability-overview.md)组件，但支持[UIAbility](../application-models/uiability-overview.md)组件，使用[startAbility](../reference/apis-ability-kit/js-apis-inner-application-uiAbilityContext.md#uiabilitycontextstartability)拉起Ability时，需使用依赖该HAR的[HAP](hap-package.md)/[HSP](in-app-hsp.md)的moduleName。
