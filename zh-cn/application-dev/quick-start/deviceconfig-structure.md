...
| keepAlive | 标识应用是否始终保持运行状态，仅支持系统应用配置，三方应用配置不生效。该标签为布尔类型。<br/>-&nbsp;true：应用始终保持为运行状态。系统启动时，该应用会被系统驱动起来。应用进程退出后，系统也会重新启动应用进程。<br/>-&nbsp;false：应用不会始终保持为运行状态。系统启动时，该应用不会被系统驱动起来。应用进程退出后，系统不会重新启动应用进程。 | 布尔值 | 可缺省，缺省值为false。 |
...
| 属性名称 | 含义 | 数据类型 | 是否可缺省 |
| -------- | -------- | -------- | -------- |
| cleartextPermitted | 标识自定义网域范围内是否允许明文流量传输。若cleartextTraffic和security同时存在，以cleartextPermitted的值为准。<br/>-&nbsp;true：允许明文流量传输。<br/>-&nbsp;false：拒绝明文流量传输。 | 布尔类型 | 可缺省，缺省值为空。 |
