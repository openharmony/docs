# !!语法：双向绑定
在老的状态管理框架中使用[$$](./arkts-two-way-sync.md)用于内置组件双向绑定。
在新状态管理框架中，提供了归一处理，提供`!!`语法糖作为统一处理双向绑定语法。
...
`!!`双向绑定语法，是一个语法糖方便用户实现数据双向绑定，用于初始化子组件的\@Param和\@Event。其中\@Event方法名需要声明为“$”+ \@Param属性名，详见[使用场景](#使用场景)。
