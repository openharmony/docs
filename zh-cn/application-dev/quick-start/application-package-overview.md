...
- **支持共享包机制：** 一个应用中多个模块公用的代码和资源，可以打包成独立的共享包。当前支持两种不同类型的共享包，其中一种可以作为二方库或三方库，发布到[OHPM](https://ohpm.openharmony.cn/)私仓或中心仓；另外一种可以实现运行时的按需加载。关于如何选择共享包类型，会在后续章节展开详细介绍。
