...
根据授权方式的不同，权限类型可分为system_grant（系统授权）、user_grant（用户授权）和manual_settings（手动设置授权）。

### system_grant（系统授权）

system_grant指的是系统授权类型，在该类型的权限许可下，应用被允许访问的数据不会涉及到用户的个人信息。

如果应用中申请了system_grant权限，系统会在用户安装应用时自动授予相应的权限。

### user_grant（用户授权）

user_grant指的是用户授权类型，在该类型的权限许可下，应用被允许访问的数据将会涉及到用户的个人信息。

该类型权限需在安装包中申请，并在应用运行时通过弹窗请求用户授权。当用户手动允许授权后，应用才能获取相应权限，从而成功访问目标对象。

### manual_settings（手动设置授权）

从API 21开始，新增支持manual_settings（手动设置授权）。

manual_settings指的是手动设置授权类型，在该类型的权限许可下，应用被允许访问的数据将会涉及到用户的个人信息，应用被允许执行的操作可能对系统或者用户产生严重的影响。
...
- **访问控制列表（ACL）**
  ...
  场景举例：如开发者正在开发APL等级为normal的A应用，由于功能场景需要，A应用需要跨级申请等级为system_basic的P权限。当P权限的“是否支持证书授权”为TRUE时，A应用可以通过ACL方式跨级申请权限P。
