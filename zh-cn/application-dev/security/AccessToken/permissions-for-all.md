...
该权限允许应用查看蓝牙的配置信息，包括蓝牙名称、蓝牙设备类型、开关状态等。
...
When an application is being closed, the application have a chance to ask the user whether to close the application to perform further action ./当一个应用被关闭时，应用有机会询问用户是否执行关闭应用以进行进一步操作。
...
允许应用读取加速度传感器的数据，包括：加速度计传感器、未校准加速度计传感器、线性加速器传感器。
...
允许应用配置数据网络，包括激活或取消激活指定网络，以及获取和监听指定网络信息。
...
允许应用读取陀螺仪传感器的数据，包括：陀螺仪传感器、未校准陀螺仪传感器。
...
该权限允许Service Ability后台持续运行，不会被清除，并支持消息推送。
...
拥有该权限后，三方应用可以使用后台代理提醒。使用该能力后，其自身在后台允许被冻结或清理，而提醒功能仍然可以正常运行。
...
该权限允许应用配置Wi-Fi设备，包括扫描、开关、连接、断开Wi-Fi，修改Wi-Fi配置，以及使用Wi-Fi P2P的能力。
...
允许应用控制马达振动，包括：单次振动、预置的振动效果、自定义振动。
...
该权限相关的API均为system api，仅部分特定系统应用可申请该权限。 
...
1. 读取云空间状态（云空间是否启用）
2. 读取接入云空间的应用相关的同步开关状态（是否开启同步）
3. 读取接入云空间的应用相关同步控制策略开关状态（可设置的同步策略开关状态与具体应用相关，包括：是否启用移动网络同步，是否优化本地存储空间等）
...
应用获取此权限后，能读取华为账号的登录/登出状态，主要用于订阅方应用访问数据管理服务中，由华为账号服务发布的登录状态信息。
...
允许应用获取在锁屏后保护本应用敏感数据的能力

应用可以通过声明ohos.permission.PROTECT_SCREEN_LOCK_DATA权限，获取在锁屏后保护本应用敏感数据的能力。在锁屏后，系统会将应用敏感数据对应的秘钥销毁，保护此类数据内容不被泄露或窃取。
