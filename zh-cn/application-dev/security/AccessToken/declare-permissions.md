...
| reason | 申请权限的原因。 | 字符串 | **可选填写**，该字段用于应用上架校验，当申请的权限为user_grant权限时必填，并且需要进行多语种适配。格式为$string: \*\*\*。<br>使用string类资源引用需要在string.json文件配置标签"name": "reason"，具体配置方式请参考[资源管理](../../quick-start/resource-categories-and-access.md#资源文件示例)。<br/>可参考[权限使用理由的文案内容规范](#权限使用理由的文案内容规范)。 |
