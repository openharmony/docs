...
以字符串参数生成RSA密钥，具体的字符串参数由RSA密钥类型和素数个数使用符号“|”拼接而成，用于在创建非对称密钥生成器时，指定密钥规格。

> **说明：**
>
> 生成RSA非对称密钥时，默认使用2个素数，PRIMES_2参数可省略。
...
>
> 由于系统对主线程有时间限制，耗时较长的操作会导致失败。建议开发者在生成位数较大的密钥时，使用对应的异步接口，或是[使用多线程并发能力](../../arkts-utils/multi-thread-concurrency-overview.md)进行开发。
>
> 创建RSA非对称密钥生成器时，若用于随机生成密钥，生成的RSA密钥规格与创建时指定的密钥规格一致；若用于密钥转换，生成的RSA密钥规格与转换时指定的密钥数据规格一致。

### 使用密钥参数生成

从API版本10开始，使用密钥参数生成RSA密钥。
...
- RSA不支持通过指定密钥参数（n）来随机生成密钥。

- RSA不支持通过指定密钥参数（n, sk）来生成私钥。
...
以字符串参数生成ECC密钥，具体的“字符串参数”由“ECC密钥类型”和“密钥长度”拼接而成，用于在创建非对称密钥生成器时，指定密钥规格。
...
从API版本11支持，使用曲线名来生成ECC公共密钥参数。

> **说明：**
> - 曲线名为要求输入的字符串参数，支持的曲线名请参考[ECC密钥字符串参数表](#使用字符串参数生成-1)中的“曲线名”一列。
> 
> - 生成的公共密钥参数可以直接用于随机生成公私钥，也可用于构造公钥、私钥及公私钥对。
...
当前支持以字符串参数生成SM2密钥，具体的“字符串参数”由“非对称密钥算法”和“密钥长度”拼接而成，用于在创建非对称密钥生成器时，指定密钥规格。
...
使用字符串参数生成 DH 密钥。该字符串参数由非对称密钥算法和知名安全素数群参数通过符号 "_" 拼接而成，用于在创建非对称密钥生成器时指定密钥规格。
