...
一种基于AES的密钥封装加密算法，用于安全地存储或传输对称密钥。其主要目的是通过加密保护密钥，防止密钥在存储或传输过程中泄露或被篡改。

### Asymmetric Key；非对称密钥

使用公钥和私钥两个密钥进行算法操作的密钥体制，公钥对外公开，私钥对外保密。加解密时一般使用公钥加密、私钥解密；签名验签时使用私钥签名、公钥验签，从而验证数据是否被篡改。
...
一种分组密码算法，分组长度为64位（8字节），密钥长度支持32位～448位（须为8的倍数），支持ECB、CBC、OFB、CFB等分组模式。不安全的算法，不建议使用。

## C

### CAST；分组密码算法

分组密码算法（如CAST-128/CAST5），分组长度为64位（8字节），密钥长度支持40位～128位（须为8的倍数），支持ECB、CBC、OFB、CFB等分组模式。不安全的算法，不建议使用。

### ChaCha20；流密码算法

一种现代化的流密码对称加密算法，密钥长度为256位。ChaCha20算法支持单独使用，但是只提供机密性，更常见的是将其与Poly1305认证器组合为ChaCha20-Poly1305 AEAD模式使用，同时提供机密性和完整性。
...
椭圆曲线公钥点的两种编码格式。非压缩格式保存x、y两个坐标，压缩格式仅保存x坐标和一个标志位。ECC、SM2公钥点数据可在压缩与非压缩格式之间相互转换。
...
### Ed25519；基于爱德华兹曲线的数字签名算法

Ed25519是一种基于EdDSA算法、采用Edwards曲线（Curve25519）的数字签名算法，密钥长度为256位。
...
一种基于哈希的消息认证码算法，以通信双方共享的密钥和消息作为输入生成固定长度的认证码，用于检验消息完整性。在消息摘要算法基础上增加了密钥输入以确保信息正确性。
...
使用伪随机函数从秘密值中导出一个或多个密钥的函数，可用于将密钥扩展到更长的密钥。
...
### PKCS#1 Public-Key Cryptography Standards (PKCS) #1；公钥密码标准1

PKCS#1标准定义了RSA密钥的格式，加密方案RSAES-OAEP和RSAES-PKCS1-v1_5、签名方案RSASSA-PSS和RSASSA-PKCS1-v1_5。

### PKCS#5 Public-Key Cryptography Standards (PKCS) #5；公钥密码标准5

PKCS#5是基于口令的加密标准，其中还定义了PKCS5填充算法，只支持8字节填充。

### PKCS#7 Public-Key Cryptography Standards (PKCS) #7；公钥密码标准7

PKCS#7是通用消息的封装标准，其中定义的填充规则是PKCS#5填充的通用版（支持任意块大小）。

### Poly1305；消息认证码算法

Poly1305是一种高速、一次性的消息认证码（MAC）算法。它主要用于在共享密钥的双方之间，验证消息的真实性与完整性，确保消息在传输过程中未被篡改。常见的使用场景是和流密码算法ChaCha20组合为ChaCha20-Poly1305 AEAD模式使用，同时提供机密性和完整性。

### PSS Probabilistic Signature Scheme；概率签名方案

RFC3447规范中的RSASSA-PSS模式，PSS 是 RSA 签名的概率性填充方案，需设置两个摘要（md和mgf1_md）并可设置盐值长度。

## R

### RC2；分组密码算法

一种分组密码算法，分组长度为64位（8字节），密钥长度支持8位～1024位（须为8的倍数），支持ECB、CBC、OFB、CFB分组模式。不安全的算法，不建议使用。

### RC4；流密码算法

一种流密码算法，无需分组与填充，密钥长度支持8位～4096位（须为8的倍数）。不安全的算法，不建议使用。
...
一种消息摘要算法，生成20字节（160位）固定长度摘要。不安全的算法，不建议使用。

### RSA Rivest-Shamir-Adleman；非对称加密算法

一种非对称加密与签名验签算法，密钥规格由密钥长度（位数）与素数个数决定，支持512～8192位及2～5个素数。加解密支持NoPadding、PKCS1、PKCS1_OAEP填充模式，签名验签支持PKCS1、PSS填充模式。
...
也称3DESede或TripleDES，使用3个64位的密钥对数据进行加密-解密-加密（相当于对每个数据块执行三次DES），密钥长度更长、安全性高于DES，但处理速度不如DES。不安全的算法，不建议使用。
