...
1. 调用[OH_CryptoKdfParams_Create](../../reference/apis-crypto-architecture-kit/_crypto_kdf_api.md#oh_cryptokdfparams_create)，指定字符串参数'PBKDF2'，创建密钥派生参数对象。

2. 调用[OH_CryptoKdfParams_SetParam](../../reference/apis-crypto-architecture-kit/_crypto_kdf_api.md#oh_cryptokdfparams_setparam)，设置PBKDF2所需的参数：
   - CRYPTO_KDF_KEY_DATABLOB：用于生成派生密钥的原始密码
   - CRYPTO_KDF_SALT_DATABLOB：盐值
   - CRYPTO_KDF_ITER_COUNT_INT：重复运算的次数，需要为正整数

3. 调用[OH_CryptoKdf_Create](../../reference/apis-crypto-architecture-kit/_crypto_kdf_api.md#oh_cryptokdf_create)，指定字符串参数'PBKDF2|SHA256'，创建密钥派生函数对象。
