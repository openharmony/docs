...
| ASSET_TAG_REQUIRE_PASSWORD_SET  | 类型为bool。                                                   | 可选     | 是否仅在设置了锁屏密码的情况下，可访问关键资产。为true时表示在筛选仅设置了锁屏密码可访问的关键资产，为false时表示筛选设置或不设置锁屏密码的情况下，都可访问的关键资产。     |
...
| ASSET_TAG_IS_PERSISTENT         | 类型为bool。                                                   | 可选     | 在应用卸载时是否需要保留关键资产。为true时表示筛选应用卸载时需要保留的关键资产，为false时表示筛选应用卸载时不保留的关键资产。              |
