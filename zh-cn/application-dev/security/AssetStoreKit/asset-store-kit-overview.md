...
* **基于群组的访问控制：** 如有需要，多业务可主动配置为同一群组，以实现群组内多业务间的数据互通。
  * 允许属于一个群组的关键资产，被该群组内所有业务访问。
  * 群组由开发者ID与群组ID唯一确定，业务开发者ID与业务所属的全部群组ID由ASSET从系统服务中获取，业务可在访问属于一个群组的关键资产时指定其所属的群组ID，并在群组ID与业务所属的全部群组ID之一匹配时，被允许访问此关键资产。开发者ID包含在应用证书内，不可被仿冒。所以即使群组ID被仿冒，仿冒者也无法获取到业务群组内的数据。
  * 允许属于一个群组的关键资产加/解密时，其所属群组信息（开发者ID与群组ID）参与了完整性保护，即使关键资产所属群组信息被篡改，攻击者也无法获取到其他群组的数据。
