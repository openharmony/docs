...
* **基于群组的访问控制：**
  ...
  * 关键资产所属的群组信息，由开发者ID和群组ID组成。其中，开发者ID无需配置，群组ID在应用开发期配置，配置规则详见[配置规则](../../quick-start/app-configuration-file.md#配置文件标签)。应用发布时使用开发者私钥签名保证群组ID的完整性。
...
* 基于群组的访问控制
  ...
  ASSET不支持在群组存储[IS_PERSISTENT](../../reference/apis-asset-store-kit/js-apis-asset.md#tag)属性设置为true的数据。
...
* 群组关键资产删除时机
  1. 业务主动调用remove删除群组关键资产时，删除符合条件的群组数据。详见删除关键资产[ArkTS](asset-js-remove.md)、[C/C++](asset-native-remove.md)开发指导。
  2. 应用卸载时，清除该应用存储在ASSET中的群组数据。
  3. 系统子用户删除时，清除该用户下ASSET中所有群组数据。
  4. 设备恢复出厂设置时，清除ASSET中所有群组数据。
