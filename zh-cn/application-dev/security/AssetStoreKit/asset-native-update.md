...
- **query的参数列表：**
  ...
  | ASSET_TAG_REQUIRE_ATTR_ENCRYPTED<sup>14+</sup> | 类型为bool。 | 可选 | 是否更新业务自定义附属信息被加密的数据，默认值为false，即更新业务附属信息不加密的数据。为true时表示更新业务自定义附属信息加密存储的数据，为false时表示更新业务自定义附属信息不加密存储的数据。|
