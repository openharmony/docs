...
- 申请应用市场审核并通过。
- 开发应用安装包时，需要修改应用的profile文件，在文件的"apl"字段声明应用的APL等级，并使用profile签名工具生成证书。具体签名流程可以查看[Hap包签名工具指导](hapsigntool-guidelines.md)。
...
* 申请应用市场审核并通过。
* 开发应用安装包时，需要修改应用的profile文件，在文件的"acl"字段声明目标的访问控制列表，并使用profile签名工具生成证书。具体签名流程可以查看[Hap包签名工具指导](hapsigntool-guidelines.md)。
