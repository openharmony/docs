...
代码签名部件是OpenHarmony提供的代码签名能力，基于linux内核fs-verity的方案设计，其原理是基于文件的hash校验，来保证系统中被加载的代码是可信的。签名工具默认开启代码签名机制，若用户确定不需要强制代码签名，可参考使用文档，关闭代码签名功能。签名工具当前仅支持对hap格式和elf格式执行代码签名，暂不支持对bin应用代码签名。
