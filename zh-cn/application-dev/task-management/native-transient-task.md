...
常用接口如下表所示，具体API说明详见API参考[TransientTask](../reference/apis-backgroundtasks-kit/_transient_task.md#transienttask#函数说明)。
...
1. 封装函数
   ...
   static napi_value GetAllTransientTasks(napi_env env, napi_callback_info info)
   {
      napi_value result;
...
### 在index.d.ts文件中声明函数
   ...
   export const GetAllTransientTasks: () => backgroundTaskManager.TransientTaskInfo;
   ```
...
### 在index.ets文件中调用函数
   ...
   struct Index {
     ...
     GetAllTransientTasks() {
       let ret = testTransientTask.GetAllTransientTasks();
