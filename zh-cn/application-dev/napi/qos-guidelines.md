...
自多道程序及多任务操作系统问世以来，CPU、内存等有限的系统资源成为系统中所有任务的竞争对象。合理安排各个任务对系统的响应速度以及资源消耗都有非常重大的意义。相比操作系统，开发者更加清楚应用中各个任务的重要程度，根据重要程度对应用的任务进行分类，能帮助系统更好地进行任务的调度。通过本指导，开发者可以了解在OpenHarmony系统中，如何使用QoS接口调节任务在系统中的运行时间分配。
...
QoS(quality-of-service)，即服务质量，在OpenHarmony中QoS特性主要指任务的优先调度属性。开发者可以利用QoS对要执行的工作进行分类，以指示其与用户交互的关联程度；系统则可以根据任务设置的QoS安排各任务的时间和运行的次序
。例如，当系统中有多个任务需要同时执行的时候，一些与用户交互的关联程度不高的后台下载任务可以推迟到更晚的时间执行且每次执行的时候分配更少的时间；而用户感知明显的动效绘制等任务则需要立即执行并分配更多的执行时间。
...
| QOS_USER_INITIATED | 用户触发并且可见进展，例如打开文档。 | 任务在几秒钟之内完成。 |
