...
使用Node-API关于string的六个接口，可以让NAPI模块和JavaScript字符串进行交互。
...
| 接口 | 描述 |
| -------- | -------- |
| napi_get_value_string_utf8 | 需要将JavaScript的字符类型的数据转换为utf8编码的字符时使用这个函数。 |
| napi_create_string_utf8 | 需要通过UTF8编码的C字符串创建JavaScript string值时使用这个函数。 |
| napi_get_value_string_utf16 | 需要将JavaScript的字符类型的数据转换为utf16编码的字符时使用这个函数。 |
| napi_create_string_utf16 | 需要通过UTF16编码的C字符串创建JavaScript string值时使用这个函数。 |
| napi_get_value_string_latin1 | 需要将JavaScript的字符类型的数据转换为utf16编码的字符时使用这个函数。 |
| napi_create_string_latin1 | 需要通过UTF16编码的C字符串创建JavaScript string值时使用这个函数。 |

## 使用示例

### napi_get_value_string_utf8

napi_get_value_string_utf8接口可以将JavaScript的字符类型的数据转换为utf8编码的字符。
...
hilog.info(0x0000, 'testTag', 'Test NAPI get_value_string_utf8_string %{public}s', testNapi.getValueStringUtf8('aaBC+-$%^你好123'));
...
napi_get_value_string_utf16，将JavaScript的字符类型的数据转换为utf16编码的字符。
...
napi_get_value_string_latin1接口可以将JavaScript的字符类型的数据转换为ISO-8859-1编码的字符。
