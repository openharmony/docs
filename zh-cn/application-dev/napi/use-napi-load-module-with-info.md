...
Node-API中的napi_load_module_with_info接口的功能是在C++新创建的线程中进行模块的加载，当模块加载出来之后，可以使用函数napi_get_property获取模块导出的变量，也可以使用napi_get_named_property获取模块导出的函数，目前支持以下场景：
- 加载系统模块
- 加载ets目录下文件中的模块
- 加载HAR包(静态共享包)
- 加载本地so库
...
1. 在加载第三方HAR包时，首先需要在oh-package.json5文件中配置dependencies项
```json
{
    "dependencies": {
        "json5": "^2.2.3"
