...
Device（终端设备）指连接到Host（主机端）的外设，负责执行特定功能（如存储、输入输出等），被动响应Host的指令。例如U盘、鼠标、打印机等均属于Device设备。

## E

### Endpoint（端点）

端点是终端设备（Device）与主机（Host）之间进行数据传输的逻辑终点，是USB通信的基本单元。每个端点具有唯一的地址和方向（IN表示设备（Device）到主机（Host），OUT表示主机（Host）到设备（Device））。每个端点代表一个单向或双向的数据传输通道。

## H

### Host（主机设备）

Host是指具有USB主机功能的设备。Host是控制和管理USB总线的设备，它负责管理总线上的连接设备，并协调数据传输和通信。Host设备通常是计算器或其他主机设备，如PC机、智能手机、平板电脑等。Host设备可以连接和控制多个终端设备，通过USB端口与终端设备相连，提供数据传输和充电的功能。
