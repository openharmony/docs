...
Notification Kit为开发者提供在本地即时发布用户通知的能力，本地通知可出现在通知中心、横幅、锁屏、熄屏显示等系统入口，帮助开发者实现在应用界面外与用户完成交互。
...
- 支持应用发布不同基础类型通知。
- 支持应用在发布通知时指定不同的通知渠道类型。
- 支持应用管理通知发布渠道类型，比如新增或删除渠道、查询渠道使能状态等。
...
- 应用可根据业务场景，选择合适的渠道发布通知。
- 应用可设置用户点击通知后的行为意图，包括跳转到应用的UIAbility或者发布自定义公共事件。
