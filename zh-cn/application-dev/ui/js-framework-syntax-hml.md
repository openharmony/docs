...
- 绑定并阻止冒泡事件向上冒泡：grab:{event}.bubble。grab:{event}等价于grab:{event}.bubble。
  ...
  >
  > 详细冒泡事件说明参见[通用事件](../reference/apis-arkui/arkui-jsjs-components-common-events.md)章节。
- 示例
  ```html
  <!-- xxx.hml -->
  <div>
...
HML可以通过element引用模板文件，详细介绍可参考[自定义组件](../reference/apis-arkui/arkui-jsjs-components-custom-basic-usage.md)章节。
