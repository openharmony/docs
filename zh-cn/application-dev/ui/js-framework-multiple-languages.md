...
- 简单格式化方法
  ...
  **表2** 简单格式化
  ...
  | 属性 | 类型 | 参数 | 必填 | 描述 |
  | -------- | -------- | -------- | -------- | -------- |
  | $t | Function | 请见**表 $t参数说明** | 是 | 根据系统语言完成简单的替换：this.$t('strings.hello') |
...
- 单复数格式化方法
  **表4** 单复数格式化
  ...
  | 属性 | 类型 | 参数 | 必填 | 描述 |
  | -------- | -------- | -------- | -------- | -------- |
  | $tc | Function | 请见**表 $tc参数说明** | 是 | 根据系统语言完成单复数替换：this.$tc('strings.people')<br/>>&nbsp;![icon-note.gif](public_sys-resources/icon-note.gif)&nbsp;**说明：**<br/>>&nbsp;定义资源的内容通过json格式的key为“zero”、“one”、“two”、“few”、“many”和“other”区分。 |
...
获取语言功能请参考[应用配置](../reference/apis/js-apis-basic-features-configuration.md)。
