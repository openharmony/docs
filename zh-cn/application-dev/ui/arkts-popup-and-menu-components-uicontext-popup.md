...
[气泡提示（Popup）](arkts-popup-and-menu-components-popup.md)在使用时依赖绑定UI组件，否则无法使用。本文介绍通过使用全局接口[openPopup](../reference/apis-arkui/js-apis-arkui-UIContext.md#openpopup16)的方式，在无UI组件的场景下直接或封装使用，例如在事件回调中使用或封装后对外提供能力。
...
通过[openPopup](../reference/apis-arkui/js-apis-arkui-UIContext.md#openpopup16)可以弹出气泡。
...
通过[updatePopup](../reference/apis-arkui/js-apis-arkui-UIContext.md#updatepopup16)可以更新气泡的样式。支持全量更新和增量更新。不支持更新showInSubWindow、focusable、onStateChange、onWillDismiss、transition。
...
通过[closePopup](../reference/apis-arkui/js-apis-arkui-UIContext.md#closepopup16)可以关闭气泡。
...
> **说明：**
>
> 由于[updatePopup](../reference/apis-arkui/js-apis-arkui-UIContext.md#updatepopup16)和[closePopup](../reference/apis-arkui/js-apis-arkui-UIContext.md#closepopup16)依赖content来更新或者关闭指定的气泡，开发者需自行维护传入的content。
...
调用HAR包对外暴露出来的接口来实现气泡弹出、更新和关闭。详细实现请参考[完整示例](#完整示例-1)。
...
### 创建ComponentContent。
   ...
   如果在wrapBuilder中包含其他组件（例如：[Popup](../reference/apis-arkui/arkui-ts/ohos-arkui-advanced-Popup.md#popup)、[Chip](../reference/apis-arkui/arkui-ts/ohos-arkui-advanced-Chip.md#chip)组件），则ComponentContent应采用带有四个参数的构造函数[constructor](../reference/apis-arkui/js-apis-arkui-ComponentContent.md#constructor-2)，其中options参数应传递{ nestingBuilderSupported: true }。
...
### 绑定组件信息。
   ...
   通过调用openPopup接口弹出的气泡，需要提供绑定组件的信息[TargetInfo](../reference/apis-arkui/js-apis-arkui-UIContext.md#targetinfo16)。若未传入有效的target，则无法弹出气泡。
...
### 设置弹出气泡样式。
   ...
   通过调用openPopup接口弹出的气泡，可以设置[PopupCommonOptions](../reference/apis-arkui/arkui-ts/ts-universal-attributes-popup.md#popupcommonoptions16类型说明)属性调整气泡样式。
