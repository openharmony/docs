...
本章重点介绍如何实现应用程序UI界面的国际化，关于应用适配国际化的详细参考，见[Localization Kit（本地化开发服务）](../internationalization/Readme-CN.md)。

## 利用资源限定词配置国际化资源

资源国际化是指通过多语言资源文件实现应用界面内容的自动切换，关于资源类型及访问方式的详细参考，见[资源分类与访问](../quick-start/resource-categories-and-access.md)。
