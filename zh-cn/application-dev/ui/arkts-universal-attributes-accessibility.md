...
ArkUI无障碍能力总体上围绕辅助功能兼容性和残障用户友好体验设计，主要有以下几点特征：

- 语义化描述

  通过设置[accessibilityText](#设置无障碍文本)和[accessibilityDescription](#设置无障碍说明)属性为组件提供屏幕阅读器可识别的语义化信息。

- 分组优化

  通过设置[accessibilityGroup](#设置无障碍分组)属性，将多个子组件合并为一个逻辑单元（如整块卡片），避免屏幕阅读器逐项朗读冗余内容。

- 焦点屏蔽

  通过设置[accessibilityLevel](#设置无障碍重要性)属性，隐藏非必要组件（如装饰性元素），减少干扰。

- 动态交互支持

  通过控制[accessibilityChecked](#在支持多选的情况下设置无障碍节点是否被选中)和[accessibilitySelected](#在支持单选的情况下设置无障碍节点是否被选中)属性，明确控件状态（如复选框选中），提升操作可感知性。

- 复杂组件的无障碍适配

  复杂组件可以设置[accessibilityVirtualNode](#设置无障碍虚拟子节点)，通过虚拟布局为无实际子节点的组件生成无障碍语义信息。

## 设置无障碍分组

accessibilityGroup属性，用于设置是否启用无障碍分组。若启用，则将该组件及所有其子组件作为一个整体处理，无障碍服务不再单独处理子组件。accessibilityGroup属性支持以下值：
...
accessibilityLevel属性，表示组件的无障碍重要性，用于控制组件是否可被无障碍服务识别，支持以下值：
