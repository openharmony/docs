...
ringtonePlayer需要和配合使用，才能完成管理系统铃声的功能。
...
系统铃声播放器，提供系统铃声的参数设置、参数获取、播放、停止等功能。在调用RingtonePlayer的接口前，需要先通过创建实例。
...
| 类型                | 说明                            |
| ------------------- | ------------------------------- |
| Promise&lt | Promise回调返回获取的AudioRendererInfo。 |
...
| callback | Callback&lt;audio.InterruptEvent | 是   | 被监听的中断事件的回调。    |
