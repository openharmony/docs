...
| 名称 | 类型 | 必填 | 装饰器类型        | 说明     |
| -------- | -------- | -------- |---------------|--------|
| mainImage | [ResourceStr空格替换ts-types  resourcestr) | 是 | @State | 传入图片。  |
| primaryText | [ResourceStr空格替换ts-types  resourcestr) | 是 | @Prop         | 标题内容。  |
| secondaryText | [ResourceStr空格替换ts-types  resourcestr) | 否 | @Prop         | 副标题内容。 |
| tertiaryText | [ResourceStr空格替换ts-types  resourcestr) | 否 | @Prop         | 辅助文本。  |
| container | ()&nbsp;=&gt;&nbsp;void | 是 | @BuilderParam | 容器内组件。 |

## 事件
不支持[通用事件空格替换ts-universal-events-click.md)
