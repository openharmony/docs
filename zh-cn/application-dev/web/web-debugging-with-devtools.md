...
## 无线调试
从API version 20开始，可使用无线调试接口[setWebDebuggingAccess<sup>20+</sup>](../reference/apis-arkweb/arkts-apis-webview-WebviewController.md#setwebdebuggingaccess20)，来简化调试流程。
...
能否调试系统浏览器打开的网页，取决于系统浏览器有没有开启Web调试开关。相关说明请参考[应用代码开启web调试开关](#应用代码开启web调试开关)。
* 若系统浏览器已启用Web调试开关，则第一步应用代码启用Web调试开关已完成，可继续执行[调试步骤](#调试步骤)中的后续步骤。
* 若系统浏览器未开启Web调试开关，则无法调试系统浏览器中打开的网页，继续执行[调试步骤](#调试步骤)中的后续步骤亦将无效。

### 如何判断应用是否开启了Web调试开关？
若具备应用的源代码，可直接查阅。若不具备应用的源代码，可跳过[应用代码开启Web调试开关](#应用代码开启web调试开关)这一步，继续执行[调试步骤](#调试步骤)中的后续步骤：
* 如果后续步骤执行成功， 说明应用开启了Web调试开关。
* 如果后续步骤执行不成功， 说明应用没有开启Web调试开关。
