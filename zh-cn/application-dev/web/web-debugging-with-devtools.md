...
## 无线调试
  上述的调试步骤中, 由于[端口转发](#端口转发)这一步需要频繁的查询domain socket并转发端口，给页面调试带来不便。因此在鸿蒙系统API20版本里添加了无线调试接口[setWebDebuggingAccess(webDebuggingAccess: boolean, port: number)](../reference/apis-arkweb/js-apis-webview.md#setwebdebuggingaccess20)， 来简化调试步骤。

### 调试步骤变化

#### 应用代码开启Web调试开关
这一步中应用需要调用[setWebDebuggingAccess(webDebuggingAccess: boolean, port: number)](../reference/apis-arkweb/js-apis-webview.md#setwebdebuggingaccess20)接口设置tcp socket端口号并开启Web调试开关。
示例代码：
  ```ts
  // xxx.ets
  import { webview } from '@kit.ArkWeb';
  ...
  ```
注意：代码中使用的8888端口仅作为示例展示，开发者使用过程中，应保证端口号可以被应用使用。如果因为端口被占用或者应用无权限使用等因素导致端口无法被应用使用，会导致接口抛出异常或者ArkWeb无法开启调试模式。

#### 将设备连接至电脑
如果ArkWeb成功开启了无线调试模式，这一步可以省略。

#### 端口转发
如果ArkWeb成功开启了无线调试模式，这一步可以省略。

#### 在Chrome浏览器上打开调试工具页面
第2步"修改Chrome调试工具的配置"中的第(2)条，改为:  
在 "Target discovery settings" 中添加被调试设备的IP地址和setWebDebuggingAccess(webDebuggingAccess: boolean, port: number)接口中指定的port端口，比如：192.168.0.3:8888。

注意： 调试工具和被调试设备要在同一局域网下，并且能够相互访问。如果被调试设备有多个IP地址，要使用与调试工具同一个网段的IP地址。
