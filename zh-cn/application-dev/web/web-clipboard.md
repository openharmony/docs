...
开发者能够自定义菜单中的功能选项，当用户选择特定选项时，开发者可以通过调用[cut](../reference/apis-arkweb/ts-basic-components-web.md#cut9)、[copy](../reference/apis-arkweb/ts-basic-components-web.md#copy9)、[copyImage](../reference/apis-arkweb/ts-basic-components-web.md#copyimage9)、[paste](../reference/apis-arkweb/ts-basic-components-web.md#paste9)等接口，将网页中的文本、HTML或图片数据复制到系统剪贴板，或从系统剪贴板粘贴到网页的可输入区域。
...
> **说明：**
>
> 通过[paste](../reference/apis-arkweb/ts-basic-components-web.md#paste9)接口访问系统剪贴板内容，需[申请访问剪贴板权限](../basic-services/pasteboard/get-pastedata-permission-guidelines.md#申请访问剪贴板权限)：ohos.permission.READ_PASTEBOARD。
...
开发者可以通过设置Web组件的[copyOptions](../reference/apis-arkweb/ts-basic-components-web-attributes.md#copyoptions11)属性，来指定Web组件上剪贴板复制的范围。可以指定的选项有：CopyOptions.None（不支持复制）、CopyOptions.InApp（支持应用内复制）以及CopyOptions.LocalDevice（支持设备内复制）。默认值为：CopyOptions.LocalDevice，即默认支持设备内部的复制。
