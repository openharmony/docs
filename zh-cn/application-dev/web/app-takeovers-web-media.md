...
本地播放器需要实现[NativeMediaPlayerBridge](../reference/apis-arkweb/js-apis-webview-NativeMediaPlayerBridge.md#interfaces-nativemediaplayerbridge)接口，以便ArkWeb内核对本地播放器进行播控操作。
...
为了方便ArkWeb内核对本地播放器进行播控操作，应用需要令本地播放器实现[NativeMediaPlayerBridge](../reference/apis-arkweb/js-apis-webview-NativeMediaPlayerBridge.md#interfaces-nativemediaplayerbridge)接口，并根据每个接口方法的功能对本地播放器进行相应操作。
...
在[onCreateNativeMediaPlayer](../reference/apis-arkweb/js-apis-webview-WebviewController.md#oncreatenativemediaplayer12)接口中， ArkWeb内核传递一个[NativeMediaPlayerHandler](../reference/apis-arkweb/js-apis-webview-NativeMediaPlayerHandler.md#interfaces-nativemediaplayerhandler)对象给应用。应用需要通过该对象，将本地播放器的最新状态信息通知给ArkWeb内核。
