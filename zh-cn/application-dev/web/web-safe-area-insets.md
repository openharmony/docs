...
早期设备屏幕多为规整矩形，应用界面可以完整展示。随着全面屏、刘海屏等异形屏幕的普及，屏幕边缘可能存在圆角、摄像头或系统导航条等，导致界面被遮挡。

安全区域是屏幕中未被设备硬件或系统UI遮挡的区域，不与系统非安全区域（如状态栏、挖孔区和导航栏）重叠。应用的布局默认限制在安全区域内，但系统提供沉浸式布局能力，应用可以通过配置使界面扩展至非安全区域。

在沉浸式效果下，Web组件网页元素会出现被状态栏、挖孔区以及导航条遮挡的问题。该场景下，需要网页开发者进行避让适配，使网页中文字、表单和交互组件等关键内容避让非安全区域，以确保用户能完整地阅读和交互。

Web组件提供了利用W3C CSS进行安全区域计算并避让适配的能力，用来支持异形屏幕设备在沉浸式效果下网页的正常显示，网页开发者可以使用该能力对被遮挡元素进行避让。

## 开启Web组件沉浸式效果

Web组件默认布局在安全区域内。开启沉浸式效果时，Web网页延伸至状态栏和导航栏，可以最大化利用屏幕可视区域，优化视觉连贯性，提升用户的UI体验。开发者可以通过以下方式开启Web组件沉浸式效果：

- 通过[setWindowLayoutFullScreen](../reference/apis-arkui/js-apis-window.md#setwindowlayoutfullscreen9)设置应用窗口全屏。窗口全屏时，Web组件可布局至非安全区域。
...
`viewport-fit=cover`时，ArkWeb内核会持续监测Web组件及系统非安全区域的位置与尺寸，依据两者的重叠部分计算出网页在四个方向上需要避让的具体距离，并设置给环境变量`safe-area-inset-*`。在矩形显示器（如普通2in1设备的屏幕）中，这些值等于零。而对于非矩形显示器（如圆形表盘或移动设备屏幕），`safe-area-inset-*`所形成的内矩形区域即安全区域，网页内容在该区域内可以完整显示，避免被非矩形显示区域切断。

网页元素的避让适配依赖CSS函数`env()`，该函数用于获取浏览器或系统提供的环境变量。使用`env()`函数可以获取`safe-area-inset-*`的值。网页开发者无需关注设备非安全区域的具体大小，在CSS样式中使用`env(safe-area-inset-*)`即可定义网页需要避让的距离，实现跨设备避让。语法如下：
...
Web组件开启沉浸式效果时，渲染内容可能会与非安全区域重叠，影响用户阅读交互，示例如图2。顶部状态栏、屏幕挖孔区和底部导航条均为非安全区域。沉浸式效果下，index.html网页标题栏被挖孔区遮挡，底部Tab区域与导航条重叠。
...
网页开发者可以使用`env(safe-area-inset-*)`灵活定义CSS样式，使文字、图片和交互组件避让非安全区域。在下面的示例中，使用`env(safe-area-inset-*)`更新了index.html的CSS样式，使网页主要内容避让非安全区域，避让效果见图3：
