...
Web组件支持同层渲染绘制Video和Button组件。主要应用于各种小程序。

开发者可通过[enableNativeEmbedMode()](../reference/arkui-ts/ts-basic-components-web.md#enablenativeembedmode11)来控制同层渲染开关。同层渲染的标签背景是白色的，支持Web组件嵌套一层Web组件。
