...
### 使用HiDebug获取系统CPU使用率。
下文将展示如何在应用内增加一个按钮，并单击该按钮以调用HiDebug Ndk接口以获取系统CPU使用率。
...
3. 编辑“test_backtrace.cpp”文件如下：
    ...
    class BackTraceObject { // 封装了抓栈过程中需要使用的资源，在使用过程中请注意线程安全和信号安全。
    public:
        ...
        void Release();
        int BackTraceFromFp(void* startFp, int size); // 该函数信号安全。
    ...
    BackTraceObject& BackTraceObject::GetInstance() { // 单例模式，用于信号处理函数线程与请求抓栈线程共同访问，以进行数据交互，注意该类非信号安全，应在业务逻辑中保证同一时刻仅单个线程在访问该类。
    ...
    void SignalHandler(int sig, siginfo_t *si, void* context) { // 信号处理函数。
        ...
        pcSize = BackTraceObject::GetInstance().BackTraceFromFp(reinterpret_cast<void*>(startFp), MAX_FRAME_SIZE); // 该函数信号安全，仅信号安全函数可在信号处理函数内使用。
