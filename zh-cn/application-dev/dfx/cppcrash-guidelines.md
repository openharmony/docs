...
| Page switch history | 页面切换轨迹 | 21 | 否 | 如果维测服务进程出现故障或未缓存切换轨迹，则不包含此字段，详见[实现原理](#实现原理)。 |
...
> **注意：**
>
> 异步线程栈跟踪维测功能仅在ARM 64位系统环境下且应用处于debug模式下开启。
...
针对包含页面切换的应用，自API 21起，维测进程会记录应用切换历史。应用发生故障后，生成的故障文件将包含页面切换历史轨迹。
