...
1. 在nolog版本中，开放应用因为js_heap泄漏导致的OOM场景下订阅虚拟机堆快照的功能。应用需要依次调用 hidebug.setAppResourceLimit和hiAppEvent.addWatcher，且在 AppScope/app.json5 文件中配置如下环境变量：
   ...
   **nolog版本虚拟机堆快照生成规格限制**
   ...
   >
   > API14后，开发者可以将后缀名修改为.rawheap后，直接使用浏览器开发者工具将其导入并展示，详情见[Raw Heap离线导入](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-snapshot-basic-operations#section1888195110017)。
