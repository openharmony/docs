...
**使用示例**
```bash
# 触控笔从(100, 1000)按下花费1000ms移动到(100, 2000)后，保持按下1000ms后抬起。
...
**使用示例**
```bash
# 手指触摸从(100, 1000)按下花费1000ms移动到(100, 2000)后，保持按下1000ms后抬起。
...
rotateValue旋转值，取值范围-360-360。旋转角度为正值时，顺时针旋转，旋转角度为负值时，逆时针旋转。
```
