...
8. 点击IDE界面中的运行按钮，运行应用工程，然后在应用界面中点击按钮“appCrash”，触发一次崩溃事件。js crash将触发本地回调，cpp crash将触发远程回调，因此cpp crash比js crash触发慢是系统机制导致，cpp crash正常触发回调时间为2~3秒。事件的长短与线程数有关，开发者可用非主线程来降低触发回调慢对应用的影响。
