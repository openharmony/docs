...
场景一：期望指定命令行参数，如`-b bundlename`，执行“非交互模式”命令行，但指定的 _command_ 参数为空。

场景二：期望进入“交互模式”命令行，但是指定了不支持“交互模式”命令行的参数。

**处理步骤**

场景一：确保命令`hdc shell [-b bundlename] [command]`指定了合适的参数，其中[command]参数需要指定对应的命令，以便在“非交互模式”命令行中直接执行命令。

场景二：移除不支持“交互模式”命令行的参数，以便进入“交互模式”命令行。
...
同错误码[E003001](#e003001-（命令行）指定的应用名称非法)
