...
1. 订阅事件。这里使用了OnReceive类型观察者、OnTrigger类型观察者的订阅方式。
...
2. 编辑"napi_init.cpp"文件，添加按钮点击事件打点接口：
...
3. 编辑"napi_init.cpp"文件，将RegisterWatcherCrash()(订阅崩溃事件)、RegisterWatcherClick()（订阅按钮点击事件）、WriteAppEvent()(按钮点击事件打点接口)注册为ArkTS接口：
...
编辑"Index.ets"文件，新增"appCrash"按钮触发崩溃事件；新增"buttonClick"按钮，在按钮点击函数中进行事件打点，示例代码如下：
...
2. 搜索关键字"HiAppEvent”，可以在HiLog窗口看到对崩溃事件数据的处理日志：
...
3. 点击buttonClick按钮，触发一次按钮点击事件打点。搜索关键字"HiAppEvent”，可以在HiLog窗口看到对按钮点击事件数据的处理日志：
