...
| bool OH_HiTrace_IsTraceEnabled(void)                         | 获取当前是否允许调用进程的跟踪输出。                         |

HiTraceMeter打点接口按功能/行为分类，主要分三类：同步时间片跟踪接口、异步时间片跟踪接口和整数跟踪接口。无论同步时间片跟踪接口还是异步时间片跟踪接口，接口本身都是同步接口，不是异步接口。HiTraceMeter打点接口可与[HiTraceChain](./hitracechain-guidelines-ndk)一起使用，进行跨设备/跨进程/跨线程打点与分析。
