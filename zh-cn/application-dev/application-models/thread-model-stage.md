...
- TaskPool线程（[@ohos.taskpool](../reference/apis-arkts/js-apis-taskpool.md)）
  - 用于执行耗时操作，支持设置调度优先级、负载均衡等功能，推荐使用。
- Worker线程（[@ohos.worker](../reference/apis-arkts/js-apis-worker.md)）
  - 用于执行耗时操作，支持线程间通信。
...
> - 执行`hdc shell`命令，进入设备的shell命令行。在shell命令行中，执行`ps -p <pid> -T`命令，可以查看指定应用进程的线程信息。其中，`<pid>`为需要指定的应用进程的[进程ID](process-model-overview.md)。
