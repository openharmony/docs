...
在下面的示例中，UIAbility运行在主进程，其中包含多个EmbeddedComponet。每个EmbeddedComponet对应一个EmbeddedUIExtensionAbility。多个EmbeddedUIExtensionAbility可以分别用于实现办公软件中的文档、表格、演示文件。不同的EmbeddedUIExtensionAbility可以共用一个EmbeddedUIExtensionAbility进程，也可以运行在不同的EmbeddedUIExtensionAbility进程。
