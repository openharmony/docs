...
>**说明：**
><br>本模块首批接口从API version 12开始支持，并对[IPCKit](../reference/apis-ipc-kit/Readme-CN.md)存在依赖
><br>后续版本新增接口，采用上角标单独标记接口的起始版本。

## 场景介绍

多进程应用模型现在在业界已广泛采用，可以大幅度提升整个应用的稳定性，单个进程出现问题导致崩溃不会影响全局；现在新增了Native子进程的支持，并在父子进程间建立IPC通道，方便开发者在Native层进行多进程编程。
...
教程假设已经建立好Native应用开发工程，在此基础上下列步骤描述了如何使用`AbilityKit`提供的C API接口，创建Native子进程，并同时在父子进程间建立IPC通道。
...
在主进程中，主要负责调用API启动Native子进程，通过回调函数获取子进程启动结果，并同子进程间进行IPC通信实现自身业务逻辑。
...
调用API启动Native子进程，需要注意返回值为NCP_NO_ERROR仅代表成功调用native子进程启动逻辑，实际的启动结果通过第二个参数中指定的[回调函数](#子进程启动结果回调函数)异步通知。
...
修改CMaklist.txt添加必要的依赖库，假设主进程所在的so名称为libmainprocesssample.so。
