...
应用进程启动后，系统会默认创建主线程并进入消息循环，应用组件运行在主线程上。如果应用需要处理复杂或耗时的任务，开发者可以创建[Worker](../reference/apis-arkts/js-apis-worker.md) 线程，或将任务提交到[Taskpool](../reference/apis-arkts/js-apis-taskpool.md)中执行。关于线程模型的详细说明，请参见[线程模型](thread-model-stage.md)。
