...
1. 支持系统识别同类应用：当调用方拉起垂类应用（例如导航类应用）时，系统会根据linkFeature字段识别到匹配的应用，并在应用面板中展现。
    ...
    |值|说明|
    |---|---|
    |[AppStorageMgmt](#清理应用沙箱缓存数据场景)|指示清理应用沙箱目录中缓存数据的功能。|
    |[FileOpen](file-processing-apps-startup.md#目标方接入步骤)|指示打开处理文件的功能。|
    |[Navigation](start-navigation-apps.md#目标方开发步骤)|指示导航功能。|
    |[RoutePlan](start-navigation-apps.md#目标方开发步骤)|指示路线规划功能。|
    |[PlaceSearch](start-navigation-apps.md#目标方开发步骤)|指示地点搜索功能。|
