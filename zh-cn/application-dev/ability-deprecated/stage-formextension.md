...
- 卡片需要在应用配置文件module.json5中的extensionAbilities标签下，配置ExtensionAbility相关信息，内部字段结构说明：
  ...
  | srcEntry    | 表示ExtensionAbility所对应的生命周期代码的路径，该标签不可缺省。 | 字符串     | 否                   |
...
- [`FormExtAbility`：Stage模型卡片（ArkTS）（API9）](https://gitee.com/openharmony/applications_app_samples/tree/OpenHarmony-4.0-Beta2/code/SuperFeature/Widget/FormExtAbility)