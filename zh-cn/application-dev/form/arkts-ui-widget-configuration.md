...
### 配置文件字段说明
   ...
   **表1** 卡片form_config.json配置文件
   ...
   |renderingMode|表示卡片的渲染模式，取值范围如下：<br/>-&nbsp;autoColor：自动模式，表示卡片中的颜色和图片允许卡片使用方修改，卡片添加至桌面或锁屏场景下可以配置为该模式。<br/>-&nbsp;fullColor：全色模式，表示仅允许由卡片提供方指定卡片中使用的颜色和图片，不允许被卡片使用方修改，卡片添加至桌面场景下可以配置为该模式。<br/>-&nbsp;singleColor：单色模式，卡片添加到锁屏场景下可以配置为该模式。	|字符串|可缺省，缺省值为“fullColor”。|
