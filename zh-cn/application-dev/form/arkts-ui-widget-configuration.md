...
### 配置文件字段说明
   ...
   **表1** 卡片form_config.json配置文件
   ...
   | displayName | 表示卡片的展示名称。主要在卡片管理页面显示，对应[卡片预览页面](./formkit-overview.md#卡片使用场景)中的<!--RP3-->"ArkTSCard"<!--RP3End-->，用于展示卡片信息，建议能够体现卡片的核心功能或用途。支持字符串或字符串资源索引，建议使用字符串资源索引方式声明，以支持完整多语言能力。字符串最小长度为1字节，最大长度为30字节。 | 字符串 | 否 |
   | description | 表示卡片的描述。用于在卡片管理页面展示卡片功能描述，对应Form Kit简介中[卡片预览页面](./formkit-overview.md#卡片使用场景)的"This is a ArkTS card game by canvas."。支持字符串或字符串资源索引，建议使用字符串资源索引方式声明，以支持完整多语言能力。字符串最大长度为255字节。 | 字符串 | 可缺省，缺省为空。 |
