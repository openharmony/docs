...
趣味交互类型互动卡片当前仅支持基于鸿蒙快游戏引擎开发的卡片小游戏，互动卡片默认处于非激活态，当用户点击卡片时，卡片切换为激活态，开始游戏。在游戏过程中，用户可以点击“暂停”按钮进入暂停态。在暂停态，用户可以通过“继续游戏”和“停止游戏”按钮选择返回激活态继续游戏，或者结束游戏。
...
在此状态下，卡片UI由卡片提供方基于鸿蒙快游戏引擎所开发的卡片小游戏页面承载。此时系统支持游戏内容渲染到卡片范围之外的“破框”效果。
...
1. 卡片的有效交互热区始终保持卡片自身大小，卡片以外区域仅做UI呈现，交互事件直接由系统响应，事件分发时跳过当前卡片。
...
端侧卡片配置开发可参考[趣味交互卡片配置指导](arkts-ui-widget-configuration.md#funinteractionparams标签)。