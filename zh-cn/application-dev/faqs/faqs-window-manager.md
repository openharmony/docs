...
## 窗口[Orientation](../reference/apis-arkui/js-apis-window.md#orientation9)枚举值8（AUTO_ROTATION_RESTRICTED）和枚举值13~16（USER_ROTATION_XXX）的区别

1. 窗口设置枚举值8，会跟随传感器自动旋转，可以旋转到竖屏、横屏、反向竖屏、反向横屏四个方向，且受控制中心的旋转开关控制
2. 窗口设置枚举值13~16，会临时旋转到指定方向（如：13会临时旋转到竖屏），之后跟随传感器自动旋转，受控制中心的旋转开关控制，且可旋转方向受系统判定（同UNSPECIFIED）。

枚举值13~16和枚举值8的主要区别是，调用枚举值13~16时会临时旋转到指定方向，且旋转方向具有延续性，即应用退后台再恢复至前台时会延续之前的方向。

**场景举例：**

1. 手机竖屏，关闭旋转锁定开关 -> 应用设置方向为8 -> 将手机旋转为竖屏（**应用方向为横屏**） -> 应用退出后台进入桌面（方向为竖屏） -> 应用切换至前台（**应用方向为竖屏**） 
2. 手机竖屏，关闭旋转锁定开关 -> 应用设置方向为13 -> 将手机旋转为竖屏（**应用方向为横屏**） -> 应用退出后台进入桌面（方向为竖屏） -> 应用切换至前台（**应用方向为横屏**） 

| 名称                               | 值  | 可旋转方向           | 是否跟随传感器自动旋转 | 是否受旋转开关控制 | 方向延续性 |
|----------------------------------|----|-----------------|-------------|-----------|-------|
| AUTO_ROTATION_RESTRICTED         | 8  | 横屏、竖屏、反向竖屏、反向横屏 | 是           | 是         | 否     |
| USER_ROTATION_PORTRAIT           | 13 | 受系统判定           | 是           | 是         | 是     |
| USER_ROTATION_LANDSCAPE          | 14 | 受系统判定           | 是           | 是         | 是     |
| USER_ROTATION_PORTRAIT_INVERTED  | 15 | 受系统判定           | 是           | 是         | 是     |
| USER_ROTATION_LANDSCAPE_INVERTED | 16 | 受系统判定           | 是           | 是         | 是     |
