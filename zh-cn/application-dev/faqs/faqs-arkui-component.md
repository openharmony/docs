...
系统基于的芯片平台除通用的CPU/GPU计算单元外，还提供Hardware Composer（简称HWC）专用硬件合成器用于图形合成送显。与通用计算单元相比，HWC在图层叠加场景中具有更高的处理效率和更低的能耗。XComponent组件的Surface模式是否能通过HWC硬件合成通道，取决于芯片平台能力、产品形态以及应用绘制行为。
