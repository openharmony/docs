...
| 参数名      | 类型                                  | 必填   | 说明                                                                                                                             |
|----------|-------------------------------------| ---- |--------------------------------------------------------------------------------------------------------------------------------|
| pid      | number                              | 是    | 需要被压制子进程的进程号，[OH_Ability_StartNativeChildProcess](../apis-ability-kit/c-apis-ability-childprocess.md#)接口创建子进程后的pid参数，即为子进程进程号。 |
...
| 名称                   |  值      | 说明                                                                             |
|----------------------| -------- |--------------------------------------------------------------------------------|
| PROCESS_BACKGROUND   | 1        | 该档位相较PROCESS_INACTIVE压制效果更显著，获取到的CPU资源更少，推荐执行用户无感知业务的后台子线程使用此档位，例如：处于后台的图文页面。  |
| PROCESS_INACTIVE     | 2        | 正在执行播放音频、导航等用户可感知业务的后台子进程，推荐设置此档位。                                             |
