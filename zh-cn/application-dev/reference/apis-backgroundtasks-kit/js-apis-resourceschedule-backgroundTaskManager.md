...
| 名称                     | 值  | 说明                    |
| ----------------------- | ---- | --------------------- |
| DATA_TRANSFER           | 1    | 数据传输。<br/>**说明：** 在数据传输时，应用需要更新进度，如果进度长时间（超过10分钟）不更新，数据传输的长时任务会被取消。更新进度的通知类型必须为实况窗，具体实现可参考[startBackgroundRunning()] (#backgroundtaskmanagerstartbackgroundrunning12)中的示例。       |
| AUDIO_PLAYBACK          | 2    | 音视频播放。<br/>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。 <br/>**说明：** 从API version 20开始，申请AUDIO_PLAYBACK类型长时任务时，若不接入AVSession，由长时任务发送通知；若接入AVSession，由AVSession发送通知。<br/>对于API version 20之前的版本，必须接入AVSession，长时任务不发送通知。                 |
