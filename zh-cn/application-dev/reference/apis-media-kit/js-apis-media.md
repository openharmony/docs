...
| callback | [Callback](../apis-basic-services-kit/js-apis-base.md#callback) | 否   | 进度更新事件回调方法，progress: number，表示当前转码进度 |
...
| 名称            | 类型                                     | 必填 | 说明                                                         |
| --------------- | ---------------------------------------- | ---- | ------------------------------------------------------------ |
| audioBitrate | number     |  否 | 输出音频的码率，单位为比特率（bps）。用户不设置，则默认设置为48Kbps|
| audioCodec | [CodecMimeType](#codecmimetype8)     | 否  | 输出音频的编码格式，当前仅支持AAC。                   |
| fileFormat         | [ContainerFormatType](#containerformattype8) | 是   | 输出视频文件的封装格式，当前视频文件仅支持MP4。|
| videoBitrate         | number |  否   | 输出视频的码率，单位为比特率（bps）。用户不设置，则默认码率按输出视频的分辨率设置，[240p，480P]默认码率值为1Mbps，(480P,720P]默认码率值为2Mbps，(720P,1080P]默认码率值为4Mbps，1080P及以上默认值为8Mbps。|
| videoCodec        | [CodecMimeType](#codecmimetype8) |  否   | 输出视频的编码格式，当前仅支持AVC和HEVC。|
| videoFrameWidth        | number |   否   | 输出视频帧的宽，单位为像素（px）。用户不设置，则默认设置为源视频帧的宽|
| videoFrameHeight        | number |   否   | 输出视频帧的高，单位为像素（px）。用户不设置，则默认设置为源视频帧的高|
