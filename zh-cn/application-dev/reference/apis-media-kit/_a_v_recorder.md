...
| 名称 | 描述 | 
| -------- | -------- |
| [avrecorder.h](avrecorder_8h.md) | 定义AVRecorder接口。应用可使用AVRecorder提供的接口录制媒体数据。 | 
| [avrecorder_base.h](avrecorder__base_8h.md) | 定义了AVRecorder的结构体和枚举。 | 
...
| 名称 | 描述 | 
| -------- | -------- |
| struct  [OH_AVRecorder_Profile](_o_h___a_v_recorder___profile.md) | 定义音视频录制的详细参数。通过参数设置可以选择仅录制音频或仅录制视频：audioBitrate 或 audioChannels 为 0 时，不录制音频；videoFrameWidth 或 videoFrameHeight 为 0 时，不录制视频。 | 
...
| struct  [OH_AVRecorder_Metadata](_o_h___a_v_recorder___metadata.md) | 提供元数据信息。 | 
| struct  [OH_AVRecorder_Config](_o_h___a_v_recorder___config.md) | 提供AVRecorder的配置定义。 | 
...
| typedef enum [OH_AVRecorder_CodecMimeType](#oh_avrecorder_codecmimetype-1) [OH_AVRecorder_CodecMimeType](#oh_avrecorder_codecmimetype) | 表示枚举编码器的 MIME 类型。 | 
...
| OH_AVErrCode [OH_AVRecorder_SetStateCallback](#oh_avrecorder_setstatecallback) ([OH_AVRecorder](#oh_avrecorder) \*recorder, [OH_AVRecorder_OnStateChange](#oh_avrecorder_onstatechange) callback, void \*userData) | 设置状态回调函数，使应用能够响应AVRecorder的状态变化事件。必须在调用[OH_AVRecorder_Start](#oh_avrecorder_start)之前调用此接口。 | 
...
设置状态回调函数，使应用能够响应AVRecorder的状态变化事件。必须在调用[OH_AVRecorder_Start](#oh_avrecorder_start)之前调用此接口。
...
设置错误回调函数，使应用可以响应AVRecorder生成的错误事件。必须在调用[OH_AVRecorder_Start](#oh_avrecorder_start)之前设置此接口。
...
设置 URI 回调函数，使应用能够响应 AVRecorder 生成的 URI 事件。此接口需在调用 [OH_AVRecorder_Start](#oh_avrecorder_start) 之前设置。
...
AV_ERR_INVALID_VAL：输入的 recorder 或回调函数为 nullptr。
