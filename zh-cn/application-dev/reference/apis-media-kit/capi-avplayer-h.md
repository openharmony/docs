...
根据指定的{AVPlaybackSpeed}，设置播放器的播放速率。
...
| -- | -- |
| [OH_AVPlayer](capi-oh-avplayer.md) *player | 指向OH_AVPlayer实例的指针。 |
| interruptMode | player音频流使用的音效模式。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | AV_ERR_OK：执行成功。<br>         AV_ERR_NO_MEMORY：输入分配内存失败。<br>         AV_ERR_INVALID_VAL： 输入player为空指针或者函数执行失败。 |
