...
| [OH_AVPlayer *OH_AVPlayer_Create(void)](#oh_avplayer_create) | - | 创建播放器。 |
...
| [OH_AVErrCode OH_AVPlayer_Prepare(OH_AVPlayer *player)](#oh_avplayer_prepare) | - | 准备播放环境，异步缓存媒体数据。此函数必须在{@link SetSource}之后调用 |
| [OH_AVErrCode OH_AVPlayer_Play(OH_AVPlayer *player)](#oh_avplayer_play) | - | 开始播放。此函数必须在{@link Prepare}之后调用。如果播放器状态为<Prepared>。调用此函数开始播放。 |
...
| [OH_AVErrCode OH_AVPlayer_Reset(OH_AVPlayer *player)](#oh_avplayer_reset) | - | 将播放器恢复到初始状态。函数调用完成后，调用{@link SetSource}添加播放源。调用{@link Prepare}后，再调用{@link Play}重新开始播放。 |
| [OH_AVErrCode OH_AVPlayer_Release(OH_AVPlayer *player)](#oh_avplayer_release) | - | 异步释放播放器资源。异步释放可以提升性能，但不能确保播放画面的 SurfaceBuffer 已释放。调用者需要确保播放画面窗口的生命周期安全。 |
...
| [OH_AVErrCode OH_AVPlayer_SetVolume(OH_AVPlayer *player, float leftVolume, float rightVolume)](#oh_avplayer_setvolume) | - | 设置播放器的音量。可以在播放或暂停的过程中使用。<0>表示无声音。,<1>为原始值。<br> |
...
| [OH_AVErrCode OH_AVPlayer_SetPlaybackSpeed(OH_AVPlayer *player, AVPlaybackSpeed speed)](#oh_avplayer_setplaybackspeed) | - | 设置播放器播放速率。 |
| [OH_AVErrCode OH_AVPlayer_SetPlaybackRate(OH_AVPlayer *player, float rate)](#oh_avplayer_setplaybackrate) | - | 设置播放器播放速率。支持的状态：已准备/正在播放/已暂停/已完成。 |
| [OH_AVErrCode OH_AVPlayer_GetPlaybackSpeed(OH_AVPlayer *player, AVPlaybackSpeed *speed)](#oh_avplayer_getplaybackspeed) | - | 获取当前播放器播放速率。 |
...
| [OH_AVErrCode OH_AVPlayer_SelectBitRate(OH_AVPlayer *player, uint32_t bitRate)](#oh_avplayer_selectbitrate) | - | 设置hls播放器使用的码率。播放比特率，以比特/秒为单位。仅对HLS协议网络流有效。默认情况下，播放器会根据网络连接情况选择合适的码率和速度。通过INFO_TYPE_BITRATE_COLLECT上报有效码率链表设置并选择指定的码率，选择小于和最接近的码率。准备好后，读取以查询当前选择的比特率。 |
...
| [OH_AVErrCode OH_AVPlayer_SetPlayerCallback(OH_AVPlayer *player, AVPlayerCallback callback)](#oh_avplayer_setplayercallback) | - | 设置播放器回调方法。 |
| [OH_AVErrCode OH_AVPlayer_SelectTrack(OH_AVPlayer *player, int32_t index)](#oh_avplayer_selecttrack) | - | 选择音频或字幕轨道。默认播放第一个带数据的音轨，不播放字幕轨迹。设置生效后，原音轨将失效。请设置字幕处于准备/播放/暂停/完成状态，并将音轨设置为准备状态。 |
| [OH_AVErrCode OH_AVPlayer_DeselectTrack(OH_AVPlayer *player, int32_t index)](#oh_avplayer_deselecttrack) | - | 取消选择当前音频或字幕轨道。 |
| [OH_AVErrCode OH_AVPlayer_GetCurrentTrack(OH_AVPlayer *player, int32_t trackType, int32_t *index)](#oh_avplayer_getcurrenttrack) | - | 获取当前有效的轨道索引。请将状态设置为准备/正在播放/暂停/完成状态。 |
...
此函数必须在[OH_AVPlayer_Prepare](#oh_avplayer_prepare)之后调用。
...
函数调用完成后，调用SetSource添加播放源。调用[OH_AVPlayer_Prepare](#oh_avplayer_prepare)后，再调用[OH_AVPlayer_Play](#oh_avplayer_play)重新开始播放。
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：执行成功。<br>         AV_ERR_INVALID_VAL：输入player为空指针，或者player SetVolume执行失败。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：执行成功。<br>         AV_ERR_INVALID_VAL：输入player为空指针，或者player GetCurrentTime执行失败。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：获取视频宽度；<br>         AV_ERR_INVALID_VAL：输入player为空指针。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：获取视频高度；<br>         AV_ERR_INVALID_VAL：输入player为空指针。 |
...
| -- | -- |
| player | 指向OH_AVPlayer实例的指针。 |
| speed可以设置速率模式[AVPlaybackSpeed](capi-avplayer-base-h.md#avplaybackspeed)。 |  |

**返回：**

| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：设置播放速率成功；<br>         AV_ERR_INVALID_VAL：输入player为空指针。 |
...
设置播放器播放速率。支持的状态：已准备/正在播放/已暂停/已完成。
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode)：<br>         AV_ERR_OK = 0 ：设置播放速率成功；<br>         AV_ERR_OPERATE_NOT_PERMIT = 2 ：如果在不支持的状态下调用或在直播期间调用；<br>         AV_ERR_INVALID_VAL = 3 ：输入player为空指针，或者速率超出范围。 |
...
| -- | -- |
| player | 指向OH_AVPlayer实例的指针。 |
| [AVPlaybackSpeed](capi-avplayer-base-h.md#avplaybackspeed) *speed | 可以获取的速率模式[AVPlaybackSpeed](capi-avplayer-base-h.md#avplaybackspeed) |

**返回：**

| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：获取播放速率成功；<br>         AV_ERR_INVALID_VAL：输入player为空指针，或者player GetPlaybackSpeed执行失败。 |
...
| -- | -- |
| OH_AVPlayer *player | 指向OH_AVPlayer实例的指针。 |
| [OH_AudioStream_Usage](../AudioKit/capi-native-audiostream-base-h.md#oh_audiostream_usage) streamUsage | player音频流设置的类型[OH_AudioStream_Usage](../AudioKit/capi-native-audiostream-base-h.md#oh_audiostream_usage)。 |

**返回：**

| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：设置音频流类型成功；<br>         AV_ERR_INVALID_VAL：输入player为空指针或者streamUsage值无效。 |
...
| -- | -- |
| OH_AVPlayer *player | 指向OH_AVPlayer实例的指针。 |
| [OH_AudioStream_VolumeMode](../AudioKit/capi-native-audiostream-base-h.md#oh_audiostream_volumemode) volumeMode | 要设置的音频流音量模式[OH_AudioStream_VolumeMode](../AudioKit/capi-native-audiostream-base-h.md#oh_audiostream_volumemode)。 |

**返回：**

| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | 函数结果代码：<br>     AV_ERR_OK： 设置音频流音量模式成功。<br>     AV_ERR_INVALID_VAL： 输入player为空指针或者volumeMode值无效。<br>     [AV_ERR_INVALID_STATE](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) 函数在无效状态下调用，应先处于准备状态。<br>     [AV_ERR_SERVICE_DIED](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) 系统错误。 |
...
| -- | -- |
| OH_AVPlayer *player | 指向OH_AVPlayer实例的指针。 |
| [OH_AudioInterrupt_Mode](../AudioKit/capi-native-audiostream-base-h.md#oh_audiointerrupt_mode) interruptMode | player音频流使用的打断模式[OH_AudioStream_Usage](../AudioKit/capi-native-audiostream-base-h.md#oh_audiostream_usage)。 |

**返回：**

| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：设置音频流的打断模式成功；<br>         AV_ERR_INVALID_VAL：输入player为空指针，或者interruptMode值无效。 |
...
| -- | -- |
| OH_AVPlayer *player | 指向OH_AVPlayer实例的指针。 |
| interruptMode | player音频流使用的音效模式[OH_AudioStream_AudioEffectMode](../AudioKit/capi-native-audiostream-base-h.md#oh_audiostream_audioeffectmode)。 |

**返回：**

| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：设置音频流的音效模式成功；<br>         AV_ERR_INVALID_VAL：输入player为空指针，或者effectMode值无效。 |
...
设置hls播放器使用的码率。播放比特率，以比特/秒为单位。仅对HLS协议网络流有效。默认情况下，播放器会根据网络连接情况选择合适的码率和速度。通过INFO_TYPE_BITRATE_COLLECT上报有效码率链表设置并选择指定的码率，选择小于和最接近的码率。准备好后，读取以查询当前选择的比特率。
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：设置码率成功；<br>         AV_ERR_INVALID_VAL：输入player为空指针，或者player SelectBitRate执行失败。 |
...
| -- | -- |
| player | 指向OH_AVPlayer实例的指针。 |
| window指向OHNativeWindow实例的指针，参见{@link | OHNativeWindow} |

**返回：**

| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：设置播放画面窗口成功；<br>         AV_ERR_INVALID_VAL：输入player为空指针，输入window为空指针或者player SetVideoSurface执行失败。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：获取当前时长；<br>         AV_ERR_INVALID_VAL：输入player为空指针，或者player GetDuration执行失败。 |
...
| 类型 | 说明 |
| -- | -- |
| bool | 如果循环播放，则返回true；如果不是循环播放或者输入player为空指针则返回false。 |
...
选择音频或字幕轨道。默认播放第一个带数据的音轨，不播放字幕轨迹。设置生效后，原音轨将失效。请设置字幕处于准备/播放/暂停/完成状态，并将音轨设置为准备状态。
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：成功选择；<br>         AV_ERR_INVALID_VAL：输入player为空指针，或者player SelectTrack执行失败。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：成功取消选择；<br>         AV_ERR_INVALID_VAL：输入player为空指针，或者player DeselectTrack执行失败。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：成功获取；<br>         AV_ERR_INVALID_VAL：输入player为空指针，或者player GetCurrentTrack执行失败。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：成功设置；<br>         AV_ERR_INVALID_VAL：输入player为空指针，callback为空指针，player SetDrmSystemInfoCallback，<br> SetDrmSystemInfoCallback或SetDrmSystemInfoCallback执行失败。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：成功获取；<br>         AV_ERR_INVALID_VAL：输入player为空指针或者内存不足。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK：成功设置；<br>         AV_ERR_INVALID_VAL：输入player为空指针或者player SetDecryptionConfig执行失败。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_AVErrCode](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) | AV_ERR_OK： 成功设置；<br>         [AV_ERR_NO_MEMORY](../AVCodecKit/capi-native-averrors-h.md#oh_averrcode) 输入分配内存失败；<br>         AV_ERR_INVALID_VAL： 输入player为空指针或者函数执行失败。 |
