...
| GET_USAGE | 获取本地窗口读写方式， 函数中的可变参数是 [输出] uint64_t \*usage，获取类型详细请参见[OH_NativeBuffer_Usage](_o_h___native_buffer.md#oh_nativebuffer_usage)。 |
| SET_USAGE | 设置本地窗口缓冲区读写方式， 函数中的可变参数是 [输入] uint64_t usage，设置类型详细请参见[OH_NativeBuffer_Usage](_o_h___native_buffer.md#oh_nativebuffer_usage)。 |
