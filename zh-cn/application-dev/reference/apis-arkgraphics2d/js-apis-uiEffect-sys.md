...
| 名称  | 类型   | 只读 | 可选 | 说明                                     |
| ----- | ------ | ---- | ---- | ---------------------------------------- |
| red   | number | 否   | 否   | 颜色的R分量（红色）。值大于等于0，当值小于0时无效。 |
| green | number | 否   | 否   | 颜色的G分量（绿色）。值大于等于0，当值小于0时无效。|
| blue  | number | 否   | 否   | 颜色的B分量（蓝色）。值大于等于0，当值小于0时无效。 |
| alpha | number | 否   | 否   | 颜色的A分量（透明度）。值大于等于0，当值小于0时无效。 |
...
| radiusX | number  | 是 | 设置椭圆的X方向的半径，半径为1等于组件的高度。<br/>取值范围[0, 10]，可取浮点数，超出边界会在实现时自动截断。 |
| radiusY | number  | 是 | 设置椭圆的Y方向的半径，半径为1等于组件的高度。<br/>取值范围[0, 10]，可取浮点数，超出边界会在实现时自动截断。 |
