...
| -------- | -------- |
| cameraManager | 相机管理器实例[Camera_Manager](#camera_manager)。 | 
| camera | 用于查询的[Camera_Device](_camera___device.md)相机设备列表，作为入参应当默认为由[OH_CameraManager_GetCameraDevice](#oh_cameramanager_getcameradevice)获取的前置与后置两个相机设备。 | 
| deviceSize | 用于查询的相机设备列表长度, 作为入参应当默认设置为2(其含义为前置与后置两个用于并发的相机设备)。 | 
