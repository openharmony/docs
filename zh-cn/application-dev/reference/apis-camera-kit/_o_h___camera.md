...
| -------- | -------- |
| cameraManager | 相机管理器实例[Camera_Manager](#camera_manager)。 | 
| camera | 用于查询的[Camera_Device](_camera___device.md)数组，作为入参应当默认为由[OH_CameraManager_GetCameraDevice](#oh_cameramanager_getcameradevice)获取的一前一后两个相机。 | 
| deviceSize | 用于查询的相机数组长度, 作为入参应当默认设置为2(其含义为一前一后两个用于并发的相机)。 | 
