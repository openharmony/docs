...
| mirror   | boolean                         | 否   | 是   | 镜像使能开关(默认关)。使用之前需要使用[isMirrorSupported](arkts-apis-camera-PhotoOutput.md#ismirrorsupported)进行判断是否支持。true表示使能，false反之。 |
...
| timestamp | number | 否   | 否   | 快门时间戳（单位：毫秒）。|
...
| timestamp   | number                                      |  是  |  否  | 当前时间戳（单位：毫秒）。|
