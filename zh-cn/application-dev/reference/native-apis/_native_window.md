...
| GET_FORMAT | 获取本地窗口缓冲区格式， 函数中的可变参数是 [输出] int32_t \*format，获取类型详细请参见[OH_NativeBuffer_Format](native__buffer_8h.md#oh_nativebuffer_format)。 |
| SET_FORMAT | 设置本地窗口缓冲区格式， 函数中的可变参数是 [输入] int32_t format，设置类型详细请参见[OH_NativeBuffer_Format](native__buffer_8h.md#oh_nativebuffer_format)。 |
| GET_USAGE | 获取本地窗口读写方式， 函数中的可变参数是 [输出] int32_t \*usage，获取类型详细请参见[OH_NativeBuffer_Usage](native__buffer_8h.md#oh_nativebuffer_usage)。 |
| SET_USAGE | 设置本地窗口缓冲区读写方式， 函数中的可变参数是 [输入] int32_t usage，设置类型详细请参见[OH_NativeBuffer_Usage](native__buffer_8h.md#oh_nativebuffer_usage)。 |
...
从API version 12开始废弃，可通过[OH_NativeImage_AcquireNativeWindow](_o_h___native_image#oh_nativeimage_acquirenativewindow)获取，或通过XComponent创建。
...
从API version 12开始废弃，使用**OH_NativeWindow_CreateNativeWindowBufferFromNativeBuffer**替代。
