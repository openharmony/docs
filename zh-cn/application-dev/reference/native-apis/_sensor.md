...
| 名称 | 描述 | 
| -------- | -------- |
| [OH_Sensor_GetInfos](#oh_sensor_getinfos) ([Sensor_Info](#sensor_info) \*\*infos, uint32_t \*count) | [Sensor_Result](#sensor_result)<br/>获取设备上所有传感器的信息。  | 
| [OH_Sensor_Subscribe](#oh_sensor_subscribe) (const [Sensor_SubscriptionId](#sensor_subscriptionid) \*id, const [Sensor_SubscriptionAttribute](#sensor_subscriptionattribute) \*attribute, const [Sensor_Subscriber](#sensor_subscriber) \*subscriber) | [Sensor_Result](#sensor_result)<br/>订阅传感器数据。系统将以指定的频率向用户报告传感器数据。 订阅加速度传感器，需要申请ohos.permission.ACCELEROMETER权限； 订阅陀螺仪传感器，需要申请ohos.permission.GYROSCOPE权限； 订阅计步器相关传感器时，需要申请ohos.permission.ACTIVITY_MOTION权限； 订阅与健康相关的传感器时，比如心率传感器，需要申请ohos.permission.READ_HEALTH_DATA权限，否则订阅失败。 订阅其余传感器不需要申请权限。  | 
| [OH_Sensor_Unsubscribe](#oh_sensor_unsubscribe) (const [Sensor_SubscriptionId](#sensor_subscriptionid) \*id, const [Sensor_Subscriber](#sensor_subscriber) \*subscriber) | [Sensor_Result](#sensor_result)<br/>取消订阅传感器数据。 取消订阅加速度计传感器，需要申请ohos.permission.ACCELEROMETER权限； 取消订阅陀螺仪传感器，需要申请ohos.permission.GYROSCOPE权限； 取消订阅计步器相关传感器时，需要申请ohos.permission.ACTIVITY_MOTION权限； 取消订阅与健康相关的传感器时，需要申请ohos.permission.READ_HEALTH_DATA权限，否则取消订阅失败。 取消订阅其余传感器不需要申请权限。  | 
...
| 枚举值 | 描述 | 
| -------- | -------- |
| SENSOR_ACCURACY_UNRELIABLE  | 传感器数据不可靠。有可能传感器不与设备接触而进行测量。<br/>自从<br/>11 | 
| SENSOR_ACCURACY_LOW  | 传感器数据精度较低。数据在使用前必须根据环境进行校准。<br/>自从<br/>11 | 
| SENSOR_ACCURACY_MEDIUM  | 传感器数据处于中等精度水平。建议用户在使用前根据实际环境进行数据校准。<br/>自从<br/>11 | 
| SENSOR_ACCURACY_HIGH  | 传感器数据具有很高的精度。数据可以直接使用。<br/>自从<br/>11 | 
...
| 枚举值 | 描述 | 
| -------- | -------- |
| SENSOR_SUCCESS  | 操作成功。<br/>自从<br/>11 | 
| SENSOR_PERMISSION_DENIED  | 权限验证失败。<br/>自从<br/>11 | 
| SENSOR_PARAMETER_ERROR  | 参数检查失败。例如，没有传入强制参数，或者传入的参数类型不正确。<br/>自从<br/>11 | 
| SENSOR_SERVICE_EXCEPTION  | 传感器服务异常。<br/>自从<br/>11 | 
...
| 枚举值 | 描述 | 
| -------- | -------- |
| SENSOR_TYPE_ACCELEROMETER  | 加速度传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_GYROSCOPE  | 陀螺仪传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_AMBIENT_LIGHT  | 环境光传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_MAGNETIC_FIELD  | 地磁传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_BAROMETER  | 气压传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_HALL  | 霍尔传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_PROXIMITY  | 接近光传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_ORIENTATION  | 方向传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_GRAVITY  | 重力传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_ROTATION_VECTOR  | 旋转矢量传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_PEDOMETER_DETECTION  | 计步器检测传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_PEDOMETER  | 计步器传感器。<br/>自从<br/>11 | 
| SENSOR_TYPE_HEART_RATE  | 心率传感器。<br/>自从<br/>11 | 
