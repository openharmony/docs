...
| onEmptyAreaClick<sup>13+</sup>   | [EmptyAreaClickCallbck](#emptyareaclickcallbck) | 否   | 点击相册组件空白区域时产生的回调事件，将该次点击通知给使用方。 |
  ...
  build() {
    Stack() {
      AlbumPickerComponent({
        ...
        onEmptyAreaClick(): (): void => this.onEmptyAreaClick(albumInfo),
