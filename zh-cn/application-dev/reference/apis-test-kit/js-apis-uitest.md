...
| 名称 | 类型   | 只读 |  可选 | 说明        |
| ---- | ------ | ---- | ---- |-----------|
| x    | number |  否   | 否   | 坐标点的横坐标，取值大于0的整数。<br> **原子化服务：**  从API version 11开始，该接口支持在原子化服务中使用。从API version 20开始，该属性不再为只读属性。 |
| y    | number |  否   | 否   | 坐标点的纵坐标，取值大于0的整数。<br> **原子化服务：**  从API version 11开始，该接口支持在原子化服务中使用。从API version 20开始，该属性不再为只读属性。|
| displayId<sup>20+</sup>    | number | 否    | 是   | 坐标点所属的屏幕ID，取值范围：大于等于0的整数。默认值为设备默认屏幕ID。<br> **原子化服务：** 从API version 20开始，该接口支持在原子化服务中使用。|
