...
| 参数名   | 类型   | 必填 | 说明                                                         |
| -------- | ------ | ---- | ------------------------------------------------------------ |
| idleTime | number | 是   | 空闲时间的阈值。在这个时间段控件不发生变化，视为该控件空闲，单位：毫秒（ms），取值大于等于0。 |
| timeout  | number | 是   | 等待空闲的最大时间，单位：毫秒（ms），取值大于等于0。                    |
...
| speed   | number           | 是   | 滑动速率，范围：200-40000，不在范围内设为默认值为600，单位：像素点/秒（pixel/s）。 |
