...
断连远端设备。需在通过connect成功建立连接后调用，用于断开已建立的远端设备连接。使用Promise异步回调。
...
| 名称 | 值 | 说明 |
| -------- | -------- | -------- |
| BASIC | 0 | 表示基础模式，无数据重传机制。适用于对数据可靠性要求不高、但对传输时延和吞吐量更敏感的业务场景。 |
| RELIABLE | 1 | 表示可靠模式，有数据重传机制。适用于对数据完整性要求高、不允许丢失的业务场景，可靠模式下传输时延相对更高。 |
