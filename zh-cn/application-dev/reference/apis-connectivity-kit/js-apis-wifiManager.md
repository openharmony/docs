...
| ipAddress | number | 是 | 否 | WLAN连接的IP地址。<br>1、wifi连接信息和关于本机里的状态信息可以查看。<br>2、ipAddress值为number类型，需要转换为IP常用格式|
...
| **名称** | **类型** | **可读** | **可写** | **说明** |
| -------- | -------- | -------- | -------- | -------- |
| ipAddress | number | 是 | 否 | IP地址。(ipAddress值为number类型，需要转换为IP常用格式)|
