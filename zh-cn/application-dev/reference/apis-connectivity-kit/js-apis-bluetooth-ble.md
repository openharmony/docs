...
| 名称        | 类型                    | 只读 | 可选   | 说明                                     |
| --------- | ----------------------- | ---- | ---- | -------------------------------------- |
| interval  | number                  | 否 | 是    | 扫描结果上报的延迟时间（单位：毫秒），默认值为0。此参数需要配合reportMode字段使用。<br>- 在普通扫描上报模式下，若该值为0，扫描到符合条件的广播报文后，会立刻上报扫描结果；若为空或其他值，则不生效。<br>- 在批量扫描上报模式下，该值应当不小于5000，扫描到符合条件的广播报文后，会先将扫描结果放入缓存队列，延时上报；若为空或其他值，则将被设置为5000后生效。<br>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。                    |
...
| 名称      | 值    | 说明                           |
| --------  | ---- | ------------------------------ |
| NORMAL  | 1    | 常规扫描上报模式，扫描到符合过滤条件的BLE广播报文后就会立刻上报。<br>- 常规扫描模式下，[ScanOptions](#scanoptions)中的interval字段的取值应当为0毫秒；若为空或其他值时，该参数不生效。<br>**原子化服务API**：从API version 15开始，该接口支持在原子化服务中使用。       |
| BATCH  | 2    | 批量扫描上报模式。<br>- 批量扫描上报模式的设计目标是通过降低蓝牙芯片上报扫描结果的频率，使系统可以更长时间保持在休眠状态，从而降低整机功耗。<br>- 在批量扫描上报模式下，扫描到符合过滤条件的BLE广播报文后不会立刻上报，而是缓存一定时间（[ScanOptions](#scanoptions)中的interval字段）后一次上报。<br>- 在批量扫描上报模式下，[ScanOptions](#scanoptions)中的interval字段的取值应当不小于5000毫秒；若为空或其他值时，将被设置为5000毫秒后生效。 <br>**原子化服务API**：从API version 20开始，该接口支持在原子化服务中使用。       |
