...
| 名称        | 类型   | 只读   | 可选   | 说明      |
| --------- | ------ | ------- | ------- | ------- |
| key       | number  | 否      | 否      | 按键键值。<br>当前仅支持[KEYCODE_VOLUME_UP](js-apis-keycode.md#keycode)键、[KEYCODE_VOLUME_DOWN](js-apis-keycode.md#keycode)键、[KEYCODE_MEDIA_PLAY_PAUSE](js-apis-keycode.md#keycode)键、[KEYCODE_MEDIA_NEXT](js-apis-keycode.md#keycode)键和[KEYCODE_MEDIA_PREVIOUS](js-apis-keycode.md#keycode)键。 |
| action    | number  | 否      | 否      | 按键事件类型。当前仅支持取值为1和2，取值为1表示订阅按键的按下事件，取值为2表示订阅按键的按下事件和按键抬起事件。 |
...
| callback   | Callback&lt;[KeyEvent](./js-apis-keyevent.md#keyevent)&gt; | 是    | 回调函数，用于返回按键事件。不同的注册需要使用不同的callback，否则注册不生效。 |
