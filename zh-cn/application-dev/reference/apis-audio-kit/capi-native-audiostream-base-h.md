...
| [OH_AudioStream_Event](#oh_audiostream_event) | OH_AudioStream_Event | 定义音频事件。通常用来描述音频事件。 |
| [OH_AudioInterrupt_ForceType](#oh_audiointerrupt_forcetype) | OH_AudioInterrupt_ForceType | 定义音频中断类型。当用户监听到音频中断时，将获取此信息。此类型表示本次音频打断的操作是否已由系统强制执行，具体操作信息（如音频暂停、停止等）可通过[OH_AudioInterrupt_Hint](capi-native-audiostream-base-h.md#oh_audiointerrupt_hint)获取。 |
| [OH_AudioInterrupt_Hint](#oh_audiointerrupt_hint) | OH_AudioInterrupt_Hint | 定义音频中断提示类型。当用户监听到音频中断时，将获取此信息。此类型表示根据焦点策略，当前需要对音频流的具体操作（如暂停、调整音量等）。可以结合[OH_AudioInterrupt_ForceType](capi-native-audiostream-base-h.md#oh_audiointerrupt_forcetype)信息，判断该操作是否已由系统强制执行。 |
| [OH_AudioInterrupt_Mode](#oh_audiointerrupt_mode) | OH_AudioInterrupt_Mode | 定义音频中断模式。通常用来设置音频中断模式。 |
...
| [typedef int32_t (\*OH_AudioRenderer_WriteDataWithMetadataCallback)(OH_AudioRenderer* renderer,void* userData, void* audioData, int32_t audioDataSize, void* metadata, int32_t metadataSize)](#oh_audiorenderer_writedatawithmetadatacallback) | OH_AudioRenderer_WriteDataWithMetadataCallback | 这个函数指针将指向用于同时写入音频数据和元数据的回调函数。 |
| [typedef OH_AudioData_Callback_Result (\*OH_AudioRenderer_OnWriteDataCallback)(OH_AudioRenderer* renderer, void* userData,void* audioData, int32_t audioDataSize)](#oh_audiorenderer_onwritedatacallback) | OH_AudioRenderer_OnWriteDataCallback | 这个函数指针将指向用于写入音频数据的回调函数。这个函数类似于 OH_AudioRenderer_Callbacks_Struct.OH_AudioRenderer_OnWriteData 函数指针。但具有返回值，用于标识音频数据回调结果。该函数的返回结果表示填充到缓冲区的数据是否有效。如果结果无效，用户填写的数据将不被播放。 |
...
| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_TYPE_RENDERER = 1 | 该类型代表音频流是输出流。<br/>**起始版本：** 10 |
| AUDIOSTREAM_TYPE_CAPTURER = 2 | 该类型代表音频流是输入流。<br/>**起始版本：** 10 |
...
| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_SAMPLE_U8 = 0 | Unsigned 8位。<br/>**起始版本：** 10 |
| AUDIOSTREAM_SAMPLE_S16LE = 1 | Short 16位小端。<br/>**起始版本：** 10 |
| AUDIOSTREAM_SAMPLE_S24LE = 2 | Short 24位小端。<br/>**起始版本：** 10 |
| AUDIOSTREAM_SAMPLE_S32LE = 3 | Short 32位小端。<br/>**起始版本：** 10 |
...
| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_ENCODING_TYPE_RAW = 0 | PCM编码。<br/>**起始版本：** 10        |
...
定义音频流使用场景。通常用来描述音频输出流的使用场景。

**起始版本：** 10

| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_USAGE_UNKNOWN = 0 | 未知类型。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_MUSIC = 1 | 音乐。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_COMMUNICATION = 2 | VoIP语音通话。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_VOICE_ASSISTANT = 3 | 语音播报。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_ALARM = 4 | 闹钟。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_VOICE_MESSAGE = 5 | 语音消息。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_RINGTONE = 6 | 铃声。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_NOTIFICATION = 7 | 通知。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_ACCESSIBILITY = 8 | 无障碍。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_MOVIE = 10 | 电影或视频。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_GAME = 11 | 游戏。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_AUDIOBOOK = 12 | 有声读物（包括听书、相声、评书）、听新闻、播客等。<br/>**起始版本：** 10 |
| AUDIOSTREAM_USAGE_NAVIGATION = 13 | 导航。<br/>**起始版本：** 10 |
...
| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_LATENCY_MODE_NORMAL = 0 | 该模式代表一个普通时延的音频流。<br/>**起始版本：** 10 |
| AUDIOSTREAM_LATENCY_MODE_FAST = 1 | 该模式代表一个低时延的音频流。<br/>**起始版本：** 10 |
...
| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_DIRECT_PLAYBACK_NOT_SUPPORTED = 0 | 该模式代表不支持direct通路播放。<br/>**起始版本：** 19      |
| AUDIOSTREAM_DIRECT_PLAYBACK_BITSTREAM_SUPPORTED = 1 | 该模式代表支持不解码的direct通路播放。<br/>**起始版本：** 19   |
| AUDIOSTREAM_DIRECT_PLAYBACK_PCM_SUPPORTED = 2 | 该模式代表支持pcm编码的direct通路播放。<br/>**起始版本：** 19 |
...
| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_VOLUMEMODE_SYSTEM_GLOBAL = 0 | 系统级音量（默认模式）。<br/>**起始版本：** 18|
| AUDIOSTREAM_VOLUMEMODE_APP_INDIVIDUAL = 1 | 应用级音量（设置为该模式后可以通过提供的接口设置、查询应用音量）。<br/>**起始版本：** 18 |
...
| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_STATE_INVALID = -1 | 不合法的状态。<br/>**起始版本：** 10 |
| AUDIOSTREAM_STATE_NEW = 0 | 新创建时的状态。<br/>**起始版本：** 10      |
| AUDIOSTREAM_STATE_PREPARED = 1 | 准备状态。<br/>**起始版本：** 10         |
| AUDIOSTREAM_STATE_RUNNING = 2 | 工作状态。<br/>**起始版本：** 10         |
| AUDIOSTREAM_STATE_STOPPED = 3 | 停止状态。<br/>**起始版本：** 10         |
| AUDIOSTREAM_STATE_RELEASED = 4 | 释放状态。<br/>**起始版本：** 10         |
| AUDIOSTREAM_STATE_PAUSED = 5 | 暂停状态。<br/>**起始版本：** 10         |
...
定义音频流使用场景。通常用来描述音频输入流的使用场景。

**起始版本：** 10

| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_SOURCE_TYPE_INVALID = -1 | 不合法状态。<br/>**起始版本：** 10    |
| AUDIOSTREAM_SOURCE_TYPE_MIC = 0      | 录音。<br/>**起始版本：** 10       |
| AUDIOSTREAM_SOURCE_TYPE_VOICE_RECOGNITION = 1   | 语音识别。<br/>**起始版本：** 10     |
| AUDIOSTREAM_SOURCE_TYPE_PLAYBACK_CAPTURE = 2    | 播放录音。<br/>**起始版本：** 10     |
| AUDIOSTREAM_SOURCE_TYPE_VOICE_COMMUNICATION = 7 | 通话。<br/>**起始版本：** 10       |
...
| AUDIOSTREAM_SOURCE_TYPE_LIVE = 17    | 直播。<br/>**起始版本：** 10       |
...
| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_EVENT_ROUTING_CHANGED = 0 | 音频的路由已更改。<br/>**起始版本：** 10<br/>**废弃版本：** 18<br/>**替代接口：** OH_AudioRenderer_OutputDeviceChangeCallback. |
...
定义音频中断类型。当用户监听到音频中断时，将获取此信息。此类型表示本次音频打断的操作是否已由系统强制执行，具体操作信息（如音频暂停、停止等）可通过[OH_AudioInterrupt_Hint](capi-native-audiostream-base-h.md#oh_audiointerrupt_hint)获取。

**起始版本：** 10

| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_INTERRUPT_FORCE = 0 | 强制打断类型，即具体操作已由系统强制执行。<br/>**起始版本：** 10        |
| AUDIOSTREAM_INTERRUPT_SHAR = 1 | 共享打断类型，即系统不执行具体操作，通过[OH_AudioInterrupt_Hint](#oh_audiointerrupt_hint)提示并建议应用操作，应用可自行决策下一步处理方式。<br/>**起始版本：** 10 |
...
定义音频中断提示类型。当用户监听到音频中断时，将获取此信息。此类型表示根据焦点策略，当前需要对音频流的具体操作（如暂停、调整音量等）。可以结合[OH_AudioInterrupt_ForceType](capi-native-audiostream-base-h.md#oh_audiointerrupt_forcetype)信息，判断该操作是否已由系统强制执行。

**起始版本：** 10

| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_INTERRUPT_HINT_NONE = 0 | 不提示。<br/>**起始版本：** 10          |
| AUDIOSTREAM_INTERRUPT_HINT_RESUME = 1 | 提示音频恢复，应用可主动触发开始渲染或开始采集的相关操作。此操作无法由系统强制执行，其对应的[OH_AudioInterrupt_ForceType](#oh_audiointerrupt_forcetype)一定为[AUDIOSTREAM_INTERRUPT_SHAR](#oh_audiointerrupt_forcetype)类型。<br/>**起始版本：** 10 |
| AUDIOSTREAM_INTERRUPT_HINT_PAUSE = 2 | 提示音频暂停，暂时失去音频焦点。后续待焦点可用时，会出现AUDIOSTREAM_INTERRUPT_HINT_RESUME事件。<br/>**起始版本：** 10     |
| AUDIOSTREAM_INTERRUPT_HINT_STOP = 3 | 提示音频停止，彻底失去音频焦点。<br/>**起始版本：** 10         |
| AUDIOSTREAM_INTERRUPT_HINT_DUCK = 4 | 提示音频躲避开始，音频降低音量播放，而不会停止。<br/>**起始版本：** 10 |
| AUDIOSTREAM_INTERRUPT_HINT_UNDUCK = 5 | 提示音量躲避结束，音频恢复正常音量。<br/>**起始版本：** 10       |
...
定义音频中断模式。通常用来设置音频中断模式。

**起始版本：** 12

| 枚举项 | 描述 |
| -- | -- |
| AUDIOSTREAM_INTERRUPT_MODE_SHARE = 0 | 共享模式。<br/>**起始版本：** 12 |
| AUDIOSTREAM_INTERRUPT_MODE_INDEPENDENT = 1 | 独立模式。<br/>**起始版本：** 12 |
...
| 枚举项 | 描述 |
| -- | -- |
| EFFECT_NONE = 0 | 无音效模式。<br/>**起始版本：** 12 |
| EFFECT_DEFAULT = 1 | 默认音效模式。<br/>**起始版本：** 12 |
...
| 枚举项 | 描述 |
| -- | -- |
| AUDIO_STREAM_PRIVACY_TYPE_PUBLIC = 0 | 表示音频流可以被其他应用录制。<br/>**起始版本：** 12 |
| AUDIO_STREAM_PRIVACY_TYPE_PRIVATE = 1 | 表示音频流不可以被其他应用录制。<br/>**起始版本：** 12 |
...
这个函数指针将指向用于写入音频数据的回调函数。这个函数类似于 OH_AudioRenderer_Callbacks_Struct.OH_AudioRenderer_OnWriteData 函数指针。但具有返回值，用于标识音频数据回调结果。该函数的返回结果表示填充到缓冲区的数据是否有效。如果结果无效，用户填写的数据将不被播放。
...
| 类型 | 说明 |
| -- | -- |
| [OH_AudioData_Callback_Result](#oh_audiodata_callback_result) | 函数返回值。<br>         [AUDIO_DATA_CALLBACK_RESULT_INVALID](capi-native-audiostream-base-h.md#oh_audiodata_callback_result) 音频数据回调结果无效，且音频数据不播放。<br>         [AUDIO_DATA_CALLBACK_RESULT_VALID](capi-native-audiostream-base-h.md#oh_audiodata_callback_result) 音频数据回调结果有效，将播放音频数据。 |
