...
| [Image_String](capi-image-string.md) | Image_String | 声明字符串结构的名称。 |
...
| [Image_ErrorCode OH_PictureMetadata_GetPropertyWithNull(OH_PictureMetadata *metadata, Image_String *key, Image_String *value)](#oh_picturemetadata_getpropertywithnull) | 获取图片元数据的属性值。输出的 value.data 以字符串结束符结尾。 |
...
| static const char * MIME_TYPE_HEIC = "image/heic" | heif图片格式。<br>**起始版本：** 12 |
...
| static const char * OHOS_IMAGE_PROPERTY_SAMPLES_PER_PIXEL = "SamplesPerPixel" | 每个像素的分量数。由于该标准适用于 RGB 和 YCbCr 图像，因此该标签的值设置为 3。在JPEG压缩数据中，使用JPEG标记代替该标签。<br>**起始版本：** 12 |
...
| static const char * OHOS_IMAGE_PROPERTY_FACE_LEYE_CENTER = "HwMnoteFaceLeyeCenter" | 左眼中心。被用于 {@link OH_ImageSource_GetImageProperty} 和 {@link OH_ImageSource_ModifyImageProperty}。<br>**起始版本：** 12 |
...
| IMAGE_UNSUPPORTED_MIME_TYPE = 7600101 | 不支持的mime type。 |
| IMAGE_UNKNOWN_MIME_TYPE = 7600102 | 未知的mime type。 |
...
| IMAGE_UNSUPPORTED_METADATA = 7600202 | 不支持的 metadata。 |
...
| IMAGE_SOURCE_UNSUPPORTED_MIME_TYPE = 7700102 |  不支持的 MIME 类型。<br>**起始版本：** 15 |
| IMAGE_SOURCE_TOO_LARGE = 7700103 |  图像太大。<br>**起始版本：** 15 |
...
| 类型 | 说明 |
| -- | -- |
| [Image_ErrorCode](#image_errorcode) | 如果操作成功返回 IMAGE_SUCCESS，如果参数错误返回 IMAGE_BAD_PARAMETER，<br> 具体请参考 [Image_ErrorCode](capi-image-common-h.md#image_errorcode)。 |
...
| 类型 | 说明 |
| -- | -- |
| [Image_ErrorCode](#image_errorcode) | 如果操作成功返回 IMAGE_SUCCESS，如果参数错误返回 IMAGE_BAD_PARAMETER，<br> 如果是不支持的元数据类型或元数据类型与辅助图类型不匹配返回 IMAGE_UNSUPPORTED_METADATA，<br> 具体请参考 [Image_ErrorCode](capi-image-common-h.md#image_errorcode)。 |
...
获取图片元数据的属性值。输出的 value.data 以字符串结束符结尾。
...
| 类型 | 说明 |
| -- | -- |
| [Image_ErrorCode](#image_errorcode) | 返回接口结果[Image_ErrorCode](capi-image-common-h.md#image_errorcode)：<br>         IMAGE_SUCCESS：执行成功；<br>         IMAGE_INVALID_PARAMETER：metadata、key 或 value 为空；<br>         IMAGE_UNSUPPORTED_METADATA：不支持的元数据类型，或元数据类型与辅助图片类型不匹配。 |
...
| 类型 | 说明 |
| -- | -- |
| [Image_ErrorCode](#image_errorcode) | 如果操作成功返回 IMAGE_SUCCESS，如果参数错误返回 IMAGE_BAD_PARAMETER，<br> 如果内存分配失败返回 IMAGE_ALLOC_FAILED，如果内存拷贝失败返回 IMAGE_COPY_FAILED，<br> 具体请参考 [Image_ErrorCode](capi-image-common-h.md#image_errorcode)。 |
