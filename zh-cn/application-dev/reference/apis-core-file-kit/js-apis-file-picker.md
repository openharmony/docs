...
| 名称                    | 类型                                          | 只读 | 可选 | 说明                                       |
| :---------------------- |---------------------------------------------| ---- | ---- | ------------------------------------------|
| maxSelectNumber<sup>10+</sup>       | number                                      | 否   | 是 | 1. maxSelectNumber描述修改为：受系统能力限制，选择文件数量过大可能导致功能异常，建议个数不超过20万个。2. 当该参数置为非DEFAULT时，仅maxSelectNumber参数可设置生效，其他参数不生效。（选择目录仅对具有该系统能力的设备开放。且目录选择的最大个数为1）。<br>**系统能力**：SystemCapability.FileManagement.UserFileService  |
