...
调用本模块接口返回的URI数组，URI中的中文及非数字字母的特殊字符会被编码为对应的ASCII码，然后拼接到URI中。
...
| defaultFilePathUri<sup>10+</sup>    | string                                      | 否   |  是 | 指定选择的文件或者目录URI。默认为空（效果为拉起最近打开页）。           |
...
| defaultFilePathUri<sup>10+</sup>    | string  | 否   | 指定保存的文件或者目录URI。  |
