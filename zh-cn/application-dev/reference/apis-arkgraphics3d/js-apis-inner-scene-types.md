...
## 导入模块
```ts
import { Vec2, Vec3, Vec4, Quaternion, Aabb, Color, Rect, GeometryType<sup>18+</sup>, PrimitiveTopology<sup>18+</sup>, CustomGeometry<sup>18+</sup>, CubeGeometry<sup>18+</sup>, PlaneGeometry<sup>18+</sup>, SphereGeometry<sup>18+</sup>, Position3, Rotation3, Scale3 } from '@kit.ArkGraphics3D';
```
...
| 名称 | 值 | 说明 |
| ---- | ---- | ---- |
| CUSTOM<sup>18+</sup> | 0 | 未定义类型。 |
| CUBE<sup>18+</sup> | 1 | 立方体类型。 |
| PLANE<sup>18+</sup> | 2 | 平面类型。 |
| SPHERE<sup>18+</sup> | 3 | 球体类型。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| geometryType<sup>18+</sup> | [GeometryType](#geometrytype18)| 是 | 否 | 定义不同的几何类型。 |
...
| 名称 | 值 | 说明 |
| ---- | ---- | ---- |
| TRIANGLE_LIST<sup>18+</sup> | 0 | 由不相交的顶点集合构成不同的三角形。 |
| TRIANGLE_STRIP<sup>18+</sup> | 1 | 每个顶点和前一个三角形的一条边构成新的三角形。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| topology<sup>18+</sup> | [PrimitiveTopology](#primitivetopology18)| 否 | 是 | 三角形图元的解析方式，默认值为TRIANGLE_LIST。 |
| vertices<sup>18+</sup> | [Vec3](#vec3)[] | 否 | 否 | 模型的顶点数组。 |
| indices<sup>18+</sup> | number[] | 否 | 是 | 顶点索引数组，数组中元素的取值范围大于等于0，默认值为undefined。 |
| normals<sup>18+</sup> | [Vec3](#vec3)[] | 否 | 是 | 顶点数组对应的法向量数组，默认值为undefined。 |
| uvs<sup>18+</sup> | [Vec2](#vec2)[] | 否 | 是 | 顶点数组对应的UV坐标数组，默认值为undefined。 |
| colors<sup>18+</sup> | [Color](#color)[] | 否 | 是 | 顶点数组对应的颜色数组，默认值为undefined。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| size<sup>18+</sup> | [Vec3](#vec3) | 否 | 否 | 立方体的宽、高和深度，表示立方体的大小。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| size<sup>18+</sup> | [Vec2](#vec2) | 否 | 否 | 平面的宽、高，表示平面的大小。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| radius<sup>18+</sup> | number | 否 | 否 | 球体半径，取值范围大于0。 |
| segmentCount<sup>18+</sup> | number | 否 | 否 | 在球体上以经纬度分割的段数，取值范围大于0。 |
