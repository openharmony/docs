...
从一个类数组对象创建一个新的ArkTS数组。<font color="red">ArkTS数组就是上个接口说的ArkTS Array吗？是的话，确定一种说法，上下文全文使用统一一致的概念！不要一会写A，一会又写B！</font>
...
从ArkTS数组中移除并返回最后一个元素。如果数组为空，则返回undefined，且数组不发生变化。
...
从ArkTS数组中移除并返回第一个元素。如果数组为空，则返回undefined，且数组不发生变化。
...
在ArkTS数组的首端插入一个或多个元素，并返回新的数组长度。
...
```ts
// 此处将原始数组中的每个字符串元素转换为大写形式，并返回一个新数组，其中包含转换后的字符串
let array = new collections.Array<string>('a', 'b', 'c');
...
| 参数名    | 类型                                                   | 必填 | 说明                                                         |
| --------- | ------------------------------------------------------ | ---- | ------------------------------------------------------------ |
| predicate | (value: T, index: number, array: Array\<T>) => boolean | 是   | 一个接受三个参数的函数，用于测试每个元素是否应该包含在新数组中。 |
...
对数组中的每个元素执行回调函数，将其结果作为累加值，并返回最终的结果。
...
与 reduce方法类似，但它接受一个初始值作为第三个参数，用于在数组遍历开始前初始化累加器。
...
```ts
// 此处使用一个初始值为0的累加器，并将其与数组中的每个元素相加，最终返回累加后的总和
...
| 参数名 | 类型   | 必填 | 说明                                                         |
| ------ | ------ | ---- | ------------------------------------------------------------ |
| index  | number | 是   | 要返回的数组元素的索引（从零开始），取值为整数。负数索引从数组末尾开始计数——如果`index < 0`，则会访问`index + array.length`位置的元素。 |
