...
```js    
import socket from "@ohos.net.socket";let tcpServer: socket.TCPSocketServer = socket.constructTCPSocketServerInstance();  
tcpServer.on('connect', (client: socket.TCPSocketConnection) => {  let tcpSendOption: socket.TCPSendOptions = {    data: 'Hello, client!'  }  client.send(tcpSendOption, () => {    console.log('send success');  });});    
```    
...
```js    
import socket from "@ohos.net.socket";let tcpServer: socket.TCPSocketServer = socket.constructTCPSocketServerInstance();tcpServer.on('connect', (client: socket.TCPSocketConnection) => {  client.close().then(() => {  console.log('close success');  }).catch((err: BusinessError) => {  console.log('close fail');  });});    
```    
...
| 参数名 | 类型 | 必填 | 说明 |  
| --------| --------| --------| --------|  
| type | string | true | 订阅的事件类型。'close'：关闭事件。 |  
...
示例callback：  
```js    
import socket from "@ohos.net.socket";let tcpServer: socket.TCPSocketServer = socket.constructTCPSocketServerInstance();let callback = () => {  console.log("on close success");}tcpServer.on('connect', (client: socket.TCPSocketConnection) => {  client.on('close', callback);  // 可以指定传入on中的callback取消一个订阅，也可以不指定callback清空所有订阅。  client.off('close', callback);  client.off('close');});    
```    
 ...
 **系统能力:**  SystemCapability.Communication.NetStack    
### listen<sup>(10+)</sup>    
绑定IP地址和端口，端口可以指定或由系统随机分配。监听并接受与此套接字建立的TCPSocket连接。该接口使用多线程并发处理客户端的数据。  
 **调用形式：**     
...
| 参数名 | 类型 | 必填 | 说明 |  
| --------| --------| --------| --------|  
| address | NetAddress | true | 目标地址信息。 |  
...
| 参数名 | 类型 | 必填 | 说明 |  
| --------| --------| --------| --------|  
| options | TCPExtraOptions | true | TCPSocketServer连接的其他属性。 |  
...
| 参数名 | 类型 | 必填 | 说明 |  
| --------| --------| --------| --------|  
| type | string | true | 订阅的事件类型。'connect'：连接事件。 |  
...
```js    
import socket from "@ohos.net.socket";let tcpServer: socket.TCPSocketServer = socket.constructTCPSocketServerInstance();tcpServer.on('connect', (data: socket.TCPSocketConnection) => {  console.log(JSON.stringify(data))});    
```    
...
示例callback：  
```js    
import socket from "@ohos.net.socket";let tcpServer: socket.TCPSocketServer = socket.constructTCPSocketServerInstance();let callback = (data: socket.TCPSocketConnection) => {  console.log('on connect message: ' + JSON.stringify(data));}tcpServer.on('connect', callback);// 可以指定传入on中的callback取消一个订阅，也可以不指定callback清空所有订阅。tcpServer.off('connect', callback);tcpServer.off('connect');```    
```    
...
| 名称 | 类型 | 只读 | 必填 | 说明 |  
| --------| --------| --------| --------| --------|  
| clientId<sup>(10+)</sup> | number | false | true | 客户端与TLSSocketServer建立连接的id。 |  
    
### send<sup>(10+)</sup>    
在TLSSocketServer通信连接成功之后，向客户端发送消息  
 **调用形式：**     
...
| 参数名 | 类型 | 必填 | 说明 |  
| --------| --------| --------| --------|  
| data | string | true | TLSSocketServer发送数据所需要的参数。    |  
    
 **回调或返回值：**     
| 类型 | 说明 |  
| --------| --------|  
| callback | 回调函数，成功返回空，失败返回错误码，错误信息。 |  
...
### close<sup>(10+)</sup>    
在与TLSSocketServer通信连接成功之后，断开连接  
 **调用形式：**     
...
### getRemoteAddress<sup>(10+)</sup>    
在TLSSocketServer通信连接成功之后，获取对端Socket地址。  
 **调用形式：**     
...
在TLSSocketServer通信连接成功之后，获取对端的数字证书，该接口只适用于客户端向服务端发送证书时  
 **调用形式：**     
...
| 类型 | 说明 |  
| --------| --------|  
| callback | 回调函数，返回对端的证书。失败返回错误码，错误信息。 |  
...
在TLSSocketServer通信连接成功之后，获取通信双方协商后的加密套件  
 **调用形式：**     
...
在TLSSocketServer通信连接成功之后，获取通信双方协商后签名算法  
 **调用形式：**     
...
| 参数名 | 类型 | 必填 | 说明 |  
| --------| --------| --------| --------|  
| type | string | true | 取消订阅TLSSocketConnection连接的error事件。使用callback方式作为异步方法。 |  
 ...
 **系统能力:**  SystemCapability.Communication.NetStack    
### listen<sup>(10+)</sup>    
绑定IP地址和端口，在TLSSocketServer上bind成功之后，监听客户端的连接，创建和初始化TLS会话，实现建立连接过程，加载证书秘钥并验证  
 **调用形式：**     
...
| 参数名 | 类型 | 必填 | 说明 |  
| --------| --------| --------| --------|  
| options | TLSConnectOptions | true | TLSSocketServer连接所需要的参数。 |  
...
| 2303503 | Error in tls writing |  
| 2303505 | Error occurred in the tls system call. |  
| 2303506 | Error occurred in the tls system call. |  
...
### getState<sup>(10+)</sup>    
在TLSSocketServer的listen成功之后，获取TLSSocketServer状态。  
 **调用形式：**     
...
| 类型 | 说明 |  
| --------| --------|  
| callback | 回调函数。成功返回TLSSocketServer状态，失败返回错误码，错误信息。 |  
...
### setExtraOptions<sup>(10+)</sup>    
在TLSSocketServer的listen成功之后，设置TLSSocketServer连接的其他属性。  
 **调用形式：**     
...
| 参数名 | 类型 | 必填 | 说明 |  
| --------| --------| --------| --------|  
| options | TCPExtraOptions | true | TLSSocketServer连接的其他属性。  |  
...
在TLSSocketServer通信连接成功之后，获取本地的数字证书  
 **调用形式：**     
...
在TLSSocketServer通信连接成功之后，获取通信的协议版本  
 **调用形式：**     
