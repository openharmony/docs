...
| ASSET_TAG_WRAP_TYPE = ASSET_TYPE_BYTES \| 0x49 | 表示关键资产支持的加密导入导出类型，其值为bytes类型。<br>**起始版本：** 16 |
