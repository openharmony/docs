...
The tag type of map should be number.

**错误描述**

map参数的Tag类型不正确，需要传入number类型。
...
1. 请检查传入的Tag参数，确保所有Tag参数在已定义的[Tag枚举](js-apis-asset.md#tag)范围内。
2. 如已完成上述检查，则说明当前系统版本不支持该能力，请使用替代方式。例如，当系统不支持asset.Tag.REQUIRE_ATTR_ENCRYPTED时，业务可自行对属性加密后，将属性密文存储在Asset中。
