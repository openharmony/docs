...
对于剪贴板中内容记录的抽象定义，称之为条目。剪贴板内容部分由一个或者多个条目构成，例如一条文本内容、一份HTML、一个URI或者一个Want。不支持在创建PasteDataRecord之后，修改PasteDataRecord的默认数据类型的值，应在创建PasteDataRecord时制定正确的默认数据类型的值。
...
| want | [Want](../apis-ability-kit/js-apis-app-ability-want.md) | 否 | 否 | Want内容。要进行赋值，请使用[createData](#pasteboardcreatedata9)或[addentry](#addentry14) |
| mimeType | string | 否 | 否 | 默认数据类型。要进行赋值，请使用[createData](#pasteboardcreatedata9)或[addentry](#addentry14) |
| plainText | string | 否 | 否 | 纯文本内容。要进行赋值，请使用[createData](#pasteboardcreatedata9)或[addentry](#addentry14) |
| uri | string | 否 | 否 | URI内容，需符合标准URI格式。要进行赋值，请使用[createData](#pasteboardcreatedata9)或[addentry](#addentry14) |
| pixelMap<sup>9+</sup> | [image.PixelMap](../apis-image-kit/arkts-apis-image-PixelMap.md) | 否 | 否 | PixelMap内容。要进行赋值，请使用[createData](#pasteboardcreatedata9)或[addentry](#addentry14) |
| data<sup>9+</sup> | Record<string, ArrayBuffer> | 否 | 否 | 自定义数据内容。要进行赋值，请使用[createData](#pasteboardcreatedata9)或[addentry](#addentry14) |
