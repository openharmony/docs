...
| 名称 | 类型 | 可读 | 可写 | 说明 | 示例 |
| -------- | -------- | -------- | -------- | -------- | -------- |
| deviceType | string | 是 | 否 | 设备类型。详细请参考[deviceTypes标签](../../quick-start/module-configuration-file.md#devicetypes标签)。<br/>**原子化服务API**：从API version 11开始，该接口支持在原子化服务中使用。 | phone |
| manufacture | string | 是 | 否 | 设备厂家名称。 | HUAWEI |
| brand | string | 是 | 否 | 设备品牌名称。<br/>**原子化服务API**：从API version 11开始，该接口支持在原子化服务中使用。 | HUAWEI |
| marketName | string | 是 | 否 | 外部产品系列。 | Mate XX |
| productSeries | string | 是 | 否 | 产品系列。 | TAS |
| productModel | string | 是 | 否 | 认证型号。<br/>**原子化服务API**：从API version 11开始，该接口支持在原子化服务中使用。 | TAS-AL00 |
| softwareModel | string | 是 | 否 | 内部软件子型号。 | TAS-AL00 |
| hardwareModel | string | 是 | 否 | 硬件版本号。 | TASA00CVN1 |
| hardwareProfile<sup>(deprecated) </sup> | string | 是 | 否 | 硬件Profile。<br/>**说明**：<br/>从API version 6 开始支持，从API version 9 开始废弃。 | NA |
| serial | string | 是 | 否 | 设备序列号,仅限系统应用使用。<br/>**说明**：可作为设备唯一识别码。<br/>**需要权限**：ohos.permission.sec.ACCESS_UDID | 序列号随设备差异 |
| bootloaderVersion | string | 是 | 否 | Bootloader版本号。 | bootloater |
| abiList | string | 是 | 否 | 应用二进制接口（Abi）。 | arm64-v8a |
| securityPatchTag | string | 是 | 否 | 安全补丁级别。 | 2021-01-01 |
| displayVersion | string | 是 | 否 | 产品版本。 | XXX X.X.X.X |
| incrementalVersion | string | 是 | 否 | 差异版本号。 | NA |
| osReleaseType | string | 是 | 否 | 系统的发布类型，取值为：<br/>-&nbsp;Canary：面向特定开发者发布的早期预览版本，不承诺API稳定性。<br/>-&nbsp;Beta：面向开发者公开发布的Beta版本，不承诺API稳定性。<br/>-&nbsp;Release：面向开发者公开发布的正式版本，承诺API稳定性。 | Canary\\Beta\\Release |
| osFullName | string | 是 | 否 | 系统版本，版本格式OpenHarmony-x.x.x.x,x为数值。<br/>**原子化服务API**：从API version 11开始，该接口支持在原子化服务中使用。 | Openharmony-5.0.0.1 |
| majorVersion | number | 是 | 否 | Major版本号，随主版本更新增加，值为osFullName中的第一位数值，建议直接使用deviceInfo.majorVersion获取，不建议开发者解析osFullName获取。 | 5 |
| seniorVersion | number | 是 | 否 | Senior版本号，随局部架构、重大特性增加，值为osFullName中的第二位数值，建议直接使用deviceInfo.seniorVersion获取，不建议开发者自主解析osFullName获取。 | 0 |
| featureVersion | number | 是 | 否 | Feature版本号，标识规划的新特性版本，值为osFullName中的第三位数值，建议直接使用deviceInfo.featureVersion获取，不建议开发者自主解析osFullName获取。 | 0 |
| buildVersion | number | 是 | 否 | Build版本号，标识编译构建的版本号，值为osFullName中的第四位数值，建议直接使用deviceInfo.buildVersion获取，不建议开发者自主解析osFullName获取。 | 1 |
| sdkApiVersion | number | 是 | 否 | 系统软件API版本。 | 12 |
| firstApiVersion | number | 是 | 否 | 首个版本系统软件API版本。 | 3 |
| versionId | string | 是 | 否 | 版本ID。由deviceType、manufacture、brand、productSeries、osFullName、productModel、softwareModel、sdkApiVersion、incrementalVersion、buildType拼接组成 | NA |
| buildType | string | 是 | 否 | 构建类型。 | release:nolog |
| buildUser | string | 是 | 否 | 构建用户。 | NA |
| buildHost | string | 是 | 否 | 构建主机。 | NA |
| buildTime | string | 是 | 否 | 构建时间。 | NA |
| buildRootHash | string | 是 | 否 | 构建版本Hash。 | NA |
| udid<sup>7+</sup> | string | 是 | 否 | 设备Udid,仅限系统应用使用。<br/>**说明**：数据长度为65字节。可作为设备唯一识别码。<br/>**需要权限**：ohos.permission.sec.ACCESS_UDID | NA |
| distributionOSName<sup>10+</sup> | String | 是 | 否 | 发行版系统名称<!--Del-->，由发行方定义<!--DelEnd-->。 | NA |
| distributionOSVersion<sup>10+</sup> | String | 是 | 否 | 发行版系统版本号<!--Del-->，由发行方定义<!--DelEnd-->。<!--RP1--><!--RP1End--> | NA |
| distributionOSApiVersion<sup>10+</sup> | number| 是 | 否 | 发行版系统api版本<!--Del-->，由发行方定义<!--DelEnd-->。 | NA |
| distributionOSReleaseType<sup>10+</sup> | String | 是 | 否 | 发行版系统类型<!--Del-->，由发行方定义<!--DelEnd-->。 | NA |
| ODID<sup>12+</sup> | String | 是 | 否 |开发者匿名设备标识符。<br/>**ODID值会在以下场景重新生成**：<br/>手机恢复出厂设置。<br/>同一设备上同一个开发者的应用全部卸载后重新安装时。<br/>**ODID生成规则**：<br/>同一设备上运行的同一个开发者的应用，ODID相同。<br/>同一个设备上不同开发者的应用，ODID不同。<br/>不同设备上同一个开发者的应用，ODID不同。<br/>不同设备上不同开发者的应用，ODID不同。<br/>**说明**：数据长度为37字节。 | NA |
