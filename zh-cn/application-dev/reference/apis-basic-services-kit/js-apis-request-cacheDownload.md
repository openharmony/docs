...
启动缓存下载任务，传输成功后，将数据下载到内存缓存和文件缓存。
...
- 使用该接口调整缓存大小时，默认以“LRU”（最近最少使用）方式清除多余缓存。
...
- 使用该接口调整缓存大小时，默认采用“LRU”（最近最少使用）方式清除多余缓存。
...
| 参数名   | 类型     | 必填 | 说明                                                         |
|-------|--------|----|------------------------------------------------------------|
| bytes | number | 是 | 设置缓存上限。默认值为104857600字节（100MB），最大值为4294967296字节（4GB）。 |
