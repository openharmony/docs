...
| 参数名 | 类型 | 必填 | 说明 |
| -------- | ------------------------- | -- | --------------------------------------------- |
| x        | number                    | 是 | 窗口在x轴方向移动的坐标位置，单位为px，值为正，表示最终位置在x轴右侧；值为负，表示最终位置在x轴左侧；值为0，表示最终位置在x轴坐标原点。该参数仅支持整数输入，浮点数输入将向下取整。 |
| y        | number                    | 是 | 窗口在y轴方向移动的坐标位置，单位为px，值为正，表示最终位置在y轴下侧；值为负，表示最终位置在y轴上侧；值为0，表示最终位置在y轴坐标原点。该参数仅支持整数输入，浮点数输入将向下取整。 |
| callback | AsyncCallback&lt;void&gt; | 是 | 回调函数。                                     |
...
仅在[onTouch](./arkui-ts/ts-universal-events-touch.md#touchevent)事件（其中，事件类型必须为TouchType.Down）的回调方法中调用此接口才会有移动效果，成功调用此接口后，窗口将跟随鼠标移动。窗口在不获焦状态无法移动。
...
| -------- | ------------------------- | ---- | ------------------------------------------------- |
| x        | number                    | 是   | 窗口在x轴方向移动的坐标位置，单位为px，值为正，表示最终位置在x轴右侧；值为负，表示最终位置在x轴左侧；值为0，表示最终位置在x轴坐标原点。该参数仅支持整数输入，浮点数输入将向下取整。 |
| y        | number                    | 是   | 窗口在y轴方向移动的坐标位置，单位为px，值为正，表示最终位置在y轴下侧；值为负，表示最终位置在y轴上侧；值为0，表示最终位置在x轴坐标原点。该参数仅支持整数输入，浮点数输入将向下取整。 |
| callback | AsyncCallback&lt;void&gt; | 是   | 回调函数。                                        |
