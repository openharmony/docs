...
setWindowTopmost(isWinddowTopmost: boolean): Promise<void>
...
| 参数名    | 类型    | 必填 | 说明                                          |
| --------- | ------- | ---- | --------------------------------------------- |
| isWinddowTopmost | boolean | 是   | 设置主窗口置顶，true为置顶，false为取消置顶。 |
  ...
  onWindowStageCreate(windowStage: window.WindowStage): void {
    ...
    windowStage.loadContent('pages/Index', (err) => {
      ...
      windowStage.getMainWindow().then(
        data => {
          mainWindow = data;
          let isWinddowTopmost: boolean = true;
          let promise = window.setWindowTopmost(isWinddowTopmost);
          promise.then(() => {
