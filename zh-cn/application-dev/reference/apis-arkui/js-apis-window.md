...
基于窗口与鼠标坐标的偏移量移动窗口，先移动窗口使当前鼠标位置相对移动后窗口位置的偏移量相同，再开始移动窗口，使用Promise异步回调。
...
| 参数名     | 类型       | 必填     | 说明                                                 |
|---------| --------- | -------- |----------------------------------------------------|
| offsetX | number | 是 | 窗口移动时预期鼠标位置相对窗口左上角的x轴偏移量，单位为px，该参数仅支持整数输入，浮点数向下取整。 |
| offsetY | number | 是 | 窗口移动时预期鼠标位置相对窗口左上角的y轴偏移量，单位为px，该参数仅支持整数输入，浮点数向下取整。 |
