...
| grayOutMaximizeButton | boolean | 是 | 仅在supportedWindowModes不支持FULL_SCREEN时生效，通过grayOutMaximizeButton选项是否将最大化按钮置灰。true表示将最大化按钮置灰不可用，false表示最大化按钮不可见，默认值为false。 |
