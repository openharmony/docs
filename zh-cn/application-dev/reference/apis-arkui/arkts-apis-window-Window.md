...
该接口仅在自由窗口模式下生效，用于创建主窗口、子窗口或悬浮窗下的子窗口，使用Promise异步回调。
...
**设备行为差异：** 当使用该窗口创建模应用子窗口时（即[SubWindowOptions](./arkts-apis-window-i.md#subwindowoptions11)中的[ModalityType](./arkts-apis-window-e.md#modalitytype14)为APPLICATION_MODALITY），该接口在2in1设备、tablet设备中可正常调用，在其他设备类型中返回801错误码。当使用该接口创建其他类型创建其他类型子窗口时，该接口在所有设备中可正常调用。
