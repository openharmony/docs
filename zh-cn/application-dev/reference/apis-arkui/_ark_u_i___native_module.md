...
| [ArkUI_AnimationDirection](#arkui_animationdirection) { ARKUI_ANIMATION_DIRECTION_NORMAL = 0, ARKUI_ANIMATION_DIRECTION_ALTERNATE, ARKUI_ANIMATION_DIRECTION_REVERSE, ARKUI_ANIMATION_DIRECTION_ALTERNATE_REVERSE } | 动画播放模式。  | 
...
| void [OH_ArkUI_ArkUI_AnimatorOption_Dispose](#oh_arkui_arkui_animatoroption_dispose) ([ArkUI_AnimatorOption](#arkui_animatoroption) \*option) | 销毁animator动画对象参数。  | 
...
| int32_t [OH_ArkUI_Animator_RegisterOnFrameCallback](#oh_arkui_animator_registeronframecallback) ([ArkUI_AnimatorHandle](#arkui_animatorhandle) animator, void \*userData, void(\*callback)(ArkUI_AnimatorOnFrameEvent \*event)) | 设置animator动画接收到帧时回调。  | 
| int32_t [OH_ArkUI_Animator_RegisterOnFinishCallback](#oh_arkui_animator_registeronfinishcallback) ([ArkUI_AnimatorHandle](#arkui_animatorhandle) animator, void \*userData, void(\*callback)(ArkUI_AnimatorEvent \*event)) | 设置animator动画完成时回调。  | 
| int32_t [OH_ArkUI_Animator_RegisterOnCancelCallback](#oh_arkui_animator_registeroncancelcallback) ([ArkUI_AnimatorHandle](#arkui_animatorhandle) animator, void \*userData, void(\*callback)(ArkUI_AnimatorEvent \*event)) | 设置animator动画被取消时回调。  | 
| int32_t [OH_ArkUI_Animator_RegisterOnRepeatCallback](#oh_arkui_animator_registeronrepeatcallback) ([ArkUI_AnimatorHandle](#arkui_animatorhandle) animator, void \*userData, void(\*callback)(ArkUI_AnimatorEvent \*event)) | 设置animator动画重复时回调。  | 
...
| [ArkUI_CurveHandle](#arkui_curvehandle) [OH_ArkUI_Curve_InitCurve](#oh_arkui_curve_initcurve) ([ArkUI_AnimationCurve](#arkui_animationcurve) curve) | 插值曲线的初始化函数，可以根据入参创建一个插值曲线对象。  | 
| [ArkUI_CurveHandle](#arkui_curvehandle) [OH_ArkUI_Curve_StepsCurve](#oh_arkui_curve_stepscurve) (int32_t count, bool end) | 构造阶梯曲线对象。  | 
| [ArkUI_CurveHandle](#arkui_curvehandle) [OH_ArkUI_Curve_CubicBezierCurve](#oh_arkui_curve_cubicbeziercurve) (float x1, float y1, float x2, float y2) | 构造三阶贝塞尔曲线对象。  | 
| [ArkUI_CurveHandle](#arkui_curvehandle) [OH_ArkUI_Curve_SpringCurve](#oh_arkui_curve_springcurve) (float velocity, float mass, float stiffness, float damping) | 构造弹簧曲线对象，曲线形状由弹簧参数决定，动画时长受animation、animateTo中的duration参数控制。  | 
| [ArkUI_CurveHandle](#arkui_curvehandle) [OH_ArkUI_Curve_SpringMotion](#oh_arkui_curve_springmotion) (float response, float dampingFraction, float overlapDuration) | 构造弹性动画曲线对象。如果对同一对象的同一属性进行多个弹性动画，每个动画会替换掉前一个动画，并继承之前的速度。  | 
| [ArkUI_CurveHandle](#arkui_curvehandle) [OH_ArkUI_Curve_ResponsiveSpringMotion](#oh_arkui_curve_responsivespringmotion) (float response, float dampingFraction, float overlapDuration) | 构造弹性跟手动画曲线对象，是springMotion的一种特例，仅默认参数不同，可与springMotion混合使用。  | 
| [ArkUI_CurveHandle](#arkui_curvehandle) [OH_ArkUI_Curve_InterpolatingSpring](#oh_arkui_curve_interpolatingspring) (float velocity, float mass, float stiffness, float damping) | 构造插值器弹簧曲线对象，生成一条从0到1的动画曲线，实际动画值根据曲线进行插值计算。  | 
| [ArkUI_CurveHandle](#arkui_curvehandle) [OH_ArkUI_Curve_CustomCurve](#oh_arkui_curve_customcurve) (void \*userData, float(\*interpolate)(float fraction, void \*userdata)) | 构造自定义曲线对象。  | 
...
| ARKUI_ANIMATION_DIRECTION_ALTERNATE  | 动画反向循环播放。  | 
| ARKUI_ANIMATION_DIRECTION_REVERSE  | 动画交替循环播放，奇数次正向播放，偶数次反向播放。  | 
...
| NODE_OVERLAY  | 遮罩文本属性，支持属性设置，属性重置和属性获取。<br/>属性设置方法[ArkUI_AttributeItem](_ark_u_i___attribute_item.md)参数格式：<br/>.string 遮罩文本；<br/>.value[0]?.i32：可选值，浮层相对于组件的位置，参数类型[ArkUI_Alignment](#arkui_alignment)， 默认值为ARKUI_ALIGNMENT_CENTER。<br/>.value[1]?.f32：可选值，浮层基于自身左上角的偏移量X，单位为vp。<br/>.value[2]?.f32：可选值，浮层基于自身左上角的偏移量Y，单位为vp。<br/>属性获取方法返回值[ArkUI_AttributeItem](_ark_u_i___attribute_item.md)格式：<br/>.string 遮罩文本；<br/>.value[0].i32：浮层相对于组件的位置，参数类型[ArkUI_Alignment](#arkui_alignment)， 默认值为ARKUI_ALIGNMENT_CENTER。<br/>.value[1].f32：浮层基于自身左上角的偏移量X，单位为vp。<br/>.value[2].f32：浮层基于自身左上角的偏移量Y，单位为vp。 | 
...
### OH_ArkUI_Animator_RegisterOnCancelCallback()

```
int32_t OH_ArkUI_Animator_RegisterOnCancelCallback (ArkUI_AnimatorHandle animator, void * userData, void(*)(ArkUI_AnimatorEvent *event) callback )
```
**描述：**
...
### OH_ArkUI_Animator_RegisterOnFinishCallback()

```
int32_t OH_ArkUI_Animator_RegisterOnFinishCallback (ArkUI_AnimatorHandle animator, void * userData, void(*)(ArkUI_AnimatorEvent *event) callback )
```
**描述：**
...
### OH_ArkUI_Animator_RegisterOnFrameCallback()

```
int32_t OH_ArkUI_Animator_RegisterOnFrameCallback (ArkUI_AnimatorHandle animator, void * userData, void(*)(ArkUI_AnimatorOnFrameEvent *event) callback )
```
**描述：**
...
### OH_ArkUI_Animator_RegisterOnRepeatCallback()

```
int32_t OH_ArkUI_Animator_RegisterOnRepeatCallback (ArkUI_AnimatorHandle animator, void * userData, void(*)(ArkUI_AnimatorEvent *event) callback )
```
**描述：**
...
### OH_ArkUI_ArkUI_AnimatorOption_Dispose()
...
### OH_ArkUI_Curve_CubicBezierCurve()
...
### OH_ArkUI_Curve_CustomCurve()

```
ArkUI_CurveHandle OH_ArkUI_Curve_CustomCurve (void * userData, float(*)(float fraction, void *userdata) interpolate )
```
**描述：**
...
### OH_ArkUI_Curve_InterpolatingSpring()
...
### OH_ArkUI_Curve_ResponsiveSpringMotion()
