...
| 名称   | 值 | 说明                    |
| ------ | --- | ------------------------ |
| FULLSCREEN | 1 | 全屏窗口，即当前台存在全屏窗口时，第一个标记位被设置为1。  |
| SPLIT  | 1 << 1 | 分屏窗口，即当前台存在分屏窗口时，第二个标记位被设置为1。 |
| FLOAT   | 1 << 2  | 悬浮窗，即当前台存在悬浮窗口时，第三个标记位被设置为1。 |
| PIP   | 1 << 3  | 画中画，即当前台存在画中画窗口时，第四个标记位被设置为1。 |
