...
| preventDefault<sup>12+</sup>      | () => void | 阻止默认事件。<br/> **说明：**&nbsp;该接口仅支持部分组件使用，当前支持组件：RichEditor、Checkbox、CheckboxGroup、Rating、Radio、Toggle、Hyperlink。暂不支持异步调用。<br/>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。|
...
### 示例2
该示例实现了Checkbox组件ClickEvent支持preventDefault，如果阻止，有按压态效果但无法选中。
```ts
@Entry
@Component
