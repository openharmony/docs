...
| composeItemPadding<sup>12+</sup> | [LocalizedPadding](ts-types.md#localizedpadding12) | 否 | \@Prop | 通用外边距，取值范围大于等于0vp。<br/>默认值：<br/>{ start: LengthMetrics.vp(12), end: LengthMetrics.vp(12) }。|
| itemSpace<sup>12+</sup> | number | 否 | \@Prop | 通用内边距，取值范围大于等于0，默认值：12，单位为vp，其作用于左侧元素的图标和文字间距12vp；左右两个元素间距12vp。 |
...
| primaryText | [ResourceStr](ts-types.md#resourcestr) | 否 | 中间元素的标题内容，超出一行后换行显示。 |
| secondaryText | [ResourceStr](ts-types.md#resourcestr) | 否 | 中间元素的副标题内容，超出一行后换行显示。 |
| description | [ResourceStr](ts-types.md#resourcestr) | 否 | 中间元素的描述内容，超出一行后换行显示。 |
...
```ts
// ComposeListItem基础功能演示demo，包含左右元素的情况
...
### 示例二：
```ts
// ComposeListItem在基础功能基础上，增加图片、文字自定义功能的Demo
import { IconType, ComposeListItem } from '@kit.ArkUI';
import promptAction from '@ohos.promptAction';
