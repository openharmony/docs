...
| placement<sup>16+</sup> | [Placement](ts-appendix-enums.md#placement8) | 否 | 设置半模态popup样式弹窗相对于目标的显示位置。<br />**默认值：** Placement.Bottom。<br />**说明：** <br /> 1. popup样式弹窗在保证指定位置能够放下弹窗大小的前提下，优先按照设置的placement进行弹出；否则，会根据先尝试垂直翻转，再尝试90°水平方向显示的调整规则，调整显示位置，以预设方向为下方举例，调整顺序为：<br /> `下->上->右->左`<br />2. 如果设置了对应的对其方式导致按照对其方式布局会超出窗口范围，则会按对应对齐方式，做水平或垂直方向的位移，直至组件完整显示在窗口内。<br />3. 如果最终四个方向上都无法容纳下当前popup样式弹窗，则根据开发者设置的`placementOnTarget`属性：<br /> 1）若为true，则根据设置的`placement`，向镜像方向平移，直至能够放下弹窗<br /> 2）若为false，则在四个方向上，寻找能够容纳下当前弹窗宽度，且剩余高度最大的方向，压缩半模态高度直至当前方向能够放下半模态弹窗，对齐方式保留预设`placement`对应的对齐方式 <br />**原子化服务API：** 从API version 16开始，该接口支持在原子化服务中使用。 |
| placementOnTarget<sup>16+</sup> | Boolean | 否 | 设置半模态popup样式弹窗在当前窗口下，四个方向均无法放下半模态弹窗大小时，是否允许覆盖在目标节点上。<br /> **默认值：** true <br />**原子化服务API：** 从API version 16开始，该接口支持在原子化服务中使用。|
## SheetSize枚举说明
