...
| lineHeight<sup>12+</sup>    | number \| string \| Resource | 否     |设置文本的文本行高，设置该值为百分比字符串时，按默认值显示。设置值不大于0时，不限制文本行高，自适应字体大小，number类型时单位为fp。 | 
| letterSpacing<sup>12+</sup> | number \| string             | 否     | 设置文本字符间距，设置该值为百分比字符串时，按默认值显示。当取值为负值时，文字会发生压缩，负值过小时会将组件内容区大小压缩为0，导致无内容显示。|
