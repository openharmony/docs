...
| enableFoldedArea<sup>11+</sup>      | boolean                                                      | 否   | 菜单组件是否响应悬停态变化。如果菜单的点击位置在悬停态折痕区域，菜单组件不会响应悬停态。 <br/>默认值：true, 菜单组件响应悬停态变化。<br/>**说明：**<br />enableFoldedArea为true、未设置或者值为非法值时，菜单组件会响应悬停态，enableArrow为false时，不响应悬停态。bindContextMenu从API version 13开始支持该属性；bindMenu从API version 13开始支持该属性。<br />**原子化服务API：** 从API version 13开始，该接口支持在原子化服务中使用。 |
