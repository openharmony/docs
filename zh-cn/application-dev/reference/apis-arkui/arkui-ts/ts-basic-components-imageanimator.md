...
| width    | number&nbsp;\|&nbsp;string | 否  | 图片宽度，string类型仅支持number类型取值的字符串形式，例如"1"。<br/>默认值：0<br/>单位：vp   <br/>**卡片能力：** 从API version 10开始，该接口支持在ArkTS卡片中使用       |
| height   | number&nbsp;\|&nbsp;string | 否  | 图片高度，string类型仅支持number类型取值的字符串形式，例如"1"。<br/>默认值：0<br/>单位：vp     <br/>**卡片能力：** 从API version 10开始，该接口支持在ArkTS卡片中使用        |
| top      | number&nbsp;\|&nbsp;string | 否  | 图片相对于组件左上角的纵向坐标，string类型仅支持number类型取值的字符串形式，例如"1"。<br/>默认值：0<br/>单位：vp  <br/>**卡片能力：** 从API version 10开始，该接口支持在ArkTS卡片中使用  |
| left     | number&nbsp;\|&nbsp;string | 否  | 图片相对于组件左上角的横向坐标，string类型仅支持number类型取值的字符串形式，例如"1"。<br/>默认值：0<br/>单位：vp <br/>**卡片能力：** 从API version 10开始，该接口支持在ArkTS卡片中使用   |
