...
本文档仅为API参数说明。组件描述和使用说明见[Repeat开发者指南](../../../quick-start/arkts-new-rendering-control-repeat.md)。
...
| disableVirtualScroll | boolean | 否 | 使用全量加载模式渲染数组。 |
...
- 显式指定cachedCount，推荐设置成和屏幕上节点个数一致。不建议设置cachedCount小于2，因为这会导致在快速滑动场景下创建新的节点，从而造成性能劣化。
