...
| 名称          | 类型   | 只读 | 可选 | 说明                      |
| --------------- | ------ | ---- | ---- | ------------------------- |
| angle  | number \| string | 否   | 是   |  线性渐变的起始角度。以12点钟方向为0度，顺时针旋转为正向角度。<br/>默认值：180<br/>角度为字符串时，支持数值加单位的组合格式，单位仅支持'deg'、'grad'、'rad'、'turn'，例如'90deg'、'180grad'、'3.14rad'、'0.25turn'。<br/>**说明：**<br/>设置direction后该属性不生效。 |
...
通过[borderImage](#borderimage)接口中的slice、width和outset属性值使用[LocalizedEdgeWidths](ts-types.md#localizededgewidths12)类型。
