...
| 参数名 | 类型                                        | 必填 | 说明                                                       |
| ------ | ------------------------------------------- | ---- | ---------------------------------------------------------- |
| value  | number |  是 | 下拉偏移量，单位vp。<br/>默认值：64vp <br/>如果取值为0或负数的时候此接口采用默认值64vp。<br/>如果取值为超小数或超大数按照设置的数值进行对应的刷新动作。<br/>设置超小值只要拉动到超小值的位置就会触发刷新状态。<br/>设置超大值拉动到超大值的位置就会触发刷新状态。|
