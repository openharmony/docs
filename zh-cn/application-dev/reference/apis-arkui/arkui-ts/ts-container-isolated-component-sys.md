...
IsolatedComponent用于支持在本页面内嵌入显示独立Abc提供的UI，展示的内容在受限worker线程中运行。
...
2、Abc需要先[VerifyAbc](../../apis-ability-kit/js-apis-bundleManager.md#bundlemanagerverifyabc11)校验通过之后才可以使用于当前组件。
...
## IsolatedOptions
用于在IsolatedComponent进行构造的时传递可选的构造参数。
...
| 通用属性 |
|----------------|
|[width](ts-universal-attributes-size.md#width)|
|[height](ts-universal-attributes-size.md#height)|
