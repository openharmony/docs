...
| 名称 | 类型  | 必填 |说明         |
| ------ | ------ | ------- |------------|
| value  | number | 是 | 当前进度值。设置小于0的数值时置为0，设置大于total的数值时置为total。<br/>默认值：0<br/>取值范围：[0, total] |
| total  | number | 是 | 进度总长。<br/>取值范围：大于0   |
...
| scaleCount   | number                       | 否   | 设置环形进度条总刻度数。<br/>默认值：120 <br/>取值范围：[2, min(width, height)/scaleWidth/2/π]                     |
