...
| void(\* [registerNodeCustomEventReceiver](#registernodecustomeventreceiver) )(void(\*eventReceiver)(ArkUI_NodeCustomEvent \*event)) | 注册自定义节点事件回调统一入口函数。  | 
...
| int32_t(\* [measureNode](#measurenode) )([ArkUI_NodeHandle](_ark_u_i___native_module.md#arkui_nodehandle) node, ArkUI_LayoutConstraint \*Constraint) | 对特定组件进行测算，可以通过getMeasuredSize接口获取测算后的大小。  | 
