...
| 类型               | 说明                    |
| ---------------- |-----------------------|
| 'retrieve'         | 表示具有检索窗口内容的能力。          |
| 'touchGuide'       | 表示具有触摸探索模式的能力。          |
| 'keyEventObserver' | 表示具有过滤按键事件的能力。          |
| 'zoom'             | 表示具有控制显示放大的能力。当前版本暂不支持。 |
| 'gesture'          | 表示具有执行手势动作的能力。          |
...
| ------ | ------------------ |
| 'add'    | 表示添加窗口的窗口变化事件。       |
| 'remove' | 表示一个窗口被删除的窗口变化事件。    |
| 'bounds' | 表示窗口边界已更改的窗口变化事件。    |
| 'active' | 表示窗口变为活动或不活动的窗口变化事件。 |
| 'focus'  | 表示窗口焦点发生变化的窗口变化事件。   |
