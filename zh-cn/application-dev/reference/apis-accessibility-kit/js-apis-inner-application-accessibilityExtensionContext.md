...
| componentType        | string                                                             | 否   | 否   | 应与元素所属的组件类型所对应，如：按钮Button类型->'Button'、图片Image类型->'Image'。 |
| contents             | Array&lt;string&gt;                                                | 否   | 否   | 内容列表，根据实际场景设定，无特殊限制。 |
| currentIndex         | number                                                             | 否   | 否   | 当前项的索引。默认值为0。|
| description          | string                                                             | 否   | 否   | 元素的描述信息，根据实际场景设定，无特殊限制。 |
