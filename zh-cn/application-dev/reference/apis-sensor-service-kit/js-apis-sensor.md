...
| 名称          | 类型     | 只读  | 可选  | 说明             |
|--------------|----------|-------|------|----------------- |
| deviceId    | number    | 否    | 是    | 设备ID：默认值为-1，表示本地设备，设备Id需通过[getSensorList](#sensorgetsensorlist9)或者监听设备上下线接口获取。<br/>**原子化服务API**：从API Version 19开始，该接口支持在原子化服务中使用。      |
| sensorIndex | number    | 否    | 是    | 传感器索引：默认值为0，为设备上的默认传感器，其它传感器Id需通过[getSensorListByDeviceSync](#sensorgetsensorlistbydevicesync19)查询。<br/>**原子化服务API**：从API Version 19开始，该接口支持在原子化服务中使用。 |
...
| 名称      | 类型   | 只读 | 可选 | 说明                     |
| --------- | ------ | ---- | ---- | ------------------------ |
| timestamp | number | 否   | 是   | 传感器数据上报的时间戳。从设备开机开始计时到上报数据的时间，单位 : ns。 |
...
| 名称 | 类型   | 只读 | 可选 | 说明                                                       |
| ---- | ------ | ---- | ---- | ---------------------------------------------------------- |
| x    | number | 否   | 是   | 施加在设备x轴的加速度，单位 : m/s²；取值为实际上报物理量。 |
| y    | number | 否   | 是   | 施加在设备y轴的加速度，单位 : m/s²；取值为实际上报物理量。 |
| z    | number | 否   | 是   | 施加在设备z轴的加速度，单位 : m/s²；取值为实际上报物理量。 |
...
| 名称 | 类型   | 只读 | 可选 | 说明                                     |
| ---- | ------ | ---- | ---- | ---------------------------------------- |
| x    | number | 否   | 是   | 施加在设备x轴的线性加速度，单位 : m/s²。 |
| y    | number | 否   | 是   | 施加在设备y轴的线性加速度，单位 : m/s²。 |
| z    | number | 否   | 是   | 施加在设备z轴的线性加速度，单位 : m/s²。 |
...
| 名称  | 类型   | 只读 | 可选 | 说明                                           |
| ----- | ------ | ---- | ---- | ---------------------------------------------- |
| x     | number | 否   | 是   | 施加在设备x轴未校准的加速度，单位 : m/s²。     |
| y     | number | 否   | 是   | 施加在设备y轴未校准的加速度，单位 : m/s²。     |
| z     | number | 否   | 是   | 施加在设备z轴未校准的加速度，单位 : m/s²。     |
| biasX | number | 否   | 是   | 施加在设备x轴未校准的加速度偏量，单位 : m/s²。 |
| biasY | number | 否   | 是   | 施加在设备y轴未校准的加速度偏量，单位 : m/s²。 |
| biasZ | number | 否   | 是   | 施加在设备z轴未校准的加速度偏量，单位 : m/s²。 |
...
| 名称 | 类型   | 只读 | 可选 | 说明                                     |
| ---- | ------ | ---- | ---- | ---------------------------------------- |
| x    | number | 否   | 是   | 施加在设备x轴的重力加速度，单位 : m/s²。 |
| y    | number | 否   | 是   | 施加在设备y轴的重力加速度，单位 : m/s²。 |
| z    | number | 否   | 是   | 施加在设备z轴的重力加速度，单位 : m/s²。 |
...
| 名称  | 类型   | 只读 | 可选 | 说明                                                  |
| ----- | ------ | ---- | ---- | ----------------------------------------------------- |
| alpha | number | 否   | 是   | 设备围绕Z轴的旋转角度，单位：度；取值范围为0-360度。  |
| beta  | number | 否   | 是   | 设备围绕X轴的旋转角度，单位：度；取值范围为0-±180度。 |
| gamma | number | 否   | 是   | 设备围绕Y轴的旋转角度，单位：度；取值范围为0-±90度。  |
...
| 名称 | 类型   | 只读 | 可选 | 说明              |
| ---- | ------ | ---- | ---- | ----------------- |
| x    | number | 否   | 是   | 旋转矢量x轴分量。 |
| y    | number | 否   | 是   | 旋转矢量y轴分量。 |
| z    | number | 否   | 是   | 旋转矢量z轴分量。 |
| w    | number | 否   | 是   | 标量，描述设备相对于某个参考方向的旋转状态，单位：弧度。            |
...
| 名称 | 类型   | 只读 | 可选 | 说明                                                   |
| ---- | ------ | ---- | ---- | ------------------------------------------------------ |
| x    | number | 否   | 是   | 设备x轴的旋转角速度，单位rad/s；取值为实际上报物理量。 |
| y    | number | 否   | 是   | 设备y轴的旋转角速度，单位rad/s；取值为实际上报物理量。 |
| z    | number | 否   | 是   | 设备z轴的旋转角速度，单位rad/s；取值为实际上报物理量。 |
...
| 名称  | 类型   | 只读 | 可选 | 说明                                       |
| ----- | ------ | ---- | ---- | ------------------------------------------ |
| x     | number | 否   | 是   | 设备x轴未校准的旋转角速度，单位rad/s。     |
| y     | number | 否   | 是   | 设备y轴未校准的旋转角速度，单位rad/s。     |
| z     | number | 否   | 是   | 设备z轴未校准的旋转角速度，单位rad/s。     |
| biasX | number | 否   | 是   | 设备x轴未校准的旋转角速度偏量，单位rad/s。 |
| biasY | number | 否   | 是   | 设备y轴未校准的旋转角速度偏量，单位rad/s。 |
| biasZ | number | 否   | 是   | 设备z轴未校准的旋转角速度偏量，单位rad/s。 |
...
| 名称   | 类型   | 只读 | 可选 | 说明                                                         |
| ------ | ------ | ---- | ---- | ------------------------------------------------------------ |
| scalar | number | 否   | 是   | 表示剧烈运动程度。测量三个物理轴（x、y&nbsp;和&nbsp;z）上，设备是否存在大幅度运动；若存在大幅度运动则数据上报为1。 |
...
| 名称     | 类型   | 只读 | 可选 | 说明                                                       |
| -------- | ------ | ---- | ---- | ---------------------------------------------------------- |
| distance | number | 否   | 是   | 可见物体与设备显示器的接近程度。0表示接近，大于0表示远离。 |
...
| 名称                            | 类型   | 只读 | 可选 | 说明                                                         |
| ------------------------------- | ------ | ---- | ---- | ------------------------------------------------------------ |
| intensity                       | number | 否   | 是   | 光强（单位：勒克斯）。                                       |
...
| 名称   | 类型   | 只读 | 可选 | 说明                                                         |
| ------ | ------ | ---- | ---- | ------------------------------------------------------------ |
| status | number | 否   | 是   | 显示霍尔状态。测量设备周围是否存在磁力吸引，0表示没有，大于0表示有。 |
...
| 名称 | 类型   | 只读 | 可选 | 说明                         |
| ---- | ------ | ---- | ---- | ---------------------------- |
| x    | number | 否   | 是   | x轴环境磁场强度，单位 : μT。 |
| y    | number | 否   | 是   | y轴环境磁场强度，单位 : μT。 |
| z    | number | 否   | 是   | z轴环境磁场强度，单位 : μT。 |
...
| 名称  | 类型   | 只读 | 可选 | 说明                                   |
| ----- | ------ | ---- | ---- | -------------------------------------- |
| x     | number | 否   | 是   | x轴未校准环境磁场强度，单位 : μT。     |
| y     | number | 否   | 是   | y轴未校准环境磁场强度，单位 : μT。     |
| z     | number | 否   | 是   | z轴未校准环境磁场强度，单位 : μT。     |
| biasX | number | 否   | 是   | x轴未校准环境磁场强度偏量，单位 : μT。 |
| biasY | number | 否   | 是   | y轴未校准环境磁场强度偏量，单位 : μT。 |
| biasZ | number | 否   | 是   | z轴未校准环境磁场强度偏量，单位 : μT。 |
...
| 名称     | 类型   | 只读 | 可选 | 说明                                                      |
| -------- | ------ | ---- | ---- | --------------------------------------------------------- |
| humidity | number | 否   | 是   | 湿度值。测量环境的相对湿度，以百分比&nbsp;(%)&nbsp;表示。 |
...
| 名称   | 类型   | 只读 | 可选 | 说明                                                         |
| ------ | ------ | ---- | ---- | ------------------------------------------------------------ |
| scalar | number | 否   | 是   | 计步器检测。检测用户的计步动作，如果取值为1则代表用户产生了计步行走的动作，取值为0则代表用户没有发生运动。 |
...
| 名称        | 类型   | 只读 | 可选 | 说明                       |
| ----------- | ------ | ---- | ---- | -------------------------- |
| temperature | number | 否   | 是   | 环境温度（单位：摄氏度）。 |
...
| 名称      | 类型   | 只读 | 可选 | 说明                                    |
| --------- | ------ | ---- | ---- | --------------------------------------- |
| heartRate | number | 否   | 是   | 心率值。测量用户的心率数值，单位：bpm。 |
...
| 名称  | 类型   | 只读 | 可选 | 说明                                             |
| ----- | ------ | ---- | ---- | ------------------------------------------------ |
| value | number | 否   | 是   | 表示设备是否被穿戴（1表示已穿戴，0表示未穿戴）。 |
...
| 名称        | 类型                | 只读 | 可选 | 说明       |
| ----------- | ------------------- | ---- | ---- | ---------- |
| rotation    | Array&lt;number&gt; | 否   | 是   | 旋转矩阵。 |
| inclination | Array&lt;number&gt; | 否   | 是   | 倾斜矩阵。 |
...
| 名称            | 类型   | 只读 | 可选 | 说明                                               |
| --------------- | ------ | ---- | ---- | -------------------------------------------------- |
| x               | number | 否   | 是   | 地磁场的北分量。                                   |
| y               | number | 否   | 是   | 地磁场的东分量。                                   |
| z               | number | 否   | 是   | 地磁场的垂直分量。                                 |
| geomagneticDip  | number | 否   | 是   | 地磁倾角，即地球磁场线与水平面的夹角。             |
| deflectionAngle | number | 否   | 是   | 地磁偏角，即地磁北方向与正北方向在水平面上的角度。 |
| levelIntensity  | number | 否   | 是   | 地磁场的水平强度。                                 |
| totalIntensity  | number | 否   | 是   | 地磁场的总强度。                                   |
...
| 名称      | 类型   | 只读 | 可选 | 说明       |
| --------- | ------ | ---- | ---- | ---------- |
| latitude  | number | 否   | 是   | 纬度。     |
| longitude | number | 否   | 是   | 经度。     |
| altitude  | number | 否   | 是   | 海拔高度。 |
