...
| 名称 | 描述 |
| -------- | -------- |
| [dataType](#datatype) | 指定张量的数据类型，要求从[OH_NN_DataType](_neural_nework_runtime.md#oh_nn_datatype)枚举类型中取值。 |
...
| [type](#type) | 指定张量的类型。type的取值和张量的用途相关，当张量用作模型的输入或输出，则要求将type设置为[OH_NN_TENSOR](_neural_nework_runtime.md)； 当张量用作算子参数，则要求从[OH_NN_TensorType](_neural_nework_runtime.md#oh_nn_tensortype)中选择除[OH_NN_TENSOR](_neural_nework_runtime.md)以外的枚举值。 |
