...
可以包含单个子组件。
<br/>建议使用文本组件构建当前数值文本和辅助文本。
<br/>若子组件宽高为百分比形式，则基准范围为以外圆环为内切圆的矩形。
...
| 名称          | 参数类型 | 必填 | 描述 |
| ------------- | ------- | ---- | -------- |
| icon | [Resource](ts-types.md#resource类型) | 否 | 图标资源路径。<br/>**说明：** <br/>不配置则使用默认的三角形样式指针。<br/>建议开发者使用svg格式的图标，若使用其他格式，则有概率出现实际效果与期待效果不符的现象。|
