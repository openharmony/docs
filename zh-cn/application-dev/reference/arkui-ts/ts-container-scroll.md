...
| scrollSnap     | [ScrollSnapOptions](#scrollsnapoptions结构图定义)                     | 设置Scroll组件的限位滚动模式。 |
...
| ---------- | ------------------------ |
| NONE       | 关闭限位模式              |
| START      | 相对于Scroll组件的视口，每页内容将在左侧（水平）或者顶部（垂直）对其限位。 |
| CENTER     | 相对于Scroll组件的视口，每页内容将在中心对齐限位。 |
| END        | 相对于Scroll组件的视口，每页内容将在右侧（水平）或者底部（垂直）对其限位。 |
