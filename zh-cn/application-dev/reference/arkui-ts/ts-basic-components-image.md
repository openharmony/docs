...
| draggable<sup>9+</sup>   | boolean                                  | 设置默认拖拽效果。（不能和[onDragStart](ts-universal-events-drag-drop.md)事件同时使用。）<br/>默认值：false<br/>该接口支持在ArkTS卡片中使用。 <br />**说明：**<br />从 API version 10 开始废弃，建议使用[draggable](ts-universal-events-drag-drop.md)替代。 |
