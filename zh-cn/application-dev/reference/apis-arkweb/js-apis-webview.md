...
| size | SizeOptions | 是 | web绘制的尺寸，最多支持16000px * 16000px, 长度单位支持px、vp、%，需保持不同参数传入长度单位一致, 默认单位vp，超过规格时返回最大规格。例如：（width:'100px', height:'200px'; 或者 width:'20%', height'30%';只写number时单位为vp。）|
