...
设置ArkWeb已经使用过的空闲socket的超时时间，即已经使用过的socket可以处于空闲状态的最大时长。若此时设置的值与已存在的空闲socket超时时间不同，则根据设置的值对已存在的空闲socket进行一次清理。
...
| 参数名   | 类型    | 必填 | 说明                                                     |
| -------- | ------- | ---- | -------------------------------------------------------- |
| timeout | int | 是   | ArkWeb中已经使用过的空闲socket超时时间，范围30~300，单位s，<br>小于30时生效值为30，大于300时生效值为300。 |
