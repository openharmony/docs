...
| imagePixelMap | [image.PixelMap](../apis-image-kit/js-apis-image.md#pixelmap7) | 否 | 是 | 全量绘制结果image.pixelMap格式。|

## OfflineResourceMap<sup>12+</sup>

本地离线资源配置对象，用于配置将被[injectOfflineResources](./js-apis-webview-WebviewController.md#injectofflineresources12)接口注入到内存缓存的本地离线资源的相关信息，内核会根据此信息生成资源缓存，并据此控制缓存的有效期。
...
| type | [OfflineResourceType](./js-apis-webview-e.md#offlineresourcetype12) | 否   | 否   | 资源的类型，目前仅支持Javascript、图片和CSS类型的资源。      |
...
提供点击区域的元素信息。示例代码参考[getLastHitTest](./js-apis-webview-WebviewController.md#getlasthittest18)。
