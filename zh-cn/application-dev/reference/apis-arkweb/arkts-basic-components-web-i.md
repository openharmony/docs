...
| 名称           | 类型                                             | 只读    | 可选    | 必填    | 说明             |
| ---------- | -----------------------------------------------------| ------ | ------ | ------ | ---------------- |
| content   | [ResourceStr](../apis-arkui/arkui-ts/ts-types.md#resourcestr)  | 否     | 否     | 是     | 显示内容。     |
| startIcon | [ResourceStr](../apis-arkui/arkui-ts/ts-types.md#resourcestr)  | 否     | 是     | 否     | 显示图标。     |
| action    | (selectedText: {plainText: string}) => void                    | 否     | 否     | 是     | 选中的文本信息。|
...
| 名称           | 类型                                             | 只读    | 可选    | 必填    | 说明             |
| ---------- | -----------------------------------------------------| ------ | ------ | ------ | ---------------- |
| onAppear   | Callback\<void\>   | 否     | 是     | 否     | 自定义选择菜单弹出时回调。     |
| onDisappear | Callback\<void\>  | 否     | 是     | 否     | 自定义选择菜单关闭时回调。     |
| preview    | [CustomBuilder](../apis-arkui/arkui-ts/ts-types.md#custombuilder8)          | 否     | 是     | 否     | 自定义选择菜单的预览内容样式，未配置时无预览内容。|
| menuType   | [MenuType](../apis-arkui/arkui-ts/ts-text-common.md#menutype13枚举说明)      | 否     | 是     | 否     | 自定义选择菜单类型。<br>默认值：`MenuType.SELECTION_MENU`。<br> 从API version 20起，`MenuType.PREVIEW_MENU`支持超链接预览。     |
| previewMenuOptions<sup>20+</sup> | [PreviewMenuOptions](#previewmenuoptions20) | 否     | 是     | 否     | 自定义选择预览菜单选项。 |
...
| 名称             | 类型      | 只读   | 可选   | 必填   | 说明                                       |
| -------------- | ---- | ---- | ---- | ---- | ---------------------------------------- |
| param  | [WebContextMenuParam](./arkts-basic-components-web-WebContextMenuParam.md) | 否 | 否 | 是 | 菜单相关参数。     |
| result | [WebContextMenuResult](./arkts-basic-components-web-WebContextMenuResult.md) | 否 | 否 | 是 | 菜单相应事件传入内核。 |
...
| 名称             | 类型                                  | 必填   | 说明                    |
| -----------     | ------------------------------------ | ---- | --------------------- |
| status     | [NativeEmbedStatus](./arkts-basic-components-web-e.md#nativeembedstatus11)             | 否    | 同层标签生命周期状态。 |
| surfaceId  | string                              | 否    | NativeImage的psurfaceid。  |
| embedId | string                              | 否    | 同层标签的唯一id。  |
| info  | [NativeEmbedInfo](./arkts-basic-components-web-i.md#nativeembedinfo11)  | 否    | 同层标签的详细信息。       |
