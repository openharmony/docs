...
设置应用中所有Web使用的代理配置，与[insertBypassRule](./js-apis-webview-ProxyConfig.md#insertbypassrule15)中插入的bypass规则匹配的URL将不会使用代理，而是直接向URL指定的源地址发起请求。代理设置成功后，不保证网络连接后会立即使用新的代理设置，在加载页面之前请等待监听器触发，这个监听器将在UI线程上被调用。
...
| 参数名          | 类型     |  必填  | 说明           |
| ---------------| ------- | ---- | ------------- |
| proxyConfig     | [ProxyConfig](./js-apis-webview-ProxyConfig.md#class-proxyconfig)  | 是   | 对代理的配置。 |
| callback     | [OnProxyConfigChangeCallback](./js-apis-webview-t.md#onproxyconfigchangecallback15)   | 是   | 代理设置成功的回调。 |
