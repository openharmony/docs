...
> **说明：**
>
> [onLoadIntercept](./ts-basic-components-web-events.md#onloadintercept10)在加载开始的时候触发，该时刻还未生成历史节点，所以在onLoadIntercept中调用getBackForwardEntries拿到的历史栈不包括当前正在加载中的跳转。
...
对Web内核赋予自定义协议url的跨域请求与fetch请求的权限。当Web在跨域fetch自定义协议url时，该fetch请求可被[onInterceptRequest](./ts-basic-components-web-events.md#oninterceptrequest9)事件接口所拦截，从而开发者可以进一步处理该请求。建议在任何Web组件初始化之前调用该接口。
...
| 参数名   | 类型    | 必填 | 说明                      |
| -------- | ------- | ---- | -------------------------------------- |
| schemes | Array\<[WebCustomScheme](./js-apis-webview-i.md#webcustomscheme)\> | 是   | 自定义协议配置，最多支持同时配置10个自定义协议。 |
...
| 参数名             | 类型                             |  必填  | 说明                                                              |
| ------------------| ------------------------------- | ---- | ------------------------------------------------------------------ |
| request           | [RequestInfo](./js-apis-webview-i.md#requestinfo12)   | 是   | 预获取请求的信息。                                                      |
| additionalHeaders | Array\<[WebHeader](./js-apis-webview-i.md#webheader)> | 否   | 预获取请求的附加HTTP请求头。                                             |
...
| 参数名          | 类型    |  必填  | 说明                                            |
| ---------------| ------- | ---- | ------------- |
| delegate      | [WebDownloadDelegate](./js-apis-webview.md#webdownloaddelegate11)  | 是   | 用来接收下载进回调的委托。 |
...
查询当前网页是否开启广告过滤功能。
当Web组件使能广告过滤功能后，默认所有页面都是开启广告过滤的，支持通过[addAdsBlockDisallowedList](./js-apis-webview.md#addadsblockdisallowedlist12)指定域名禁用广告过滤。
...
| 参数名       | 类型           | 必填  | 说明                      |
| ----------- | ------------- | ---- | ------------------------ |
| mode        | [RenderProcessMode](./js-apis-webview-e.md#renderprocessmode12)| 是   | 渲染子进程模式。<br>可以先调用[getRenderProcessMode()](#getrenderprocessmode12)查看当前设备的ArkWeb渲染子进程模式，枚举值0为单子进程模式，枚举值1为多子进程模式。<br>手机默认为单渲染子进程模式，平板和PC/2in1默认为多渲染子进程模式。<br>如果传入RenderProcessMode枚举值之外的非法数字，则默认识别为多渲染子进程模式。 |
...
| 类型                                      | 说明                                                         |
| ----------------------------------------- | ------------------------------------------------------------ |
| [RenderProcessMode](./js-apis-webview-e.md#renderprocessmode12) | 渲染子进程模式类型。<br>调用getRenderProcessMode()获取当前设备的ArkWeb渲染子进程模式，枚举值0为单子进程模式，枚举值1为多子进程模式。<br>如果获取的值不在RenderProcessMode枚举值范围内，则默认为多渲染子进程模式。 |
...
| 类型                                | 说明                        |
| ----------------------------------- | --------------------------- |
| [SecurityLevel](./js-apis-webview-e.md#securitylevel11) | 当前网页的安全级别，具体值为NONE、SECURE、WARNING、DANGEROUS。 |
...
| type       | [ScrollType](./js-apis-webview-e.md#scrolltype12) |  否 | 网页可触发的滚动类型，支持缺省配置。<br/> - enable为false时，表示禁止ScrollType类型的滚动，当ScrollType缺省时表示禁止所有类型网页滚动。<br/> - enable为true时，ScrollType缺省与否，都表示允许所有类型的网页滚动。|
...
通过[registerJavaScriptProxy](#registerjavascriptproxy)或者[javaScriptProxy](./ts-basic-components-web-attributes.md#javascriptproxy)注入JavaScript对象到window对象中。该接口可以获取最后一次调用注入的对象的frame的url。
...
| 类型                                        | 说明                                                      |
| ------------------------------------------- | --------------------------------------------------------- |
| [MediaPlaybackState](./js-apis-webview-e.md#mediaplaybackstate12) | 当前网页的播控状态，具体值为NONE、PLAYING、PAUSED、STOPPED。 |
...
为当前Web组件设置[WebSchemeHandler](./js-apis-webview.md#webschemehandler12), [WebSchemeHandler](./js-apis-webview.md#webschemehandler12)类用于拦截指定scheme的请求。
...
| handler    | [WebSchemeHandler](./js-apis-webview.md#webschemehandler12) | 是   | 拦截此协议的拦截器。 |
...
开启当前网页摄像头捕获。开启当前网页摄像头捕获。开启当前网页摄像头捕获。使用摄像头功能前请在module.json5中添加权限: ohos.permission.CAMERA，具体权限的添加方法请参考[在配置文件中声明权限](../../security/AccessToken/declare-permissions.md#在配置文件中声明权限)。
...
| cacheOptions | [CacheOptions](./js-apis-webview-i.md#cacheoptions12) | 是   | 用于控制字节码缓存更新。      |
...
注册回调函数，开启[应用接管网页媒体播放功能](./ts-basic-components-web-attributes.md#enablenativemediaplayer12)后，当网页中有播放媒体时，触发注册的回调函数。
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| callback | [CreateNativeMediaPlayerCallback](./js-apis-webview.md#createnativemediaplayercallback12) | 是 | 接管网页媒体播放的回调函数。 |
...
| 参数名       | 类型           | 必填  | 说明                      |
| ----------- | ------------- | ---- | ------------------------ |
| info        | [SnapshotInfo](./js-apis-webview-i.md#snapshotinfo12)| 是   | 全量绘制结果入参。 |
| callback        | AsyncCallback\<[SnapshotResult](./js-apis-webview-i.md#snapshotresult12)>| 是   | 全量绘制回调结果。 |
...
injectOfflineResources(resourceMaps: Array\<[OfflineResourceMap](./js-apis-webview-i.md#offlineresourcemap12)\>): void
...
| 参数名  | 类型    | 必填 | 说明                  |
| ------- | ------ | ---- | :-------------------- |
| resourceMaps | Array\<[OfflineResourceMap](./js-apis-webview-i.md#offlineresourcemap12)\> | 是   | 本地离线资源配置对象，单次调用最大支持注入30个资源，单个资源最大支持10Mb。      |
...
设置一个路径列表，当file协议访问该路径列表中的资源时，允许跨域访问本地文件。此外，当设置了路径列表时，file协议仅允许访问路径列表中的资源（[fileAccess](./ts-basic-components-web-attributes.md#fileaccess)的行为将会被此接口行为覆盖）。路径列表中的路径必须满足以下路径格式之一：
...
当路径列表中有其中一个路径不满足以上条件之一，则会抛出异常码401，并且设置路径列表失败。当设置的路径列表为空，则file协议可访问范围以[fileAccess](./ts-basic-components-web-attributes.md#fileaccess)的行为为准。
...
| 参数名          | 类型    |  必填  | 说明                                            |
| ---------------| ------- | ---- | ------------- |
| features     |  [BackForwardCacheSupportedFeatures](./js-apis-webview.md#backforwardcachesupportedfeatures12) | 是   | 允许使用特定的页面进入前进后退缓存中。|
...
| 参数名          | 类型    |  必填  | 说明                                            |
| ---------------| ------- | ---- | ------------- |
| options     |  [BackForwardCacheOptions](./js-apis-webview.md#backforwardcacheoptions12) | 是   | 用来控制Web组件前进后退缓存相关选项。|
...
| 参数名  | 类型    | 必填 | 说明                  |
| ------- | ------ | ---- | :-------------------- |
| level | [PressureLevel](./js-apis-webview-e.md#pressurelevel14) | 是 | 需要清理内存的内存等级。|
...
| 参数名        | 类型                                    | 必填 | 说明                    |
| ------------- | --------------------------------------- | ---- | ----------------------- |
| configuration | [PdfConfiguration](./js-apis-webview-i.md#pdfconfiguration14) | 是   | 生成PDF所需参数。       |
| callback      | AsyncCallback<[PdfData](./js-apis-webview.md#pdfdata14)>    | 是   | 回调返回网页PDF数据流。 |
...
| 类型                           | 说明                          |
| ------------------------------ | ----------------------------- |
| Promise<[PdfData](./js-apis-webview.md#pdfdata14)> | Promise实例，返回网页数据流。 |
...
| 类型         | 说明                 |
| ------------ | -------------------- |
| [HitTestValue](./js-apis-webview-i.md#hittestvalue) | 点击区域的元素信息。 |
...
| 类型                                                         | 说明                   |
| ------------------------------------------------------------ | ---------------------- |
| [WebHitTestType](./js-apis-webview-e.md#webhittesttype)| 被点击区域的元素类型。 |
