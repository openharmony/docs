...
| 参数名 | 类型 | 必填 | 说明 |
| -------- | -------- | -------- | -------- |
| elementName | [ElementName](js-apis-bundle-ElementName.md) | 是 | 被拉起应用的ElementName信息。 |
...
| -------- | -------- | -------- | -------- |
| elementName | [ElementName](js-apis-bundle-ElementName.md) | 是 | 被拉起应用的ElementName信息。 |
| message | string | 是 | 拉起应用失败时的信息。该信息采用JSON格式，样式如下：<br>{<br>    "errMsg": "xxx"<br>}<br>其中，"xxx"的取值说明如下：<br>Failed to call startAbility：表示调用startAbility接口出错。<br>User refused redirection：表示用户关闭了应用跳转弹框。<br>User closed the implicit startup picker：表示用户关闭了隐式启动时的应用选择弹框。<br>User closed the app clone picker：表示用户关闭了分身应用选择弹框。<br>Free installation failed：表示免安装失败。</br> |
