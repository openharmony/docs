...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| timeoutMs | number | 否 | 是 | 执行所有组件初始化的超时时间，默认值为10000毫秒。 |
| startupListener | [StartupListener](./js-apis-app-appstartup-startupListener.md) | 否 | 是 | 表示启动框架的监听器，该监听器将在所有组件初始化完成时调用。 |
