...
| fds<sup>15+</sup> | Record\<string, number> | 否 | 表示Want文件描述符，用于标识被打开的Want文件，可通过[fs.open](../apis-core-file-kit/js-apis-file-fs.md#fsopen)获取。当业务不再使用该文件描述符时，需要使用[fs.close](../apis-core-file-kit/js-apis-file-fs.md#fsclose)及时销毁，防止fd泄露。 |
