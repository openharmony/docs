...
| windowLeft<sup>11+</sup> | number | 否 | 是 | 窗口左边的位置，单位为vp。如果设置为负值，系统会默认设置为0。当配置了windowLeft时，建议同时配置windowTop，并设置为正值。 |
| windowTop<sup>11+</sup> | number | 否 | 是 | 窗口顶部的位置，单位为vp。如果设置为负值，系统会默认设置为0。当配置了windowTop时，建议同时配置windowLeft，并设置为正值。 |
| windowWidth<sup>11+</sup> | number | 否 | 是 | 窗口的宽度，单位为vp。最小值320vp，最大值1920vp。 |
| windowHeight<sup>11+</sup> | number | 否 | 是 | 窗口的高度，单位为vp。最小值72vp，最大值1920vp。 |
