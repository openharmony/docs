...
| windowLeft<sup>11+</sup> | number | 否 | 是 | 窗口左边的位置，单位为px。如果设置为负值，系统会默认设置为0。当配置了windowLeft时，建议同时配置windowTop，并设置为正值，且不超过maxWindowWidth。 |
| windowTop<sup>11+</sup> | number | 否 | 是 | 窗口顶部的位置，单位为px。如果设置为负值，系统会默认设置为0。当配置了windowTop时，建议同时配置windowLeft，并设置为正值，且不超过maxWindowHeight。 |
...
| minWindowWidth<sup>17+</sup> | number | 否 | 是 | 窗口最小的宽度，单位为px。<br>**约束：**<br/>1.仅在2in1和tablet设备上生效。<br/>2.可以通过[getWindowLimits](../apis-arkui/js-apis-window.md#getwindowlimits11)获得当前的窗口尺寸限制。 |
| minWindowHeight<sup>17+</sup> | number | 否 | 是 | 窗口最小的高度，单位为px。<br/>**约束：**<br/>1.仅在2in1和tablet设备上生效。<br/>2.可以通过[getWindowLimits](../apis-arkui/js-apis-window.md#getwindowlimits11)获得当前的窗口尺寸限制。 |
| maxWindowWidth<sup>17+</sup> | number | 否 | 是 | 窗口最大的宽度，单位为px。<br/>**约束：**<br/>1.仅在2in1和tablet设备上生效。<br/>2.可以通过[getWindowLimits](../apis-arkui/js-apis-window.md#getwindowlimits11)获得当前的窗口尺寸限制。 |
| maxWindowHeight<sup>17+</sup> | number | 否 | 是 | 窗口最大的高度，单位为px。<br/>**约束：**<br/>1.仅在2in1和tablet设备上生效。<br/>2.可以通过[getWindowLimits](../apis-arkui/js-apis-window.md#getwindowlimits11)获得当前的窗口尺寸限制。 |
