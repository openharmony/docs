...
| LAUNCH_REASON_MESSAGE<sup>18+</sup>  | ohos.params.launchReasonMessage  | 调用方拉起目标方Ability时，通过该字段设置拉起原因。调用方必须是系统应用，原因类型必须为ReasonMessage_SystemShare类型，目前只支持ReasonMessage_SystemShare类型，且需要申请ohos.permission.SET_LAUNCH_REASON_MESSAGE权限。 <br>**原子化服务API**：从API version 18开始，该接口支持在原子化服务中使用。 |
