...
| 名称 | 描述 |
| -------- | -------- |
| **JSVM_CDECL** | 用于指定函数的C调用约定，主要应用于回调函数指针的声明，以确保跨平台下回调函数使用统一的C调用约定。<br>当预处理指令_WIN32检查到为Windows平台时：JSVM_CDECL被赋值为"__cdecl"，表示使用C调用约定。当预处理指令_WIN32检查到不是Windows平台时：无须标记，JSVM_CDECL为空。 |
