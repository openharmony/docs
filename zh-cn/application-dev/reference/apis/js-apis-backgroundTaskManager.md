...
当应用或业务模块处于后台（无可见界面）时，如果有需要继续执行或者后续执行的业务，可基于业务类型，申请短时任务延迟挂起（Suspend）或者长时任务避免进入挂起状态。应用有不可中断且短时间能完成的任务时（如文件管理器点击垃圾文件清理，用户退后台，文件清理器需要再申请一小段时间把清理的动作做完），可以使用短时任务机制；应用有用户能够直观感受到的且需要一直在后台运行的业务时（如应用需要在后台播放音乐），可以使用长时任务机制。
