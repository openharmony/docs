...
| 名称 | 参数类型 | 可读 | 可写 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| length | number | 是 | 否 | 获取Vector&nbsp;的元素个数 |
...
根据下标截取Vector中的一段元素,并返回这一段vector实例,包前不包后
- 参数：
  | 参数名 | 类型 | 必填 | 说明 |
  | -------- | -------- | -------- | -------- |
  | fromIndex | number | 是 | 起始下标 |
...
从指定索引向后搜索, 返回该元素的下标索引,如果未找到该元素,则返回 -1,如果没有此类索引,则返回 -1
...
从指定索引向前搜索, 返回该元素的下标索引,如果未找到该元素,则返回 -1,如果没有此类索引,则返回 -1
