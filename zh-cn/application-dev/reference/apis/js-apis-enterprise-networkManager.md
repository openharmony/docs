...
| 类型   | 说明                              |
|------|---------------------------------|
| void | 当方法调用成功时不会返回任何信息，当方法调用错误时会抛出异常。 |
...
// 清空所有规则
let cleanRule: networkManager.FirewallRule = {}
networkManager.removeFirewallRule(wantTemp, cleanRule);
...
// 清空所有规则
let cleanRule: networkManager.DomainFilterRule = {}
networkManager.removeDomainFilterRule(wantTemp, cleanRule);
...
| direction | [Direction](#direction) | 否  | 规则链。添加防护墙过滤规则时必填；移除防火墙时非必填，表示清空所有的[Direction](#direction)链。当值为空时srcAddr，destAddr，srcPort，destPort，appUid也必须传入空值。      |
| action    | [Action](#action)       | 否  | 接收或者丢弃数据包。添加防护墙过滤规则时必填；移除防火墙时非必填，表示清空所有的匹配[Action](#action)规则的链。当值为空时srcAddr，destAddr，srcPort，destPort，appUid也必须传入空值。 |
...
| appUid     | string            | 否  | 应用uid。                                                                                            |
| action     | [Action](#action) | 否  | 接收或者丢弃数据包。添加域名过滤规则时必填；移除域名过滤规则时非必填，表示清空所有的匹配[Action](#action)规则的链。当值为空时，domainName，appUid也必须传入空值。 |