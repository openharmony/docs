...
| ACCELEROMETER_ROTATION_STATUS    | string | 是   | 否   | 是否使用加速计更改屏幕方向，即是否启用自动旋转。<br>值 1 表示默认情况下启用加速度计；<br/> 值为0 表示默认情况下不启用加速计。 |
...
| 类型    | 说明                                                         |
| ------- | ------------------------------------------------------------ |
| boolean | 返回设置数据项的值是否成功的结果。true表示设置成功，false表示表示设置失败。 |
