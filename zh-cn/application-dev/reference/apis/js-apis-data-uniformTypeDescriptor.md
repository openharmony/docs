...
let typeA = uniformTypeDescriptor.getTypeDescriptor('general.type-script');
let typeB = uniformTypeDescriptor.getTypeDescriptor('general.type-script');
let ret;
try{
    if(typeA.equals(typeB)) {
        console.info(`typeA equals typeB`);   
    }
} catch(e){
    let error: BusinessError = e as BusinessError;
    console.error(`equals throws an exception. code is ${error.code}, message is ${error.message} `);
}
...
let typeObj = uniformTypeDescriptor.getTypeDescriptor('com.adobe.photoshop-image');
