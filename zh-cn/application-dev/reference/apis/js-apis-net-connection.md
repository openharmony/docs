...
| port  | number | 否  |  主机端口。 |
| exclusionList  | Array<string> | 否  | 不使用代理的主机名列表，具体规则如下：<br/>1、域名匹配规则：<br/>（1）完全匹配：主机名必须与列表中的主机名完全相同。<br/>（2）包含字符串：主机名只要包含列表中的任意一个主机名子串，就可以匹配。<br/>例如，如果在主机名列表中设置了 “ample.com”，则  “ample.com”、“www.ample.com”、“ample.com:80”都会被匹配，而 “www.example.com”、“ample.com.org”则不会被匹配。<br/>（3）通配符模式：通配符可以使用一个星号“*”，通配符模式表示模糊匹配，用于匹配一组主机名，它表示任意长度的字符串（不含点号），例如“\*.example.com”将匹配所有以“example.com”结尾的主机名。<br/>2、IP 地址匹配规则：<br/>（1）完全匹配：IP 地址必须与列表中的 IP 地址完全相同。<br/>（2）CIDR 表示法：CIDR表示法表示模糊匹配，用于匹配一组IP地址。CIDR使用“/”后面跟一个数字表示要匹配的网络前缀长度，例如“192.168.0.0/16”表示匹配所有以“192.168”开头的IP地址。<br/>3、每个主机名可以包含可选的端口号（例如“www.example.com:8080”、“192.168.0.1:8080”），如果主机名中包含端口号，那么只有指定端口的主机才会匹配。<br/>4、匹配规则不区分大小写。<br/>5、匹配主机名时，不考虑http和https等协议前缀。 |
