...
| dataShareUpdate<sup>11+</sup> | [DataShareUpdate](#datashareupdate) | 否 | 点击按钮将更新应用自己的数据库。<br>**系统接口**: 系统接口，三方应用不支持调用。 |
...
| 名称 | 类型 | 必填 | 说明 |
| -------- | -------- | -------- | -------- |
| uri<sup>11+</sup> | string | 是 | 数据使用的URI，是跨应用数据访问的唯一标识。<br>**系统接口**: 系统接口，三方应用不支持调用。 |
| equalTo<sup>11+</sup> | [key: string]: number \| string \| boolean | 是 | 指示筛选条件，当前仅支持通过等于筛选。<br>**系统接口**: 系统接口，三方应用不支持调用。 |
| value<sup>11+</sup> | [ValueBucket](js-apis-data-valuesBucket.md#valuesbucket) | 是 | 指示要更新的数据。<br>**系统接口**: 系统接口，三方应用不支持调用。 |
...
| groupId | string | 否 | 指明提醒使用相同的组id。相同组id中，一个提醒被点击不在提醒后，组内其他提醒也会被取消。 |
...
| daysOfWeek<sup>11+</sup> | Array\<number> | 否 | 指明每周哪几天需要重复提醒。范围为周一到周末，对应数字为1到7。 |
