...
添加应用至应用运行禁用名单，添加至禁用名单的应用不允许在当前/指定用户下运行。
...
| 类型                | 说明                             |
| ------------------- | -------------------------------- |
| Array&lt;string&gt; | 返回当前/指定用户下的应用运行禁用名单。 |
...
添加保活应用，当前仅支持2in1设备。如果将应用添加至应用运行禁用名单[addDisallowedRunningBundlesSync](#applicationmanageradddisallowedrunningbundlessync)，就不能将应用添加至保活，否则会冲突。
