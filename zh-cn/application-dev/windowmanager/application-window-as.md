...
> **说明：**  
> 由于以下几种情况，移动设备（手机、平板在非自由多窗模式）场景下不推荐使用子窗口，优先推荐使用控件[overlay](../reference/apis-arkui/arkui-ts/ts-universal-attributes-overlay.md)能力实现。  
> - 移动设备（手机、平板在非自由多窗模式）场景下子窗不能超出主窗口范围，与控件一致。  
