...
开发者可以WindowManager模块提供的能力去将多模触摸事件注入给目标窗口，仅支持注入同进程窗口，且该注入不会触发窗口焦点和层级变化，不支持窗口拖拽，事件会直接发送给ArkUI。该接口需要在指定窗口loadcontent之后调用。
...
- 仅支持注入同进程窗口，且该注入不会触发窗口焦点和层级变化，不支持窗口拖拽，事件会直接发送给ArkUI。
