# Window Manager<!--window-manager-->

<!--Kit: ArkUI-->
<!--Subsystem: Window-->
<!--Owner: @fei_1007-->
<!--Designer: @gcw_sPCsris4-->
<!--Tester: @qinliwen0417-->
<!--Adviser: @ge-yafang-->
<!-- md-trans-meta sourceCommit=e3c52b80ea412371fb2dea52b278788d7531f840 translatedAt=2026-07-16T06:46:50.682Z pushedAt=2026-07-16T11:44:26.777Z -->

- [Window Overview](window-overview.md)

- Window Types<!--window-type-->

  - [Window Type Development Overview](window-type-overview.md)

  - [Subwindow Development Guide](subwindow-guide.md)

  - [Modal Window Development](dialog-window-guide.md)

  - [Global Floating Window Development](global-floating-window-guide.md)<!--RP1--><!--RP1End-->

  <!--Del-->

  - [System Window Development (Stage Model Only) (for System Applications Only)](system-window-stage-sys.md)

  <!--DelEnd-->

- Window Modes<!--window-mode-->

  - [Window Mode Overview](window-mode-overview.md)<!--RP2--><!--RP2End-->

  - Freeform Window<!--freeform-window-->

    - [Freeform Window Overview](freeform-window-overview.md)

    - [App Adaptation to Freeform Window](application-adaptation-freeform-window.md)

- Basic Window Capabilities<!--window-basic-capabilities-->

  - [Window Lifecycle](window-lifecycle.md)

  - Window Rotation and Practical Cases<!--rotation-->

    - [Window Rotation](window-rotation.md)

    - [Window Rotation Practical Cases](window-rotation-practical-case.md)

  - [Window Layout](window-layout.md)

  - [Window Focus](window-focus-guide.md)

  - [Window Z-Order](window-zorder-guide.md)

  - [Immersive Window](immersive-window-feature.md)

- Other Development Scenarios<!--other-scenarios-->

  - [Screenshot and Privacy Mode (ArkTS)](screenshot-and-privacy-mode.md)

  - [Controlling Brightness and Always-On (ArkTS)](brightness-control.md)

  - [Setting Window Animation Effects (ArkTS)](window-animation.md)

  - [Controlling Window Appearance (ArkTS)](window-appearance.md)

  - [Using WindowManager to Manage Multimodal Input Events (C/C++)](native-window-event-filter.md)

- Configuring and Using the Application Starting Window<!--launch-page-->

  - [Application Starting Window Overview](launch-page-overview.md)

  - [Configuring the Application Starting Window](launch-page-config.md)

  - [Configuring Starting Window Resources](launch-page-resource-config.md)

- [Window Metadata Configuration](window-config-m.md)

- [Glossary](window-terminology.md)

- [Window Development FAQs](window-faqs.md)

- [Common Log Issues and Troubleshooting in Window Development](window-log-faqs.md)