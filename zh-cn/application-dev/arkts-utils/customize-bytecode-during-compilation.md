...
准备一个操作方舟字节码文件的动态库文件，在工程的配置文件build-profile.json5中[配置编译选项transformLib](arkoptions-guide.md#transformlib)，选项值为这个动态库的路径。编译器会在编译过程中加载该动态库，调用其中的Transform方法，并在Transform方法执行完成后卸载该动态库。
