...
本文将介绍如何在多级Worker间高性能地进行消息通信，高性能消息通信的关键在于[Sendable对象](./arkts-sendable.md)，搭配Worker的[postMessageWithSharedSendable接口](../reference/apis-arkts/js-apis-worker.md#postmessagewithsharedsendable12)使用，可以在线程间进行高性能的对象传递。以数据克隆场景为例，假设有三个Worker，一个父Worker和两个子Worker，父Worker负责创建子Worker，并向子Worker发送数据克隆任务，子Worker负责接收任务并执行数据克隆操作，最后将克隆结果返回给父Worker。
