...
### 【普通应用】通过预授权的方式申请Download目录权限和获取对应路径
...
### 【普通应用】通过FilePicker选择路径后设置永久授权，以及应用重启后的使能过程

应用如果需要对选择的路径获取永久授权，需要自己进行持久化授权。

第一步：应用调用picker的select接口，通过FilePicker选择和保存路径URI，此时应用获得该路径的临时访问权限。以下示例代码演示了获取文件夹URI的过程：
