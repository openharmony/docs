...
```
 ...
 ### PNO扫描
  ...
  PNO 扫描即首选分流网络扫描（Preferred Network Offload scanning），是一种用于降低移动设备功耗的Wi-Fi扫描技术。
  触发PNO扫描条件：当设备未连接Wi-Fi且屏幕关闭后，会触发PNO扫描，让设备在后台寻找和连接首选的Wi-Fi网络。

 ### 周期扫描

  WiFi周期扫描是指无线设备（如智能手机、平板电脑、笔记本电脑等）按照一定的时间间隔，自动搜索周围可用的WiFi网络的过程。
  触发场景描述：
  1、设备已与Wi-Fi连接且亮屏，判断当前链路是否可以上网，可以上网的话扫描间隔1h，不可上网的话判断是否静止，静止场景下扫描周期从20s开始倍增，最大300s；非静止场景扫描周期从20s开始最大160s。
  2、设备亮屏且Wi-Fi未连接，判断当前是否支持扫描精灵，扫描精灵开启场景判断是否静止，静止场景下扫描周期为10/10/10/30/30/30/300...，非静止为10/10/10/30/30/30/60...；扫描精灵未开启场景下，静止场景扫描周期从20s开始倍增，最大300s；非静止场景扫描周期从20s开始最大160s。

### 扫描管控

  WiFi扫描管控是指对无线设备扫描WiFi网络的行为进行管理和控制，以下是一些常见的管控方式和应用场景：
  1、WiFi在关闭的情况下不触发周期扫描和PNO扫描。
  2、WiFi连接过程中不允许触发扫描。
  3、前台应用除了白名单应用，2分钟内最多发起4次扫描。
  4、设备温度达到阈值，设备扫描会被管控。
  5、除白名单应用，设备灭屏状态下不允许发起扫描。
