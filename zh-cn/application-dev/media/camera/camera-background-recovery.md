...
- 相机应用从后台切换至前台时，需要重启相机设备的预览流、拍照流以及相机会话管理。在参考以下示例前，开发者了解[设备输入](camera-device-input.md)、[会话管理](camera-session-management.md)等单个操作。 
...
相机应用从后台切换至前台启动恢复需要通过onPageShow调用。
