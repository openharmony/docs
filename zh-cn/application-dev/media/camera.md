...
## 开发步骤
在计算器中连接分布式设备，在获取相机列表时getCameras()，使用camera[1000]、camera[1001]创建camerainput，之后调用与本地相机使用一样。参考：[相机开发指导](../application-dev/media/camera.md)
...
打开计算器，点击右上角小箭头，出现新的窗口，按步骤完成设备连接。
