...
音频流基础信息通过[AudioStreamInfo](../../reference/apis-audio-kit/js-apis-audio.md#audiostreaminfo8)表示，包含采样、声道、位宽、编码信息，是创建音频播放或录制流的必要参数，描述了音频数据的基本属性。在配置时开发者需要保证基础信息与传输的音频数据相匹配，音频系统才能正确处理数据，可参考[使用合适的音频类型](using-right-streamusage-and-sourcetype.md)。
