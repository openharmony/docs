...
使用AudioHaptic播放音频并同步开启振动，涉及到音频及振动资源的管理、音频时延模式及音频流使用类型的配置、音振播放器的创建及管理等。本开发指导将以一次音振协同播放的过程为例，向开发者讲解如何使用AudioHaptic进行音振协同播放，建议配合[AudioHaptic的API说明](../reference/apis/js-apis-audioHaptic.md#audiohapticmanager)阅读。

### 开发步骤及注意事项

1. 获取音振管理器实例，并注册音频及振动资源，资源支持情况可以查看[AudioHapticManager](../reference/apis/js-apis-audioHaptic.md#audiohapticmanager)。
...
2. 设置音振播放器参数，各参数作用可以查看[AudioHapticManager](../reference/apis/js-apis-audioHaptic.md#audiohapticmanager)。
