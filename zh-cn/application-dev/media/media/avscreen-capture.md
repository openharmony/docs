...
使用AVScreenCapture录制屏幕涉及到AVScreenCapture实例的创建、音视频采集参数的配置、采集的开始与停止、资源的释放等。本开发指导将以一次录制屏幕数据的过程为例，向开发者讲解如何使用AVScreenCapture进行屏幕录制，详细的API声明请参考<!--start_del-->[AVScreenCapture API参考](../../reference/apis-media-kit//_a_v_screen_capture.md)<!--del_end-->。
...
在开发前，请根据应用实际需求，参考[申请应用权限]<!--start_del-->(../../security/AccessToken/determine-application-mode.md)<!--del_end-->申请以下权限：
