...
2. 设置业务需要的监听事件，搭配全流程场景使用。支持的监听事件包括：
   ...
   | bufferingUpdate | 用于网络播放，监听网络播放缓冲信息，用于上报缓冲百分比以及缓存播放进度。 |
   | audioInterrupt | 监听音频焦点切换信息，搭配属性audioInterruptMode使用。<br/>如果当前设备存在多个音频正在播放，音频焦点被切换（即播放其他媒体如通话等）时将上报该事件，应用可以及时处理。 |
   | trackChange | 用于监听切轨成功事件。<br/>在使用selectTrack()切换轨道时，须注册监听该事件。<br/>
...
4. 准备播放：调用prepare()，AVPlayer进入prepared状态，此时可调用getTrackDescription获取该DASH资源的所有轨道信息，获取duration或者设置音量等。

5. 播放控制：播放play()，暂停pause()，跳转seek()，停止stop() 等操作。
...
以下示例描述了播放一个DASH流媒体视频的完整流程，仅供开发者参考。
...
export class AVPlayerDemo {
  ...
  setAVPlayerCallback(avPlayer: media.AVPlayer) {
    ...
    avPlayer.on('trackChange', (index: number, isSelect: boolean) => {
      console.info(`trackChanged, index: ${index}, isSelect: ${isSelect}`);
    });
  ...
  async avPlayerDashDemo() {
    ...
    // 选择其中一个音频轨道
    avPlayer.selectTrack(this.audioTrackList[0]);
    // 取消选择的音频轨道
    avPlayer.deselectTrack(this.audioTrackList[0]);
    // 选择其中一个视频轨道
    // avPlayer.selectTrack(this.audioTrackList[0]);
    // 取消选择的视频轨道
    // avPlayer.deselectTrack(this.audioTrackList[0]);
