...
- Video组件：封装了视频播放的基础能力，需要设置数据源以及基础信息即可播放视频，但相对扩展能力较弱。Video组件由ArkUI提供能力，相关指导请参考UI开发文档-[Video组件]<!--start_del-->(../../ui/arkts-common-components-video-player.md)<!--del_end-->。

本开发指导将介绍如何使用AVPlayer开发视频播放功能，以完整地播放一个视频作为示例，实现端到端播放原始媒体资源。如果要实现后台播放或熄屏播放，需要使用[AVSession（媒体会话）]<!--start_del-->(../avsession/avsession-overview.md)<!--del_end-->和<!--start_del-->[申请长时任务](../../task-management/continuous-task.md)<!--del_end-->，避免播放过程中音频模块被系统强制中断。
...
3. 设置资源：设置属性url，AVPlayer进入initialized状态。
   ...
   > 
   > - 如果使用本地资源播放，必须确认资源文件可用，并使用应用沙箱路径访问对应资源，参考<!--start_del-->[获取应用文件路径](../../application-models/application-context-stage.md#获取应用文件路径)<!--del_end-->。应用沙箱的介绍及如何向应用沙箱推送文件，请参考[文件管理]<!--start_del-->(../../file-management/app-sandbox-directory.md)<!--del_end-->。
   > 
   > - 如果使用网络播放路径，需<!--start_del-->[声明权限](../../security/AccessToken/declare-permissions.md)<!--del_end-->：ohos.permission.INTERNET。
   > 
   > - 如果使用ResourceManager.getRawFd打开HAP资源文件描述符，使用方法可参考[ResourceManager API参考]<!--start_del-->(../../reference/apis-localization-kit/js-apis-resource-manager.md#getrawfd9)<!--del_end-->。
   > 
   > - 需要使用[支持的播放格式与协议](media-kit-intro.md#支持的格式与协议)。

4. 设置窗口：获取并设置属性SurfaceID，用于设置显示画面。
   应用需要从XComponent组件获取surfaceID，获取方式请参考[XComponent]<!--start_del-->(../../reference/apis-arkui/arkui-ts/ts-basic-components-xcomponent.md)<!--del_end-->。
