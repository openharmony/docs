...
用户进行了分布式投播以后，在投播的被控端设备上自动创建出对应的媒体会话，两侧的媒体会话可以相互联动，具体体现在：
...
4. 主控端本地媒体会话接收远端控制命令后，回调通知本端的音频类应用。

5. 主控端本地的媒体会话信息变更实时同步到远端播控中心显示。

6. 远端设备连接断开后，音频切回主控端本地并暂停播放（音频模块完成回切，媒体会话通知应用暂停）。
...
- 应用投播：音视频应用通过集成投播组件，在应用内发起分布式投播（当前暂未支持）。

  应用投播对单个应用生效。应用投播后，主控端设备中发起投播的应用音频从被控端设备播放，其它应用的音频依旧从主控端设备播放。

此外，投播支持抢占，后发起投播的应用可以抢占之前投播的应用，在远端设备进行音频播放。
...
因此，通过分布式媒体会话进行投播，不仅可以实现音频在远端设备播放，还可以让远端设备显示播放信息。同时借助媒体会话的机制，可以实现对远端设备上播放的音频进行控制。
