...
| time   | 运行命令行并报告实际、用户和系统时间（以秒为单位）。（real=墙上的时钟，user =命令代码使用的cpu）system =操作系统代表命令使用的cpu。)<br />usage: time [-pv] COMMAND [ARGS...] |
...
| timeout | 作为子进程运行命令行，向子进程发送一个信号，如果命令不会很快退出。DURATION可以是小数。可选的后缀可以是“m”（分钟）、“h”（小时）、“d”（天）或“s”（秒，默认值）。<br />usage: timeout [-k DURATION] [-s SIGNAL] DURATION COMMAND... |
| top     | 实时显示流程活动。<br />usage: top [-Hbq] [-k FIELD,] [-o FIELD,] [-s SORT] [-n NUMBER] [-m LINES]  [-d SECONDS] [-p PID,] [-u USER,] |
| iotop   | 按I/O对进程进行排名。<br />usage: iotop [-AaKObq] [-n NUMBER] [-d SECONDS] [-p PID,] [-u USER,] |
| watch   | 每隔-n秒运行一次PROG，显示输出。按q退出。<br />usage: watch [-teb] [-n SEC] PROG ARGS |
| xargs   | 运行命令行一次或多次，附加stdin中的参数。<br />usage: xargs [-0prt] [-s NUM] [-n NUM] [-E STR]  COMMAND... |

### 设备节点操作(仅支持查询)

| 命令 | 说明 |
| :- | :- |
| df    | “disk  free”命令显示的磁盘空间总量/已用/可用命令行中列出的每个文件系统，或当前已装载的所有文件系统。<br />usage: df [-HPkhi] [-t type] [FILESYSTEM...] |
...
| cmp       | 比较文件的内容（如果只给出一个，则与stdin进行比较），可选在开始时跳过字节。<br />usage: cmp [-l] [-s] FILE1 [FILE2 [SKIP1 [SKIP2]]] |
...
| dos2unix  | 将换行符格式从dos“\\r\\n”转换为unix“\\n”。如果没有列出文件，从stdin获取输入。“-”代表stdin。<br />usage: dos2unix [FILE...] |
| echo      | 将每个参数写入stdout，每个参数之间有一个空格，后跟一个换行符。<br />usage: echo [-neE] [args...] |
| egrep     | 显示匹配正则表达式的行。如果没有-e，则第一个参数为要匹配的正则表达式。没有文件（或“-”文件名）读取stdin。如果匹配，则返回0；如果找不到匹配，则返回1；如果命令错误，则返回2。<br />usage: egrep [-EFrivwcloqsHbhn] [-ABC NUM] [-m MAX] [-e REGEX]... [-MS  PATTERN]... [-f REGFILE] [FILE]... |
| expand    | 根据制表符展开制表符为空格。<br />usage: expand [-t TABLIST] [FILE...] |
| fgrep     | 显示匹配正则表达式的行。如果没有-e，则第一个参数为要匹配的正则表达式。没有文件（或“-”文件名）读取stdin。如果匹配，则返回0；如果找不到匹配，则返回1；如果命令错误，则返回2。<br />usage: fgrep [-EFrivwcloqsHbhn] [-ABC NUM] [-m MAX] [-e REGEX]... [-MS  PATTERN]... [-f REGFILE] [FILE]... |
...
| head      | 将文件中的第一行复制到stdout中。如果未列出任何文件从stdin拷贝. “-”代表stdin<br />usage: head [-n number] [file...] |
...
| md5sum    | 计算每个输入文件的哈希，如果没有，则从stdin读取。每个输入文件输出一行哈希后跟文件名。<br />usage: md5sum [-bcs] [FILE]... |
...
| sort      | 对从输入文件（或stdin）到stdout的所有文本行进行排序。<br />usage: sort [-Mbcdfginrsuz] [FILE...] [-k#[,#[x]] [-t X]] [-o FILE] |
| split     | 将输入（或stdin）数据复制到一系列输出（或“x”）文件，使用按字母顺序递增的后缀（aa,ab,ac...az,ba,bb...）。<br />usage: split [-a SUFFIX\_LEN] [-b BYTES] [-l LINES] [INPUT [OUTPUT]] |
...
| tail      | 将文件中的最后一行复制到stdout中。“-”代表stdin<br />usage: tail [-n\|c NUMBER] [-f] [FILE...] |
...
| unix2dos  | 将换行符格式从unix“\\n”转换为dos“\\r\\n”。如果没有列出文件，从stdin获取输入。“-”是stdin的同义词。<br />usage: unix2dos [FILE...] |
...
| xxd       | Hexdump将文件转储到stdout。若没有列出任何文件，请从stdin复制。<br />usage: xxd [-c n] [-g n] [-i] [-l n] [-o n] [-p] [-r] [-s n] [file] |
