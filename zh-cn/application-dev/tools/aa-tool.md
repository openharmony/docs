...
```
  ...
  **启动命令参数列表**
  ...
  | -W | 可选参数，仅当显式启动UIAbility，该参数生效，从API version 20开始支持。统计UIAbility从启动到切换至[前台](../application-models/uiability-lifecycle.md#foreground和background状态)状态耗时。
输出如下信息：<br> StartMode：UIAbility启动模式，值：Cold表示冷启动/Hot表示热启动。<br> BundleName：目标应用bundleName。<br>AbilityName：目标应用abilityName。<br>ModuleName：目标应用moduleName。<br> TotalTime：<br> - 冷启动场景下，系统侧接收到aa启动UIAbility请求到该UIAbility完成首帧绘制的耗时，单位毫秒（ms）。<br> - 热启动场景下，系统侧接收到aa启动UIAbility请求到该UIAbility状态切换至[前台](../application-models/uiability-lifecycle.md#foreground和background状态)的耗时，单位毫秒（ms）。<br>WaitTime：命令启动到命令执行结束的耗时，单位毫秒（ms）。<br>**说明：**<br>  命令中带有"-m"参数时会打印moduleName，否则不打印。      |
