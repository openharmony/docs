...
打包工具和拆包工具，在程序编译完成后，对编译出的文件等进行打包，以供安装发布。打包工具支持应用程序包(HAP)，上架应用市场的应用集合（APP），快速修复包（HQF），上架应用市场的快速修复包集合（APPQF），静态共享库（AHR），动态共享库（HSP）包的生成，拆包工具用于对HAP，APP，HQF，APPQF，HAR，HSP包的拆包及对HAP，HSP，APP，APPQF的解析。通常打包过程会在DevEco Studio中自动完成，但开发者也可根据需要通过打包工具的jar包进行打包，jar包通常存放在sdk路径下的toolchains目录中。
