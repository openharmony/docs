...
>-   When data is updated through  **onDataChanged**  of  **LazyForEach**, if  **itemGenerator**  contains a fully static view \(the view does not contain state variables\), the view will not be updated.
>-   **LazyForEach**  is used to automatically generate child components from the provided data source.
>-   **LazyForEach**  must be used in the container component. Only the  **<List\>**,  **<Grid\>**, and  **<Swiper\>**  components support on-demand data loading \(that is, only the visible part and a small amount of data before and after the visible part are loaded for buffering\). For other components, all data is loaded at a time.
...
>-   The generated child component must be allowed in the parent container component of  **LazyForEach**, enabling  **LazyForEach**  to be included in the  **if/else**  conditional rendering statement.
...
>-   The  **if/else**  conditional rendering statement is not allowed in  **LazyForEach**.
