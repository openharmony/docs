...
In the commands listed in the following table, -t<!--Del-->, -r, and <!--DelEnd-->-v are mandatory parameters, and -i, -b, -n and -p are optional parameters. For the <!--Del-->atm dump -r command and <!--DelEnd-->atm dump -v command, the -i and -p parameters can be used together. <!--DelEnd-->For the atm dump -t command, the -i, -b, and -n parameters can only be used independently.
