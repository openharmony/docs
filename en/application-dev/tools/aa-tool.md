...
2. Implicitly start the ability.
  ...
  - Run the **-U** command to directly start the ability.

    ```bash
    aa start -U https://www.test.com:8080/path
    ```

  - If matching by type is required, carry the **-t** field.

    ```bash
    aa start -U https://www.test.com:8080/path -t text/plain
    ```

  - If parameters need to be carried during startup, run the following command:
