...
```
  ...
  **Parameters**
  ...
  | -N | (Optional) Whether to enable the debugging in the startup phase.       |
  | -C | (Optional) Whether to enable ASan debugging.       |
...
```
  ...
  **Parameters in the command for forcibly stopping a process**
  ...
  | -r | Specifies the exit reason of a process. This parameter must be used together with -p to set the exit reason of the process with the specified PID.|
