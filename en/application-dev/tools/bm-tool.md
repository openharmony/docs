...
> **NOTE**
>
> If the same plug-in is installed in the same application, the plug-in version is updated and the plug-in cannot be downgraded. After the plug-in version is updated, you need to restart the application plug-in for the update to take effect. You are not advised to install a plugin with the same name as the host application module. Currently, this plugin is not supported in the runtime state.
...
* Scenario 2: The [release certificate and release profile](https://developer.huawei.com/consumer/en/doc/app/agc-help-releaseharmony-0000001933963166) is used during signature.
...
1. The signatures of the existing application and new application are different, or the signatures of HAPs and HSPs are different. If at least one of the signature [key](https://developer.huawei.com/consumer/en/doc/harmonyos-guides/ide-signing#section462703710326) or [app ID](https://developer.huawei.com/consumer/en/doc/app/agc-help-createharmonyapp-0000001945392297) of the two applications is the same, the signatures of the two applications are considered to be the same. **Keep Application Data** is selected in **Edit Configurations** (the application installation is overwritten) and the application is re-signed.
2. If an application is uninstalled but its data is kept, and a new application with the same bundle name is later installed, it is necessary to check whether the identity details match. This error is reported if the values of [key](https://developer.huawei.com/consumer/en/doc/harmonyos-guides/ide-signing#section462703710326) and [app ID](https://developer.huawei.com/consumer/en/doc/app/agc-help-createharmonyapp-0000001945392297) are different.
...
Do not install applications of the enterprise_mdm or enterprise_normal type in [singing certification profile](https://developer.huawei.com/consumer/en/doc/app/agc-help-add-releaseprofile-0000001914714796) on non-enterprise devices.
...
Do not run the bm command to install apps whose type is app_gallery and signature certificate type is release in [singing certification profile](https://developer.huawei.com/consumer/en/doc/app/agc-help-add-releaseprofile-0000001914714796).

**Possible Causes**

Install the app. The type in [singing certification profile](https://developer.huawei.com/consumer/en/doc/app/agc-help-add-releaseprofile-0000001914714796) is app_gallery and the signature certificate type is release.

**Solution**

1. Use a file whose type is not app_gallery in [singing certification profile](https://developer.huawei.com/consumer/en/doc/app/agc-help-add-releaseprofile-0000001914714796) to re-sign the app.
...
The [key](https://developer.huawei.com/consumer/en/doc/harmonyos-guides/ide-signing#section462703710326) and [APP ID](https://developer.huawei.com/consumer/cn/doc/app/agc-help-createharmonyapp-0000001945392297) are different from those of the uninstalled pre-installed app.

**Solution**

1. Re-sign the app to ensure that the [key](https://developer.huawei.com/consumer/en/doc/harmonyos-guides/ide-signing#section462703710326) and [APP ID](https://developer.huawei.com/consumer/cn/doc/app/agc-help-createharmonyapp-0000001945392297) are the same as those of the pre-installed app.
...
The type in the [singing certification profile](https://developer.huawei.com/consumer/en/doc/app/agc-help-add-releaseprofile-0000001914714796) is restricted and cannot be installed on the current device.
