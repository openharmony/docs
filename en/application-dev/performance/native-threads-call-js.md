# Using Native to Implement Cross-Thread Invoking
...
The following describes how to create subthreads in native mode to execute time-consuming tasks and ensure seamless interaction with JavaScript. Therefore, developers can use the [NAPI (Node-API)](../reference/native-lib/napi.md) API provided by the [arkui_napi](https://gitee.com/openharmony/arkui_napi) repository to implement cross-language invoking. The design of NAPI strictly complies with the NAPI specifications of the [Node.js](https://nodejs.org/api/n-api.html) so that developers can understand and use NAPI with ease. 
It is particularly emphasized that JavaScript functions can be called only in the main thread. If a subthread is created using std::thread or pthread on the native side, napi_env, napi_value, and napi_ref cannot be directly used in the subthread context. To ensure correctness, if a JavaScript function needs to be called back after the native side completes the calculation or processing of the subthread, the result must be transferred back to the main thread through the thread synchronization mechanism. Then the JavaScript function can be securely called in the main thread environment. 
To solve this problem, choose either of the following two effective solutions.

## Solution

### Linear Safe Functions

napi_threadsafe_function provides an API to create a function object that can be shared and securely used among multiple threads. With this mechanism, subthreads can transfer data to the main thread. After receiving the data, the main thread calls the JavaScript callback function to process the data. This API contains methods for creating and destroying thread-safe functions and for sending messages and synchronizing data between them. The general procedure for using napi_threadsafe_function includes:

**Creating a thread-safe function**: Create a thread-safe function object by calling **napi_create_threadsafe_function()**. During this process, you need to specify a JavaScript callback function, which is executed on the main thread. In addition, you need to set related context information, which can be shared among multiple threads and can be obtained by calling **napi_get_threadsafe_function_context()** at any time. You can also selectively provide a napi_finalize callback to perform resource cleanup operations when thread-safe functions are destroyed.

**Obtaining the right to use**: Before using a thread-safe function, call the **napi_acquire_threadsafe_function()** function to indicate that the thread is ready and then perform operations on the thread-safe function.

**Calling the callback function from a subthread**: In a subthread, call **napi_call_threadsafe_function()** to asynchronously trigger the JavaScript callback function and transfer the required data as parameters to the callback function. The call will be queued and finally executed on the JavaScript main thread.

**Resource cleanup**: When a thread-safe function is no longer needed, the resources associated with it should be correctly released and cleaned up. Generally, this can be done by calling **napi_release_threadsafe_function()**. This function processes the callback that is not complete according to the preset policy and destroys the thread-safe function object.
...
In-depth understanding and proper management of the lifecycle of objects between JavaScript and native APIs are critical to writing efficient code without memory leakage risks. It is recommended that developers further study documents and best practices related to [Lifecycle management](./develop-Native-modules-using-NAPI-safely-and-efficiently.md) to better master them.

### libuv

[libuv](../reference/native-lib/libuv.md) is an event-driven asynchronous I/O library. If time-consuming operations are directly processed in the main loop (event loop) of libuv, the execution of subsequent tasks will be blocked. To solve this problem, a thread pool is maintained inside libuv to perform some time-consuming operations. After these operations are complete, the callback function is added back to the event loop of the main thread for execution. 
...
**work_cb**: Callback function that performs the actual work. Some time-consuming operations can be performed here. This function runs on a thread in the thread pool. 
**after_work_cb**: Callback function called on the event loop thread after the work is complete. It is used to process the work_cb execution result or trigger further operations at the JavaScript layer.   
Note that although the uv_queue_work method does not directly involve the NAPI (Node-API) API, when interaction with the JavaScript thread is involved, especially when data is transferred from the native layer to the JavaScript layer and callback is triggered, the lifecycle of the napi_value object needs to be correctly managed. **napi_handle_scope** and related APIs must be properly used to ensure that the **napi_value** object created by the JavaScript callback method is valid during the entire execution process and resources are released at a proper time to avoid memory leakage. 

## Samples
The following example uses thread-safe functions and libuv to implement native cross-thread invoking. In this example, the variable value is increased by 10 in the JavaScript callback function transferred from ArkTS. Three subthreads are enabled on the native side to execute the service logic. After the service logic of the subthreads is complete, the main thread executes the JavaScript callback function transferred from ArkTS, in this way, the value of the ArkTS variable is increased by 30. [Complete sample code](https://gitee.com/openharmony/applications_app_samples/tree/OpenHarmony-5.0-Beta1/code/Performance/PerformanceLibrary/feature/nativeThreadsCallJS) is as follows:
### 1. Using Thread-safe Functions
**ArkTS implements a JavaScript callback function.** 
The parameter is param. In the function body, add 10 to param, bind the variable value, and return the latest param value. Use the callback function as a parameter to call the ThreadSafeTest API on the native side. 
   ...
   ```
**Implement a ThreadSafeTest API in the native main thread.**    
After receiving the JavaScript callback function transferred by ArkTS, the API creates a thread-safe function tsfn through **napi_create_threadsafe_function**, and tsfn calls back **ThreadSafeCallJs** in the main thread and then call the JavaScript callback function transferred from ArkTS in ThreadSafeCallJs. 
    
  ```c++
    //  src/main/cpp/hello.cpp

    napi_threadsafe_function tsfn; // Thread-safe function
    static int g_cValue; // Save the latest value of value and transfer it to the JavaScript callback function as a parameter.
    ...
    static void ThreadSafeCallJs(napi_env env, napi_value js_cb, void *context, void *data)
    {
        ...
        // Use the current value as a parameter to call the JavaScript function.
        napi_value argv;
        napi_create_int32(env, g_cValue, &argv);
        napi_value result = nullptr;
        napi_call_function(env, nullptr, js_cb, 1, &argv, &result);
        // g_cValue saves the result returned after the JavaScript function is called.
    ...
    // Use safe functions to call the JavaScript function across threads.
    static napi_value ThreadSafeTest(napi_env env, napi_callback_info info)
    {
        ...
        // Use the thread-safe function to call the JavaScript function across threads.
  ...
  ```
**Call the thread-safe function in the native subthread.**   
  ...
  //   src/main/cpp/hello.cpp
    ...
    for (int i = 0; i < g_threadNum; i++) {
        ...
        std::thread t([asyncContext]() {
            ...
            // Request a thread-safe function.
            ...
            /* If the JavaScript function is directly called in a subthread, the application breaks down.
            napi_value result = nullptr;
            napi_value argv;
            napi_create_int32(env,g_cValue, &argv);
            napi_call_function(env, nullptr, js_cb, 1, &argv, &result);
...
### 2. Using libuv
**ArkTS implements a JavaScript callback function.**    
The parameter is param. In the function body, add 10 to param, bind the variable value, and return the latest param value. Then use the callback function as a parameter to call the UvWorkTest API on the native side.
   ...
   ```
**Implement a UvWorkTest API in the native main thread.**   
After receiving the JavaScript callback function transferred by ArkTS, the API creates a subthread, creates a work task workReq in the execution function CallbackUvWorkTest of the subthread, and adds the work task to the libuv queue through uv_queue_work.
  ...
  //   src/main/cpp/hello.cpp
    ...
    void CallbackUvWorkTest(CallbackContext *context)
    {
        ...
        // Create a working data structure and add the customized data structure to data.
        ...
        // This print is located in the subthread.
    ...
    // Use **uv_work callback** to call the JavaScript function across threads.
...
work_cb is located in the subthread and executes the actual service logic. after_work_cb is located in the main thread and calls the JavaScript callback function transferred from ArkTS through napi_call_function.
  ...
  ```c++
    ...
    {
        CallbackContext *context = (CallbackContext *)workReq->data;
        // Executed by the main thread. The JavaScript function can be called here.
        ...
        // Save the JavaScript callback result.
...
## Takeaways
In both the thread-safe function and libuv solution, after the execution function of the subthread is complete, the subthread returns to the main thread, and pushes the JavaScript callback function to the event-loop queue of the main thread for execution. 
The difference is that the subthreads of libuv belong to the libuv thread pool, while the subthreads of thread-safe functions need to be created based on service requirements. In addition, in libuv, the JavaScript callback function can be passively executed only after the main function of the subthread is executed. In thread-safe functions, the JavaScript callback function can be actively called in any thread. 
