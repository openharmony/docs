# Intercepting Network Requests Initiated by Web components

The [Network Interception APIs](../reference/apis-arkweb/arkweb__scheme__handler_8h.md) is used to intercept requests sent by web components and provide user-defined response headers and bodies for intercepted requests.

## Setting a Network Interceptor for a Web Component

Sets **ArkWeb_SchemeHandler** for a specified web component or ServiceWorker. When the web kernel sends a scheme request, the callback of **ArkWeb_SchemeHandler** is triggered. You need to set the network interceptor after the Web component is initialized.

When a request starts, **ArkWeb_OnRequestStart** is called back. When a request ends, **ArkWeb_OnRequestStop** is called back.

To intercept the first request sent by a web component, you can use [initializeWebEngine](../reference/apis-arkweb/js-apis-webview.md#initializewebengine) to initialize the web component in advance and set an interceptor to intercept the request.
...
## Rules for Setting a User-Defined Scheme

To intercept the request of a user-defined scheme, you need to register the user-defined scheme with the web kernel in advance. You need to register the Web component before the Web component is initialized. If you register the Web component after the Web component is initialized, the registration fails.

  ```c++
    //Register the custom scheme with the web component and specify that the scheme must comply with the standard scheme rules to allow the scheme to send cross-domain requests.
    OH_ArkWeb_RegisterCustomSchemes("custom", ARKWEB_SCHEME_OPTION_STANDARD | ARKWEB_SCHEME_OPTION_CORS_ENABLED);
    //Register the custom-local scheme with the web component and specify that the scheme must comply with the same rules as the file scheme.
    OH_ArkWeb_RegisterCustomSchemes("custom-local", ARKWEB_SCHEME_OPTION_LOCAL);
    //Register custom-csp-bypassing with the web component and specify that the scheme must comply with the standard scheme rules to allow CSP check to be ignored.
    OH_ArkWeb_RegisterCustomSchemes("custom-csp-bypassing", ARKWEB_SCHEME_OPTION_CSP_BYPASSING | ARKWEB_SCHEME_OPTION_STANDARD);
    //Register "custom-isolated" with the web component and specify that the request of the scheme must be initiated from the web page loaded by the same scheme.
    OH_ArkWeb_RegisterCustomSchemes("custom-isolated", ARKWEB_SCHEME_OPTION_DISPLAY_ISOLATED);
  ```

The registration scheme needs to be registered before the web component is initialized, and the network interceptor needs to be set after the web component is initialized. Therefore, you are advised to call the C++ interface in onCreate of EntryAbility to register the scheme.
After the scheme is registered, use the [initializeWebEngine](../reference/apis-arkweb/js-apis-webview.md#initializewebengine) to initialize the web component. After the initialization is complete, set the network interceptor.
...
## Obtaining Information About an Intercepted Request

Use the OH_ArkWebResourceRequest_* interface to obtain information about intercepted requests. You can obtain information such as url, method, referrer, headers, and resourceType.
...
Uploaded data of PUT/POST requests can be obtained. The data type can be BYTES, FILE, BLOB, or CHUNKED.

  ```c++
    //Obtain the upload data of the intercepted request.
    OH_ArkWebResourceRequest_GetHttpBodyStream(resourceRequest(), &stream_);
    // Set the read callback function for reading the uploaded data.
...
## Provides a customized response body for intercepted requests.

The network interception of web components supports the function of providing customized response bodies for intercepted requests in stream mode in the worker thread. You can also end the currently intercepted request with a specific [network error code] (../reference/apis-arkweb/arkweb__net__error__list_8h.md).
  ...
  ```c++
    ...
    //This function can be called for multiple times. Data can be transferred to the web component in multiple copies.
    OH_ArkWebResourceHandler_DidReceiveData(resourceHandler, buffer, bufLen);

    //Reading the response body ends. If you want the request to fail, you can also call OH_ArkWebResourceHandler_DidFailWithError(resourceHandler_, errorCode).
...
To use DevEco Studio to create a default Native C++ project, you need to prepare an MP4 file named test.mp4 in advance and place the test.mp4 file in main/resources/rawfile.
...
//Register the configuration of the third-party protocol. This interface must be called before the web kernel is initialized. Otherwise, the registration fails.
...
//Callback for starting a request. In this function, a RawfileRequest is created to intercept web kernel requests.
...
//Callback when the request ends. In this function, RawfileRequest needs to be marked as ended, and ResourceHandler should not be used internally.
...
// Initialization callback of ArkWeb_HttpBodyStream.
...
//Read rawfile in the worker thread and return it to the web kernel through ResourceHandler.
...
<a href="https://www.example.com/video.html"&gt: Intercepts the video resource request and reads the local MP4 file.
<a href="https://www.example.com/csp_bypassing.html">The third-party protocol ignores the CSP check and successfully intercepts the request.
<a href="https://www.example.com/isolated.html">Test the interception of the third-party protocol whose ISOLATED attribute is set.
<a href="https://www.example.com/local.html">Test the interception of the third-party protocol whose local attribute is set.
<a href="https://www.example.com/service_worker.html"&gt: Intercept the request triggered by the service worker.
<a href="https://www.example.com/post_data.html">Test the BLOB type. http body stream</a><br/>
<a href="https://www.example.com/chunked_post_stream.html">Test the function of reading the chunked type http body stream</a>.
</body>
</html>
```
...
<div id="isolated_test">isolated_script.js blocked </div>
...
<div id="local_test">local_script.js blocked </div>
