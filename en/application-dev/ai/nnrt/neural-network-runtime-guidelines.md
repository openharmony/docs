...
This document uses the `Add` single-operator model shown in Figure 1 as an example to describe the development process of NNRt. The `Add` operator involves two inputs, one parameter, and one output. Wherein, the `activation` parameter is used to specify the type of the activation function in the `Add` operator.
...
This section describes the common APIs used in the development process of NNRt. 
