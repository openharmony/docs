...
<!--Tester: @jane_lz-->
<!--Adviser: @zengyawen-->
<!-- md-trans-meta sourceCommit=92364f50b50ccb69e019a60e7888e52daf004040 translatedAt=2026-08-14T06:45:26.296Z pushedAt=2026-08-14T07:59:38.473Z -->

<!--RP1-->

User Authentication Kit provides the capability to authenticate user identity based on the lock screen password, face, and fingerprints enrolled locally on the device. Starting from API version 26.0.0, companion device authentication is added as a new authentication method.

It provides system-level user identity authentication and a system-level user authentication widget that delivers a unified experience across devices and integrates multiple authentication methods (face, fingerprint, password, and companion device).

When a user requests access to personal data or performs sensitive operations, the application or system service invokes the user authentication widget to verify the user identity. The system responds to the user request accordingly only when the authentication is successful.
...
- Unified authentication interface
    ...
    Shield differences between authentication factors and provide unified APIs for invoking lock screen password, face, fingerprint, and companion device authentication.

    Provide combined authentication methods such as face, fingerprint, and lock screen password through the same set of APIs.

    Provide combinations of face authentication, fingerprint authentication, and custom authentication through the same set of APIs.
...
4. Authentication capabilities: including password authentication, face authentication, fingerprint authentication, and companion device authentication. They implement the capability to authenticate user identity based on the lock screen password, face, fingerprint, and companion device respectively, for scheduling by the unified user authentication framework.
...
| authType | Authentication type | uint32_t | Authentication type used for authentication success, including face, fingerprint, password (specifically lock screen password authentication), and companion device. |
...
| ATL3 | ACL ≥ 3, ASL ≥ 1<br>ACL ≥ 2, ASL ≥ 2| Capable of precisely identifying individual users with relatively strong liveness detection capabilities, such as specially enhanced secure 2D facial authentication.| Device unlocking, application login, and account login|
