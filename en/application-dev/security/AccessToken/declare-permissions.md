...
| usedScene | Scene under which the permission is used. This field is optional and must be set when a user_grant permission is requested.<br>This field consists of **abilities** and **when**, where **abilities** (mandatory) is an array of multiple UIAbility components, and **when** (optional) specifies when the permission is used.| **abilities**: an array of UIAbilities or ExtensionAbilities.<br>**when**: **inuse** (when used) and **always** (always).|
