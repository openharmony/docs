...
| struct&nbsp;&nbsp;[ArkWeb_ControllerAPI](_ark_web___controller_a_p_i.md) | Defines the native APIs of the controller. | 
...
| typedef enum [ArkWeb_ResourceType](#arkweb_resourcetype) [ArkWeb_ResourceType](#arkweb_resourcetype) | Defines an enum for the resource type. | 
...
| typedef struct ArkWeb_HttpBodyStream_ [ArkWeb_HttpBodyStream](#arkweb_httpbodystream) | Defines a struct for **ArkWeb_HttpBodyStream_**. | 
...
| Name| Description| 
| -------- | -------- |
| [ArkWeb_ErrorCode](#arkweb_errorcode) { ARKWEB_ERROR_UNKNOWN = 17100100, ARKWEB_INVALID_PARAM = 17100101, ARKWEB_SCHEME_REGISTER_FAILED = 17100102 } | Enumerates the error codes of ArkWeb NDK APIs. | 
| [ArkWeb_NativeAPIVariantKind](#arkweb_nativeapivariantkind) { ARKWEB_NATIVE_COMPONENT, ARKWEB_NATIVE_CONTROLLER } | Enumerates the Native API types. | 
...
| int32_t [OH_ArkWebResourceHandler_DidReceiveResponse](#oh_arkwebresourcehandler_didreceiveresponse) (const [ArkWeb_ResourceHandler](#arkweb_resourcehandler) \*resourceHandler, const [ArkWeb_Response](#arkweb_response) \*response) | Transfers the constructed response header to the intercepted request. | 
| int32_t [OH_ArkWebResourceHandler_DidReceiveData](#oh_arkwebresourcehandler_didreceivedata) (const [ArkWeb_ResourceHandler](#arkweb_resourcehandler) \*resourceHandler, const uint8_t \*buffer, int64_t bufLen) | Transfers the constructed response header to the intercepted request. | 
...
Defines a struct for **ArkWeb_HttpBodyStream_**.
...
Defines an enum for the resource type.

These constants match the corresponding items of ResourceType in Chromium and should not be renumbered.
...
| Value| Description| 
| -------- | -------- |
| ARKWEB_ERROR_UNKNOWN  | The error is unknown. | 
...
| Value| Description| 
| -------- | -------- |
| ARKWEB_NATIVE_COMPONENT  | Component-related API type. | 
| ARKWEB_NATIVE_CONTROLLER  | Controller-related API type. | 
...
| ARKWEB_ERR_FAILED  | The operation failed. | 
...
| ARKWEB_ERR_CONNECTION_RESET  | The connection is reset. | 
...
| ARKWEB_ERR_CERT_AUTHORITY_INVALID  | The server responds with a certificate signed by an authority that we do not trust. This may mean: 1. The attacker replaces the real certificate with a certificate that contains its public key and is signed by its associates.<br>1. The server has a valid certificate from a CA that we do not know but should trust.3. The server presents a self-signed certificate, which cannot defend against active attackers but passive attackers. | 
...
You can use **WebviewController.initializeWebEngine** to initialize BrowserContext without creating ArkWeb.
...
| Name| Description| 
| -------- | -------- |
| requestHeaderList | The list of the request headers. | 
...
| -------- | -------- |
| resourceRequest | ArkWeb_ResourceRequest. | 
| frameUrl | The URL of the frame that triggers the request. This function allocates memory for the URL string, and you should release the string using **OH_ArkWeb_ReleaseString**.| 
...
| -------- | -------- |
| resourceRequest | ArkWeb_ResourceRequest. | 
| method | The method of a HTTP request. This function allocates memory for the **method** string. You should use **OH_ArkWeb_ReleaseString** to release the string.| 
...
The resource types of a request. If **resourceRequest** is invalid, the value is -1.
...
| -------- | -------- |
| response | ArkWeb_Response. | 
| mimeType | The mime type of **ArkWeb_Response**. This function will allocate memory for the **mimeType** string. You need to release the string using **OH_ArkWeb_ReleaseString**.| 
...
Sets an **ArkWeb_SchemeHandler** for a specified scheme to intercept requests of the scheme type triggered by **ServiceWorker**.
