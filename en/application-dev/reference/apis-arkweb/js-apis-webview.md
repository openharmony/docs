...
Resources injected in this mode can be loaded only through HTML tags. If a **script** tag in the web page uses the **crossorigin** attribute, the **Cross-Origin** response header must be set in the **responseHeaders** parameter of the API. The value for this header should be **anonymous** or **use-credentials**.
...
The [getLastHitTest](#getlasthittest18) API is used to indicate a cursor node.
...
Provides the element information of the area being clicked. For details about the sample code, see [getLastHitTest](#getlasthittest18).
