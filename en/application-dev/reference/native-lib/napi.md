...
Node-API provides APIs to encapsulate JavaScript capabilities as native addons. It is independent of the underlying JavaScript and is maintained as part of Node.js.

## Supported Capabilities

Node-API insulates addons from changes in the underlying JavaScript engine and allows the modules compiled for one major version to run on later major versions without recompilation.
...
|FUNC|napi_throw|Throws a JavaScript value.|10|
|FUNC|napi_throw_error|Throws a JavaScript **Error** with text information.|10|
|FUNC|napi_throw_type_error|Throws a JavaScript **TypeError** with text information.|10|
|FUNC|napi_throw_range_error|Throws a JavaScript **RangeError** with text information.|10|
|FUNC|napi_is_error|Checks whether **napi_value** indicates an error object.|10|
|FUNC|napi_create_error|Creates a JavaScript **Error** with text information.|10|
|FUNC|napi_create_type_error|Creates a JavaScript **TypeError** with text information.|10|
|FUNC|napi_create_range_error|Creates a JavaScript **RangeError** with text information.|10|
...
|FUNC|napi_escape_handle|Promotes the handle to the input JavaScript object so that it is valid for the lifespan of its outer scope.|10|
...
|FUNC|napi_get_reference_value|Obtains the JavaScript **Object** associated with the reference.|10|
|FUNC|napi_create_array|Creates a JavaScript array.|10|
|FUNC|napi_create_array_with_length|Creates a JavaScript array of the specified length.|10|
|FUNC|napi_create_arraybuffer|Creates a JavaScript ArrayBuffer of the specified size.|10|
|FUNC|napi_create_external|Allocates a JavaScript value with external data.|10|
|FUNC|napi_create_external_arraybuffer|Allocates a JavaScript **ArrayBuffer** with external data.|10|
|FUNC|napi_create_object|Creates a default JavaScript **Object**.|10|
|FUNC|napi_create_symbol|Creates a JavaScript **Symbol**.|10|
|FUNC|napi_create_typedarray|Creates a JavaScript **TypeArray** from an existing **ArrayBuffer**.|10|
|FUNC|napi_create_dataview|Creates a JavaScript **DataView** from an existing **ArrayBuffer**.|10|
|FUNC|napi_create_int32|Creates a JavaScript number from C int32_t data.|10|
|FUNC|napi_create_uint32|Creates a JavaScript number from C uint32_t data.|10|
|FUNC|napi_create_int64|Creates a JavaScript number from C int64_t data.|10|
|FUNC|napi_create_double|Creates a JavaScript number from C double data.|10|
|FUNC|napi_create_string_latin1|Creates a JavaScript string from an ISO-8859-1-encoded C string.|10|
|FUNC|napi_create_string_utf8|Creates a JavaScript string from a UTF8-encoded C string.|10|
|FUNC|napi_create_string_utf16|Creates a JavaScript string from a UTF16-encoded C string.|10|
...
|FUNC|napi_get_prototype|Obtains the prototype of a JavaScript object.|10|
...
|FUNC|napi_get_value_bool|Obtains the C Boolean equivalent of a JavaScript Boolean value.|10|
|FUNC|napi_get_value_double|Obtains the C double equivalent of a JavaScript number.|10|
|FUNC|napi_get_value_external|Obtains the external data pointer previously passed through **napi_create_external()**.|10|
|FUNC|napi_get_value_int32|Obtains the C int32 equivalent of a JavaScript number.|10|
|FUNC|napi_get_value_int64|Obtains the C int64 equivalent of a JavaScript number.|10|
|FUNC|napi_get_value_string_latin1|Obtains the ISO-8859-1-encoded string corresponding to the given JavaScript value.|10|
|FUNC|napi_get_value_string_utf8|Obtains the UTF8-encoded string corresponding to the given JavaScript value.|10|
|FUNC|napi_get_value_string_utf16|Obtains the UTF16-encoded string corresponding to the given JavaScript value.|10|
|FUNC|napi_get_value_uint32|Obtains the C uint32 equivalent of a JavaScript number.|10|
|FUNC|napi_get_boolean|Obtains a JavaScript Boolean object based on a C Boolean value.|10|
...
|FUNC|napi_coerce_to_bool|Forcibly converts a JavaScript value to a JavaScript Boolean value.|10|
|FUNC|napi_coerce_to_number|Forcibly converts a JavaScript value to a JavaScript number.|10|
|FUNC|napi_coerce_to_object|Forcibly converts a JavaScript value to a JavaScript object.|10|
|FUNC|napi_coerce_to_string|Forcibly converts a JavaScript value to a JavaScript string.|10|
|FUNC|napi_typeof|Obtains the JavaScript type of a JavaScript value.|10|
|FUNC|napi_instanceof|Checks whether an object is an instance of the specified constructor.|10|
|FUNC|napi_is_array|Checks whether a JavaScript value is an array.|10|
|FUNC|napi_is_arraybuffer|Checks whether a JavaScript value is a **ArrayBuffer**.|10|
|FUNC|napi_is_typedarray|Checks whether a JavaScript value is a **TypedArray**.|10|
|FUNC|napi_is_dataview|Checks whether a JavaScript value is a **DataView**.|10|
|FUNC|napi_is_date|Checks whether a JavaScript value is a JavaScript **Date** object.|10|
|FUNC|napi_strict_equals|Checks whether two JavaScript values are strictly equal.|10|
...
|FUNC|napi_check_object_type_tag|Checks whether a tag pointer is associated with a JavaScript object.|10|
|FUNC|napi_call_function|Calls a JavaScript function object in a native method, that is, native calls JavaScript.|10|
|FUNC|napi_create_function|Creates a function object in native code for JavaScript to call.|10|
...
|FUNC|napi_define_class|Defines a JavaScript class corresponding to the C++ class.|10|
|FUNC|napi_wrap|Wraps a native instance in a JavaScript object.|10|
|FUNC|napi_unwrap|Obtains the native instance that was previously wrapped in a JavaScript object.|10|
|FUNC|napi_remove_wrap|Obtains the native instance that was previously wrapped in a JavaScript object and removes the wrapping.|10|
...
|FUNC|napi_make_callback|Allows a JavaScript function to be called in the asynchronous context. The capabilities related to **async_hook** are not supported currently.|11|
...
|FUNC|napi_create_promise|Creates a deferred object and a JavaScript promise.|10|
|FUNC|napi_resolve_deferred|Resolves a deferred object that is associated with a JavaScript promise.|10|
|FUNC|napi_reject_deferred|Rejects a deferred object that is associated with a JavaScript promise.|10|
|FUNC|napi_is_promise|Checks whether the given JavaScript value is a promise object.|10|
...
|FUNC|napi_create_date|Creates a JavaScript **Date** object from C double data.|10|
|FUNC|napi_get_date_value|Obtains the C double equivalent of the given JavaScript **Date** object.|10|
|FUNC|napi_create_bigint_int64|Creates a JavaScript BigInt from C int64 data.|10|
|FUNC|napi_create_bigint_uint64|Creates a JavaScript BigInt from C uint64 data.|10|
|FUNC|napi_create_bigint_words|Creates a single JavaScript BigInt from a C uint64 array.|10|
|FUNC|napi_get_value_bigint_int64|Obtains the C int64 equivalent of the given JavaScript BigInt.|10|
|FUNC|napi_get_value_bigint_uint64|Obtains the C uint64 equivalent of the given JavaScript BigInt.|10|
|FUNC|napi_get_value_bigint_words|Obtains information from the given JavaScript BigInt, including the sign bit, 64-bit little-endian array, and number of elements in the array.|10|
|FUNC|napi_create_buffer|Creates a JavaScript **Buffer** instance of the specified size.|10|
|FUNC|napi_create_buffer_copy|Creates a JavaScript **Buffer** instance of the specified size, and initializes it with data copied from the passed-in buffer.|10|
|FUNC|napi_create_external_buffer|Creates a JavaScript **Buffer** instance of the specified size, and initializes it with the given data. The **Buffer** instance created can include extra.|10|
|FUNC|napi_get_buffer_info|Obtains the underlying data of **Buffer** and its length.|10|
|FUNC|napi_is_buffer|Checks whether the given JavaScript value is a **Buffer** object.|10|
...
|FUNC|napi_add_finalizer|Adds a **napi_finalize** callback, which will be called when the JavaScript object in **js_Object** is garbage-collected.|11|

## Symbols Not Exported from the Node-API Library

|Symbol Type|Symbol|Description|
| --- | --- | --- |
|FUNC|napi_run_script|Runs an object as JavaScript code.|
|FUNC|napi_adjust_external_memory|Adjusts the external memory held by a JavaScript object.|
|FUNC|napi_fatal_exception|Throws **UncaughtException** to JavaScript.|
...
|FUNC|napi_create_object_with_properties|Creates a JavaScript object using the given **napi_property_descriptor**. The key of the descriptor must be a string and cannot be converted into a number.|11|
|FUNC|napi_create_object_with_named_properties|Creates a JavaScript object using the given **napi_value** and key. The key must be a string and cannot be converted into a number.|11|
|FUNC|napi_coerce_to_native_binding_object|Forcibly binds a JavaScript object and a native object.|11|
