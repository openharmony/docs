...
- Supports subscription to ranging state change events, enabling listening for status changes such as ranging start and stop in real time.
...
- If the query is successful, a promise is used to return whether the ranging type is supported. You can call [startRanging](#rangingstartranging) to start NearLink [HADM](../../connectivity/fusion-connectivity/fusion-connectivity-terminology.md#hadm) ranging or call [startPassiveRanging](#rangingstartpassiveranging) to start passive ranging only when the value of [nearlinkHadm](#rangingcapabilitysupported) is **true**.
...
Initiates active ranging to a specified device to obtain information such as the distance and signal strength of the target device. Typical application scenarios include intelligent anti-loss and finding, near-field object finding, and digital keys.

The execution process of this API depends on the NearLink connection status between the local device and the target device.
...
- If no NearLink connection has been established between the local device and the target device, this API performs the following operations:
  1. The integrated ranging service first attempts to establish a connection with the target device. After the connection is successfully established, pairing and encryption are performed. During pairing, the user needs to actively perform authorization on the device. If the user rejects the authorization or the authorization times out, the ranging will be stopped. The stop status will be notified through the callback registered using the [onRangingStateChange](#rangingonrangingstatechange) API. After the ranging is stopped, the app needs to proactively call [stopRanging](#rangingstopranging) to release the ranging resources.
...
After ranging is successfully started, the result will be frequently reported through callback. You are advised to call stopRanging to stop ranging in a timely manner after obtaining the ranging result based on the actual requirements. You can initiate ranging again when necessary to avoid unnecessary power consumption.
...
> - If [startRanging](#rangingstartranging) is repeatedly called for the same device, a message will be displayed indicating that the device has started ranging, and error code 34900051 will be returned. To initiate ranging again for the same device, call [stopRanging](#rangingstopranging) to stop the previous ranging first.
...
> **NOTE**
>
> - This method also releases the resources occupied by ranging. To ensure proper resource management, [stopRanging](#rangingstopranging) must be called to stop ranging after [startRanging](#rangingstartranging) is called to start ranging, preventing ranging resource leak.
...
Starts the passive ranging mode. The local device broadcasts ranging data packets as a ranging beacon, allowing other active ranging devices that support the corresponding ranging type to discover the local device. Typical application scenarios include: The local device functions as a tag to be located, an anti-loss sticker, or a fixed beacon. This mode is applicable when the local device needs to be measured by other devices.
...
| Name        | Type                        | Mandatory| Description        |
| ------------ | -------------------------- | ---- | ---------- |
| handle       | number                     | Yes  | Ranging monitoring handle, which must be a valid handle returned by [startPassiveRanging](#rangingstartpassiveranging). Otherwise, error code [34900054](errorcode-fusionConnectivity.md#34900054-invalid-parameters) is thrown. After the handle is stopped, it becomes invalid and cannot be reused.  |
| capabilityType | [RangingTypes](#rangingtypes) | Yes  | Ranging capability type. The value must be the same as that of capabilityType passed by the [startPassiveRanging](#rangingstartpassiveranging) API.  |
...
Describes the ranging state change information. This struct is shared by the status changes of active and passive ranging.
...
| handle | number           | No  | Yes  | Ranging monitoring handle, which identifies the passive ranging session whose status has changed in passive ranging scenarios. This field is not applicable in active ranging scenarios. The default value is **undefined**. |
...
| angle   | [RangingMeasurement](#rangingmeasurement) | No  | No  | Azimuth output by the ranging device, in degrees. The value range is [0, 360). If **-1** is returned, azimuth measurement is not supported.  |
...
| confidence | [RangingConfidence](#rangingconfidence) | No  | No  | Confidence of the measurement result, indicating the reliability of the measurement value.  |
