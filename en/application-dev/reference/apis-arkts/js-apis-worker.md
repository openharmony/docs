...
Adds an event listener for the Worker instance. This API provides the same functionality as [addEventListener<sup>9+</sup>](#addeventlistener9).
...
Removes the event listener of the Worker instance. This API provides the same functionality as [removeEventListener<sup>9+</sup>](#removeeventlistener9).
...
Called when the Worker thread exits. The event handler is executed in the host thread. The **code** value is of the number type, where the value **1** indicates abnormal exit and **0** indicates normal exit.
...
Called when an exception occurs within the lifecycle of the Worker thread. The event handler is executed in the host thread.<br/>
...
**onAllErrors** can capture global exceptions generated during the **onmessage** callback, timer callback, and file execution of the Worker thread. After an exception is captured by **onAllErrors**, the Worker thread remains alive and can continue to be used.
...
Called when the host thread receives a message sent by the Worker thread through the **workerPort.postMessage** API. The handler is executed in the host thread. In the callback function, the **event** type is [MessageEvents](#messageevents9), indicating the received message data.
...
Called when the worker receives a message that cannot be serialized. The event handler is executed in the host thread. In the callback function, the **event** type is [MessageEvents](#messageevents9), indicating the received message data.
...
Adds an event listener for the Worker instance. This API provides the same functionality as [on<sup>9+</sup>](#on9).
...
Removes all event listeners of the Worker instance.
...
Called when the worker thread exits. The handler is executed in the host thread. In the callback function, the **code** value is of the number type, where the value **1** indicates abnormal exit and **0** indicates normal exit.
