...
| OH_NN_OPS_RSQRT | Calculates the reciprocal of the square root of an input tensor.<br>Input:<br>- **input**: *n*-dimensional tensor, where *n* is less than 8. Each element of the tensor cannot be less than **0**.<br>Output:<br>- **output**: *n*-dimensional tensor, with the same shape and data type as **input**. | 
| OH_NN_OPS_RESHAPE | Reshapes an input tensor.<br>Input:<br>- **input**: *n*-dimensional input tensor.<br>- **inputShape**: shape of the output tensor. The value is a 1D constant tensor.<br>Output:<br>- **output**: tensor whose data type is the same as that of **input** and shape is determined by **InputShape**.| 
