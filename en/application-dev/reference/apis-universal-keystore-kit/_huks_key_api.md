...
| paramSet | Pointer to the parameters for importing the wrapped key. | 
| wrappedKeyData | Pointer to the wrapped key to import. The data must be of the [OH_Huks_AlgSuite](_huks_type_api.md#oh_huks_algsuite) type.| 
...
Returns [OH_Huks_ErrCode#OH_HUKS_ERR_CODE_ITEM_NOT_EXIST](_huks_type_api.md#oh_huks_errcode) if the key does not exist. <br>Returns other errors in other cases.
