...
| sharedSecret | Uint8Array                  | No  | Yes  | Indicates the shared key generated by key encapsulation or decapsulation. This parameter is left empty by default.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.|
...
| options  | [HuksOptions](#huksoptions) | Yes  | Tags required for generating the key. The algorithm, key purpose, and key length are mandatory.<!--RP8--><!--RP8End--> |
...
| ID| Error Message     |
| -------- | ------------- |
| 201 | The application permissions are insufficient, possibly because the ohos.permission.ACCESS_SE_KEY permission is missing.<br>Applicable versions: 26.0.0+|
...
| 12000017 | The key with the same alias already exists.<br>Applicable versions: 20+|
| 12000018 | the group id specified by the access group tag is invalid.<br>Applicable versions: 23+|
| 12000011 | The queried entity does not exist. This may happen because the key resource ID specified by keyAlias has not been opened in the external crypto scenario.<br>Applicable versions: 26.0.0+|
| 12000020 | the provider operation failed.<br>Applicable versions: 26.0.0+|
| 12000021 | the UKey PIN is locked.<br>Applicable versions: 26.0.0+|
| 12000023 | the UKey PIN not authenticated.<br>Applicable versions: 26.0.0+|
| 12000024 | the provider or UKey is busy.<br>Applicable versions: 26.0.0+|
| 12000026 | the secure element is not available.<br>Applicable versions: 26.0.0+|
...
```ts
/* The following uses ECC key generation as an example. */
import { huks } from '@kit.UniversalKeystoreKit';
...
```ts
/*The following uses the generation of an ECC key as an example. */
import { huks } from '@kit.UniversalKeystoreKit';
...
| 12000005 | IPC communication failed. |
| 12000006 | error occurred in crypto engine. |
| 12000011 | queried entity does not exist.<br>Applicable versions: 9-19|
| 12000012 | Device environment or input parameter abnormal. |
| 12000013 | queried credential does not exist. |
| 12000014 | memory is insufficient. |
| 12000015 | Failed to obtain the security information via UserIAM. |
...
```ts
/*The following uses the AES key as an example.*/
import { huks } from '@kit.UniversalKeystoreKit';
...
```ts
/*The following example describes how to import an AES key.*/
import { huks } from '@kit.UniversalKeystoreKit';
...
```ts
/*The following uses obtaining an RSA key certificate as an example.*/
import { huks } from '@kit.UniversalKeystoreKit';
...
async function generateKeyThenAttestKey() {
  ...
  /* 1. Configure key generation parameters. */
  ...
  /* 2. Configure key attestation parameters. */
  ...
  /* 3. Generate a key and obtain the key attestation. */
...
```ts
/*The following uses the RSA key certificate as an example. */
import { huks } from '@kit.UniversalKeystoreKit';
...
/* 2. Obtain the key certificate. */
async function attestKey() {
  let aliasString = keyAliasString;
  let aliasUint8 = stringToUint8Array(aliasString);
  /* Configure key attestation parameters. */
  let properties: Array<huks.HuksParam> = [
    {
      tag: huks.HuksTag.HUKS_TAG_ATTESTATION_ID_SEC_LEVEL_INFO,
      value: securityLevel
...
```ts
/* The following uses the RSA anonymization key certificate as an example. */
import { huks } from '@kit.UniversalKeystoreKit';
...
async function generateKeyThenAttestKey(): Promise<void> {
  ...
  /* 2. Configure anonymous attestation parameters. */
  ...
  /* 3. Generate a key and obtain the anonymous key attestation. */
  huks.generateKeyItem(aliasString, generateOptions, (error) => {
    if (error) {
      console.error(`callback: generateKeyItem failed`);
    } else {
...
| 801 | api is not supported. |
| 12000001 | algorithm mode is not supported. |
| 12000002 | algorithm param is missing.<br>Applicable versions: 11+|
| 12000003 | algorithm param is invalid.<br>Applicable versions: 11+|
| 12000004 | operating file failed. |
| 12000005 | IPC communication failed. |
| 12000006 | error occurred in crypto engine. |
| 12000011 | queried entity does not exist. |
...
/* 2. Obtain the anonymization key certificate. */
...
```ts
/*The following example describes how to obtain the ECC anonymous key certificate offline.*/
import { huks } from '@kit.UniversalKeystoreKit';
...
/* 2. Obtain the anonymization key certificate offline. */
...
/* Parameter set for generating an ECC key */
...
/*Parameter set for importing a secure key*/
...
| 801 | api is not supported. |
| 12000001 | algorithm mode is not supported. |
| 12000002 | algorithm param is missing.<br>Applicable versions: 9–11|
| 12000003 | algorithm param is invalid.<br>Applicable versions: 9–11|
| 12000004 | operating file failed. |
| 12000005 | IPC communication failed. |
| 12000006 | error occurred in crypto engine. |
| 12000011 | queried entity does not exist. |
...
Encrypts a key and generates a ciphertext and shared key using the ML-KEM public key. This API uses a promise to return the result.
...
| Name  | Type                       | Mandatory| Description                                        |
| -------- | --------------------------- | ---- | -------------------------------------------- |
| keyAlias | string                      | Yes  | Alias of the ML-KEM public/private key.|
| params  | [HuksParam[]](#huksparam) | Yes  | Key encapsulation operation parameter set.|
| sharedKeyAlias | string | No  | Alias of the shared key storage.|
| sharedKeyParams | [HuksParam[]](#huksparam) | No  | Attribute parameter set of the shared key.|

**Return value**

| Type                                          | Description                                                        |
| ---------------------------------------------- | ------------------------------------------------------------ |
| Promise<[HuksReturnResult](#huksreturnresult9)> | Promise that returns the operation result. outData is the encapsulated ciphertext data, and sharedSecret is the shared key. (If sharedKeyAlias is not empty, sharedSecret is empty.)|
...
Decrypts the key and restores the shared key from the ciphertext using the ML-KEM private key. This API uses a promise to return the result.
...
| Name  | Type                       | Mandatory| Description                                        |
| -------- | --------------------------- | ---- | -------------------------------------------- |
| keyAlias | string                      | Yes  | Alias of the ML-KEM private key.|
| params  | [HuksParam[]](#huksparam) | Yes  | Parameter set for key decapsulation.|
| encapData | Uint8Array | Yes  | Encapsulated ciphertext data.|
| sharedKeyAlias | string | No  | Alias for storing the shared key.|
| sharedKeyParams | [HuksParam[]](#huksparam) | No  | Attribute parameter set of the shared key.|
...
| Type                                          | Description                                                        |
| ---------------------------------------------- | ------------------------------------------------------------ |
| Promise<[HuksReturnResult](#huksreturnresult9)> | Promise that returns the operation result. The sharedSecret is the shared key. (If sharedKeyAlias is not empty, sharedSecret is empty.)|
...
| 12000012 | Device environment or input parameter abnormal. |
| 12000014 | memory is insufficient. |
| 12000018 | the input parameter is invalid. Possible causes: 1. the aead length is invalid. 2. the group id specified by the access group tag is invalid.<br>Applicable versions: 22+|
| 12000020 | the provider operation failed.<br>Applicable versions: 22+|
| 12000021 | the UKey PIN is locked.<br>Applicable versions: 22+|
| 12000023 | the UKey PIN not authenticated.<br>Applicable versions: 22+|
| 12000024 | the provider or UKey is busy.<br>Applicable versions: 22+|
| 12000026 | the secure element is not available.<br>Applicable versions: 26.0.0+|
 ...
 *
 * The following uses the callback function of the RSA key as an example.
 */
    ...
    if (error) {
      ...
      huks.initSession(keyAlias, options, (error, data) => { // Use abortSession to abort initSession.
        if (error) {
          ...
          handle = data.handle;
          /* 3. If an error occurs, terminate the key operation. */
...
function testThreeStageEncrypt() {
    ...
    /* 2. Initialize the encryption session. */
...
function testThreeStageDecrypt() {
    ...
    /*5. Terminate the decryption session.*/
    huks.initSession(keyAlias, initOptions, (err, data) => {
        if (err) {
 ...
 * If an error occurs in any of the three operations, call huks.abortSession to abort the key operation.
 *
 * The following uses the promise function of the RSA key as an example.
 */
let keyAlias = "HuksDemoRSA";
...
```ts
/* The following is an example of querying the alias set of DE keys. */
import { huks } from '@kit.UniversalKeystoreKit';
...
| HUKS_ERR_CODE_SE_FAULT  | 12000026 | The secure element is faulty.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.<br> **System capability**: SystemCapability.Security.Huks.Core   |
...
| HUKS_ML_DSA_KEY_PARAM_SET_44          | 44  | ML-DSA security parameter set of 44.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.<br> **System capability**: SystemCapability.Security.Huks.Core|
| HUKS_ML_DSA_KEY_PARAM_SET_65          | 65  | ML-DSA security parameter set of 65.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.<br> **System capability**: SystemCapability.Security.Huks.Core|
| HUKS_ML_DSA_KEY_PARAM_SET_87          | 87  | ML-DSA security parameter set of 87.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.<br> **System capability**: SystemCapability.Security.Huks.Core|
| HUKS_ML_KEM_KEY_PARAM_SET_768  | 768  | Indicates that the key length of the ML-KEM algorithm is 768 bits.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.<br> **System capability**: SystemCapability.Security.Huks.Core|
| HUKS_ML_KEM_KEY_PARAM_SET_1024  | 1024  | Indicates that the key length of the ML-KEM algorithm is 1024 bits.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.<br> **System capability**: SystemCapability.Security.Huks.Core|
...
| HUKS_ALG_ML_DSA           | 201    | ML-DSA.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.<br> **System capability**: SystemCapability.Security.Huks.Core|
| HUKS_ALG_ML_KEM | 200  | Indicates that the ML-KEM algorithm is used.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.<br> **System capability**: SystemCapability.Security.Huks.Core|
...
| HUKS_KEY_SECURITY_LEVEL_SE | 1    | The key is generated and used in the secure environment.<br>**Required permissions**: [ohos.permission.ACCESS_SE_KEY](../../security/AccessToken/restricted-permissions.md#ohospermissionaccess_se_key)|
...
| HUKS_TAG_KEY_SECURITY_LEVEL                                  | HuksTagType.HUKS_TAG_TYPE_UINT \| 526   | Security level of the key.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.<br> **System capability**: SystemCapability.Security.Huks.Core|
| HUKS_TAG_AAD<sup>24+</sup>                                  | HuksTagType.HUKS_TAG_TYPE_BYTES \| 527   | Additional verification data indicating the GCM or CCM mode.<br>**Atomic service API**: This API can be used in atomic services since API version 24.<br> **System capability**: SystemCapability.Security.Huks.Core|
| HUKS_TAG_CONTEXT                                           | HuksTagType.HUKS_TAG_TYPE_BYTES \| 528   | **context** of ML-DSA signature verification.<br>**Since**: 26.0.0<br>**Atomic service API**: This API can be used in atomic services since API version 26.0.0.<br> **System capability**: SystemCapability.Security.Huks.Core|
