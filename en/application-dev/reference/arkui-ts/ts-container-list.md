...
| onScroll(event: (scrollOffset: number, scrollState: [ScrollState](#scrollstate)) => void) | Triggered when the list scrolls.<br>- **scrollOffset**: scroll offset of each frame. The offset is positive when the list is scrolled up and negative when the list is scrolled down.<br>- **scrollState**: current scroll state.<br>Since API version 9, this API is supported in ArkTS widgets.|
