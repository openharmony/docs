...
| onSubmit(callback: (enterKey: EnterKeyType, event: SubmitEvent) =&gt; void) | Triggered when the Enter key on the keyboard is pressed. The return value is the current type of the Enter key and whether the current text is in the editing state.<br>**enterKeyType**: type of the Enter key. For details, see [EnterKeyType](#enterkeytype).<br>**event**: event.text is the input text content. Call event.keepEditableState() to change the current state to the editing state.  |
