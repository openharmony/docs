...
Function overview: The standard JS engine is a JavaScript code execution engine that strictly complies with the Ecmascript specifications. Supports the standard library defined in the Ecmascript specification and provides complete native APIs of C++ interaction JS. Use jit to accelerate code execution and provide secure and efficient JS execution capabilities for applications. The capabilities of the standard JS engine are provided through a stable set of ABI, that is, JSVM-API. JSVM-APIs can be dynamically linked to JS engine libraries of different versions to shield API differences between engines for developers. JSVM-API provides capabilities such as engine lifecycle management, JS context management, JS code execution, JS/C++ interoperability, execution environment snapshot, and codecache. Platform: ARM64. Usage: Link libjsvm.so in the SDK and include the ark_runtime/jsvm.h header file in the C++ code.

Provides developers with independent, standard, and complete JavaScript engine capabilities through APIs, including managing the engine lifecycle, compiling and running JS code, implementing JS/C++ cross-language invoking, and taking snapshots.
...
| Name| Description| 
| -------- | -------- |
| struct&nbsp;&nbsp;[JSVM_CallbackStruct](_j_s_v_m___callback_struct.md) | Pointers and data of native callback functions provided by users. These functions are exposed to JavaScript through JSVM-API APIs. | 
| struct&nbsp;&nbsp;[JSVM_HeapStatistics](_j_s_v_m___heap_statistics.md) | Heapstatisics structure, which is used to store statistics about JavaScript heap memory usage. | 
...
| struct&nbsp;&nbsp;[JSVM_PropertyHandlerConfigurationStruct](_j_s_v_m___property_handler_configuration_struct.md) | When the getter, setter, deleter, or enumerator operation is performed on an object, the corresponding function callback in the structure is triggered. | 
...
| typedef struct JSVM_CpuProfiler__ \* [JSVM_CpuProfiler](#jsvm_cpuprofiler) | Represents a JavaScript CPU time performance analyzer. | 
...
| typedef [JSVM_CallbackStruct](_j_s_v_m___callback_struct.md) \* [JSVM_Callback](#jsvm_callback) | Function pointer type of native functions provided by users. These functions are exposed to JavaScript through JSVM-API APIs. | 
...
| typedef bool(JSVM_CDECL \* [JSVM_OutputStream](#jsvm_outputstream)) (const char \*data, int size, void \*streamData) | Pointer to the callback function of the ASCII output stream. The data parameter indicates the output data pointer. The size parameter indicates the size of the output data. The null data pointer indicates the end of the stream. The streamData parameter refers to the pointer passed to the API function together with the callback. The API function generates data to the output stream. If true is returned, the stream can continue to receive data. Otherwise, it aborts the stream. | 
| typedef [JSVM_PropertyHandlerConfigurationStruct](_j_s_v_m___property_handler_configuration_struct.md) \* [JSVM_PropertyHandlerCfg](#jsvm_propertyhandlercfg) | Pointer to the structure that contains the property listening callback. | 
...
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_GetVM](#oh_jsvm_getvm) ([JSVM_Env](#jsvm_env) env, [JSVM_VM](#jsvm_vm) \*result) | Obtains a VM instance. | 
...
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_CompileScriptWithOrigin](#oh_jsvm_compilescriptwithorigin) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) script, const uint8_t \*cachedData, size_t cacheDataLength, bool eagerCompile, bool \*cacheRejected, [JSVM_ScriptOrigin](_j_s_v_m___script_origin.md) \*origin, [JSVM_Script](#jsvm_script) \*result) | Compiles a string of JavaScript code that contains sourcemap information and returns the compiled script. | 
...
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_Equals](#oh_jsvm_equals) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) lhs, [JSVM_Value](#jsvm_value) rhs, bool \*result) | Provides behavior similar to invoking a loose equality algorithm. Regardless of the JavaScript value type, true is returned as long as the values are equal. | 
...
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_GetHeapStatistics](#oh_jsvm_getheapstatistics) ([JSVM_VM](#jsvm_vm) vm, [JSVM_HeapStatistics](_j_s_v_m___heap_statistics.md) \*result) | Obtains heap statistics of a VM. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_StartCpuProfiler](#oh_jsvm_startcpuprofiler) ([JSVM_VM](#jsvm_vm) vm, [JSVM_CpuProfiler](#jsvm_cpuprofiler) \*result) | Creates and starts a CPU profiler instance. | 
...
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_TakeHeapSnapshot](#oh_jsvm_takeheapsnapshot) ([JSVM_VM](#jsvm_vm) vm, [JSVM_OutputStream](#jsvm_outputstream) stream, void \*streamData) | Obtains a snapshot of the current heap and outputs it to a stream. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_OpenInspector](#oh_jsvm_openinspector) ([JSVM_Env](#jsvm_env) env, const char \*host, uint16_t port) | Opens an inspector instance on the specified host and port for debugging JS code. | 
...
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_WaitForDebugger](#oh_jsvm_waitfordebugger) ([JSVM_Env](#jsvm_env) env, bool breakNextLine) | Waits for the host to set up a socket connection with an inspector. After the connection is set up, the application continues to run. You can use **Runtime.runIfWaitingForDebugger** to run paused targets. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_DefineClassWithPropertyHandler](#oh_jsvm_defineclasswithpropertyhandler) ([JSVM_Env](#jsvm_env) env, const char \*utf8name, size_t length, [JSVM_Callback](#jsvm_callback) constructor, size_t propertyCount, const [JSVM_PropertyDescriptor](_j_s_v_m___property_descriptor.md) \*properties, [JSVM_PropertyHandlerCfg](#jsvm_propertyhandlercfg) propertyHandlerCfg, [JSVM_Callback](#jsvm_callback) callAsFunctionCallback, [JSVM_Value](#jsvm_value) \*result) | Defines a JavaScript class attribute operation with a given class name, constructor, attribute, and callback handler, including getter, setter, deleter, and enumerator, and calls the operation as a function callback. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_IsUndefined](#oh_jsvm_isundefined) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) value, bool \*isUndefined) | Checks whether the value passed in is **Undefined**. This API is equivalent to executing JS code **value === undefined**. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_IsNull](#oh_jsvm_isnull) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) value, bool \*isNull) | Checks whether the value passed in is a **Null** object. This API is equivalent to executing JS code **value === null**. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_IsNullOrUndefined](#oh_jsvm_isnullorundefined) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) value, bool \*isNullOrUndefined) | Checks whether the value passed in is **Null** or **Undefined**. This API is equivalent to executing JS code **value == null**. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_IsBoolean](#oh_jsvm_isboolean) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) value, bool \*isBoolean) | Checks whether the value passed in is a Boolean value. This is equivalent to'typeof value ==='boolean'` in JS. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_IsNumber](#oh_jsvm_isnumber) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) value, bool \*isNumber) | Checks whether the value passed in is a number. This is equivalent to'typeof value ==='number'` in JS. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_IsString](#oh_jsvm_isstring) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) value, bool \*isString) | Checks whether the value passed in is a string. This is equivalent to'typeof value ==='string'` in JS. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_IsSymbol](#oh_jsvm_issymbol) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) value, bool \*isSymbol) | Checks whether the value passed in is a symbol. This is equivalent to'typeof value ==='symbol'` in JS. | 
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_IsFunction](#oh_jsvm_isfunction) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) value, bool \*isFunction) | Checks whether the value passed in is a function. This is equivalent to'typeof value ==='function'` in JS. | 
...
| JSVM_EXTERN [JSVM_Status](#jsvm_status)[OH_JSVM_IsBigInt](#oh_jsvm_isbigint) ([JSVM_Env](#jsvm_env) env, [JSVM_Value](#jsvm_value) value, bool \*isBigInt) | Checks whether the value passed in is a BigInt. This is equivalent to'typeof value ==='bigint'` in JS. | 
...
```
**Description**
Function pointer type of native functions provided by users. These functions are exposed to JavaScript through JSVM-API APIs.
...
```
**Description**
Represents a JavaScript CPU time performance analyzer.
...
```
**Description**
Pointer to the callback function of the ASCII output stream. The data parameter indicates the output data pointer. The size parameter indicates the size of the output data. The null data pointer indicates the end of the stream. The streamData parameter refers to the pointer passed to the API function together with the callback. The API function generates data to the output stream. If true is returned, the stream can continue to receive data. Otherwise, it aborts the stream.
...
| JSVM_NUMBER  | The value is a number.&nbsp;&nbsp; | 
...
The execution status code JSVM_Status is returned. JSVM_OK indicates that the execution is successful.

JSVM_INVALID_ARG indicates that the input parameter is invalid.
...
JSVM_PENDING_EXCEPTION indicates that a JS exception occurs during execution.

JSVM_GENERIC_FAILURE indicates that the execution fails due to unknown reasons.
...
| Name| Description| 
| -------- | -------- |
| env | target environment where the JSVM-API API will be called. | 
...
JSVM_HANDLE_SCOPE_MISMATCH indicates that the execution fails.
...
```
**Description**
Attempt to close all remaining inspector connections.
...
| cachedData | This parameter is optional. Code cache data of the script. | 
...
JSVM_STRING_EXPECTED indicates that the input parameter is not of the string type.
...
```
**Description**
Compiles a string of JavaScript code that contains sourcemap information and returns the compiled script.
...
| eagerCompile | Indicates whether to compile the script immediately. | 
| cacheRejected | Indicates whether the code cache is rejected by compilation. | 
| origin | Source code information, including the location of the source map and source code file name. | 
| result | Compiled script. | 
...
JSVM_GENERIC_FAILURE indicates that the execution fails.
...
```
**Description**
Defines a JavaScript class attribute operation with a given class name, constructor, attribute, and callback handler, including getter, setter, deleter, and enumerator, and calls the operation as a function callback.
...
| length | Length of utf8name (in bytes) or JSVM_AUTO_LENGTH (if ending with null). | 
| constructor | Callback function used to create the constructor of a class. This method must be of the JSVM_Callback type. The callback in the constructor must be a static member. C++ class constructors cannot be used. For details, see JSVM_Callback. | 
| propertyCount | Number of items in the properties array parameter. | 
| properties | For details about the properties, accessors, and methods on the property descriptor array class that describes static data and instance data, see JSVM_PropertyDescriptor. | 
| propertyHandlerCfg | Accessing the instance object attribute triggers the corresponding callback function. | 
| callAsFunctionCallback | This callback is triggered when an instance object is called as a function. | 
| result | JSVM_Value representing the constructor of the JavaScript class. | 
...
The execution status code JSVM_Status is returned. If JSVM_OK is displayed, the execution is successful.

JSVM_DETACHABLE_ARRAYBUFFER_EXPECTED indicates that the input parameter is not an analyzable ArrayBuffer.
...
```
**Description**
Provides behavior similar to invoking a loose equality algorithm. Regardless of the JavaScript value type, true is returned as long as the values are equal.
...
JSVM_ARRAY_EXPECTED indicates that the input parameter is not of the Array type.
...
JSVM_DATE_EXPECTED indicates that the input parameter is not of the Date type.
...
```
**Description**
Returns statistics for a set of virtual machine heaps.
...
| Name| Description| 
| -------- | -------- |
| vm | Indicates the VM that returns heap statistics. | 
...
JSVM_BIGINT_EXPECTED indicates that the input parameter is not of the BitInt type.
...
JSVM_BOOLEAN_EXPECTED indicates that the input parameter is not of the Boolean type.
...
JSVM_NUMBER_EXPECTED indicates that the input parameter is not of the number type.
...
JSVM_INVALID_ARG indicates that the input parameter is not an external JSVM_Value.
...
```
**Description**
The virtual machine instance for the given environment is retrieved.
...
| -------- | -------- |
| env | target environment where the JSVM-API API will be called. | 
| result | VM instance in a given environment. | 
...
JSVM_FUNCTION_EXPECTED indicates that the input parameter is not of the Function type.
...
```
**Description**
Checks whether the value passed in is a BigInt. This is equivalent to'typeof value ==='bigint'` in JS.
...
| isBigInt | Indicates whether the given JSVM_Value is BigInt. | 

**Returns**

The execution status code JSVM_Status is returned. JSVM_OK indicates that the execution is successful. This API does not trigger any exception.
...
```
**Description**
Checks whether the value passed in is a Boolean value. This is equivalent to'typeof value ==='boolean'` in JS.
...
| env | Environment for invoking the JSVM-API. | 
| value | JSVM_Value to be checked. | 
| isBoolean | Indicates whether the given JSVM_Value is Boolean. | 
...
```
**Description**
Checks whether the value passed in is a function. This is equivalent to'typeof value ==='function'` in JS.
...
| env | Environment for invoking the JSVM-API. | 
| value | JSVM_Value to be checked. | 
| isFunction | Indicates whether the given JSVM_Value is a function. | 
...
```
**Description**
Checks whether the value passed in is a **Null** object. This is equivalent to value === null in JS.
...
| env | Environment for invoking the JSVM-API. | 
| value | JSVM_Value to be checked. | 
| isNull | Indicates whether the given JSVM_Value is null. | 
...
```
**Description**
Checks whether the value passed in is **Null** or **Undefined**. This is equivalent to value == null in JS.
...
| env | Environment for invoking the JSVM-API. | 
| value | JSVM_Value to be checked. | 
| isNullOrUndefined | Indicates whether the given JSVM_Value is Null or Undefined. | 
...
```
**Description**
Checks whether the value passed in is a number. This is equivalent to'typeof value ==='number'` in JS.
...
| env | Environment for invoking the JSVM-API. | 
| value | JSVM_Value to be checked. | 
| isNumber | Indicates whether the given JSVM_Value is a number. | 
...
```
**Description**
This API checks whether the input value is Object.
...
| env | Environment for invoking the JSVM-API. | 
| value | JSVM_Value to be checked. | 
| isObject | Indicates whether the given JSVM_Value is an Object. | 
...
```
**Description**
Checks whether the value passed in is a string. This is equivalent to'typeof value ==='string'` in JS.
...
| env | Environment for invoking the JSVM-API. | 
| value | JSVM_Value to be checked. | 
| isString | Indicates whether the given JSVM_Value is a String. | 
...
```
**Description**
Checks whether the value passed in is a symbol. This is equivalent to'typeof value ==='symbol'` in JS.
...
| env | Environment for invoking the JSVM-API. | 
| value | JSVM_Value to be checked. | 
| isSymbol | Indicates whether the given JSVM_Value is Symbol. | 
...
```
**Description**
Checks whether the value passed in is **Undefined**. This is equivalent to value === undefined in JS.
...
| env | Environment for invoking the JSVM-API. | 
| value | JSVM_Value to be checked. | 
| isUndefined | Indicates whether the given JSVM_Value is Undefined. | 
...
```
**Description**
Activate the inspector on the specified host and port to debug the JS code.
...
| -------- | -------- |
| env | Environment for invoking the JSVM-API. | 
| host | IP address of the host to which the inspector is connected. | 
| port | Port number for listening on the inspector connection. | 
...
```
**Description**
Create and start a CPU profiler.
...
| Name| Description| 
| -------- | -------- |
| vm | Start the VM of the CPU profiler. | 
...
```
**Description**
Stops the CPU profiler and outputs the result to the stream.
...
| -------- | -------- |
| vm | Start the VM of the CPU profiler. | 
| profiler | Indicates the CPU profiler to be stopped. | 
| stream | Callback of the output stream for receiving data. | 
| streamData | Optional data passed to the output stream callback. For example, it can be a file stream used to write the sampled data transferred in the output stream callback to a file. | 
...
| Name| Description| 
| -------- | -------- |
| vm | Virtual machine whose heap snapshot will be obtained. | 
| stream | Callback of the output stream for receiving data. | 
| streamData | Optional data passed to the output stream callback. For example, it can be a file stream used to write the sampled data transferred in the output stream callback to a file. | 
...
```
**Description**
Waits for the host to set up a socket connection with an inspector. After the connection is set up, the application continues to run. Sends the Runtime.runIfWaitingForDebugger command.
...
| -------- | -------- |
| env | Environment for invoking the JSVM-API. | 
| breakNextLine | Indicates whether to interrupt in the next line of JavaScript code. If this parameter is set to Yes, the running of the next line of JS code will be suspended. To continue running the JS code, you need to use the debugging button of the debugger to control the execution of the JS code. | 
