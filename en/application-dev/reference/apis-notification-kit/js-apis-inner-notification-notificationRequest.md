...
| updateOnly<sup>16+</sup>        | boolean                                       | No | Yes | Whether to update notifications only. The default value is **false**.<br>When the specified value is **true**, the notification is updated if the notification with the same ID exists; otherwise, the notification update fails and no notification is created.<br>When the specified value is **false**, the notification is updated if the notification with the same ID exists; otherwise, a notification is created.        |
