...
class EntryAbility extends UIAbility {
  onWindowStageCreate(windowStage: window.WindowStage){
    ...
    // Before calling this API, make sure the required permission has been obtained.
...
calendarMgr?.getCalendar((err: BusinessError, data:calendarManager.Calendar) => {
  if (err) {
    ...
    calendar.addEvents(events).then(() => {
      ...
      console.error(`Failed to add the event, err -> ${JSON.stringify(err)}`);
    });
  }
});
```
...
calendarMgr?.getCalendar(async (err: BusinessError, data:calendarManager.Calendar) => {
  if (err) {
    ...
    calendar.deleteEvent(id, (err: BusinessError) => {
      if (err) {
        console.error(`Failed to delete the event, err -> ${JSON.stringify(err)}`);
      } else {
        console.info("Succeeded in deleting the event");
      }
    });
  }
});
  ...
  if (err) {
    ...
    calendar.deleteEvents([id1, id2], (err: BusinessError) => {
      if (err) {
        ...
        console.info("Succeeded in deleting the events");
      }
    });
  }
});
...
calendarMgr?.getCalendar(async (err: BusinessError, data:calendarManager.Calendar) => {
  if (err) {
    ...
    calendar.updateEvent(oriEvent, (err: BusinessError) => {
      if (err) {
        console.error(`Failed to update the event, err -> ${JSON.stringify(err)}`);
      } else {
  ...
  if (err) {
    ...
    calendar.getEvents((err: BusinessError, data: calendarManager.Event[]) => {
      if (err) {
        ...
        console.info(`Succeeded in getting the events, data -> ${JSON.stringify(data)}`);
      }
    });
  }
});
...
calendarMgr?.getCalendar((err: BusinessError, data:calendarManager.Calendar) => {
  if (err) {
    ...
    calendar.setConfig(config, (err: BusinessError) => {
      if (err) {
        console.error(`Failed to set the configuration, err -> ${JSON.stringify(err)}`);
      } else {
        console.info(`Succeeded in setting the configuration, config -> ${JSON.stringify(config)}`);
      }
    });
  }
});
...
calendarMgr?.getCalendar((err: BusinessError, data:calendarManager.Calendar) => {
  if (err) {
    ...
    calendar.setConfig(config).then(() => {
      console.info(`Succeeded in setting the configuration, data->${JSON.stringify(config)}`);
    }).catch((err: BusinessError) => {
...
| Name          | Type                             | Read-only| Mandatory| Description                                                        |
| -------------- | --------------------------------- | ---- | ---- | ------------------------------------------------------------ |
| id             | number                            | No  | No  | Event ID. This parameter does not need to be set in the [addEvent()](#addevent) or [addEvents()](#addevents) API.|
