...
| Name        | Type                                      | Mandatory  | Description                     |
| ----------- | ---------------------------------------- | ---- |-------------------------|
| elementName | [ElementName](js-apis-bundleManager-elementName.md#elementname) | Yes   | Information about the page, on which the application declares the NFC card emulation capability. It must contain at least **bundleName** and **abilityName**, and cannot be empty.|
...
Provides APIs for implementing HCE, including receiving Application Protocol Data Units (APDUs) from the peer card reader and sending a response. Before using HCE-related APIs, check whether the device supports HCE. This API is used only for declaration and cannot be used currently.
...
Stops HCE, including exiting the current application from the foreground, releasing the dynamically registered AID list, and releasing the subscription of [hceCmd](#on8). This API is used only for declaration and cannot be used currently.
...
Stops HCE, including canceling the subscription of APDUs, exiting this application from the foreground, and releasing the dynamically registered AID list. The application needs to call this API in **onDestroy** of the HCE page. This API is used only for declaration and cannot be used currently.
...
Registers a callback to receive APDUs from the peer card reader. The application needs to call this API in **onCreate** of the HCE page. This API is used only for declaration and cannot be used currently.
...
Transmits APDU data to the peer card reader. This API uses a promise to return the result. The application calls this API only after receiving an APDU sent by the card reader via [on](#on8). This API is used only for declaration and cannot be used currently.
...
Transmits APDU data to the peer card reader. This API uses an asynchronous callback to return the result. The application calls this API only after receiving an APDU sent by the card reader via [on](#on8). This API is used only for declaration and cannot be used currently.
