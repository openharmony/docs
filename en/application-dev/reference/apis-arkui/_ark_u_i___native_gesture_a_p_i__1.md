...
| void(\* [dispose](#dispose) )(ArkUI_GestureRecognizer \*recognizer) | Disposes a gesture to release resources. | 
...
0 - The operation is successful. returns **401** if a parameter error occurs. Returns 401 if a parameter exception occurs.
...
0 - The operation is successful. returns **401** if a parameter error occurs.
...
| durationNum | Minimum duration for triggering touch and hold, in milliseconds. If the value is less than or equal to 0, the default value **500** is used. | 
...
| directions | Specifies the direction of the gesture that triggers the drag. The enumerated values support logical AND (&amp;) and logical OR (\).|operations. | 
...
| Name | Description | 
| -------- | -------- |
| fingersNum | Minimum number of fingers to trigger a pinch. The value ranges from 2 to 5. Default value: **2** | 
...
| Name | Description | 
| -------- | -------- |
| fingersNum | Minimum number of fingers to trigger a rotation. The value ranges from 2 to 5. Default value: **2** | 
...
| speedNum | Minimum sliding speed, in px/s. If this parameter is set to a value less than or equal to 0, the default value **100** is used. | 
...
| Name | Description | 
| -------- | -------- |
| recognizer | A gesture that needs to be destroyed. | 
...
Obtains the hand gesture type.

**Parameters**

| Name | Description | 
| -------- | -------- |
| recognizer | Gesture pointer. | 
...
| Name | Description | 
| -------- | -------- |
| group | Gesture group whose sub-gestures need to be deleted. | 
| child | Sub-gesture. | 
...
| -------- | -------- |
| node | Node whose gesture needs to be removed. | 
...
| Name | Description | 
| -------- | -------- |
| recognizer | Pointers to gestures that need to be bound to callback events. | 
| actionTypeMask | * @param actionTypeMask Indicates the set of gesture event types. Multiple callbacks can be registered at once, with the callback event types distinguished in the callbacks. Example: actionTypeMask = GESTURE_EVENT_ACTION_ACCEPT \| GESTURE_EVENT_ACTION_UPDATE;  | 
| extraParams | Context data transferred during targetReceiver callback. | 
| targetReceiver | * @param targetReceiver Indicates the callback to register for processing the gesture event types. **event** indicates the gesture callback data. | 
...
| Name | Description | 
| -------- | -------- |
| node | ARKUI node whose callback needs to be interrupted by gestures. | 
| interrupter | Interrupt callback. info returns gesture interrupt data. The interrupter returns the GESTURE_INTERRUPT_RESULT_CONTINUE, gesture normally, and the interrupter returns the GESTURE_INTERRUPT_RESULT_REJECT gesture. | 
