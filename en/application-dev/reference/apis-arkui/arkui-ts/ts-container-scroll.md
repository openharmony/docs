...
| Name | Type                                                | Mandatory | Description                                                     |
| ------ | ---------------------------------------------------- | ---- | --------------------------------------------------------- |
| value  | [Resource](ts-types.md#resource) \| number | Yes  | Friction coefficient.<br>Default value: **0.9** for wearable devices and **0.6** for non-wearable devices<br>Since API version 11, the default value for non-wearable devices is **0.7**.<br>Since API version 12, the default value for non-wearable devices is **0.75**. |
...
onDidScroll(handler: [ScrollOnScrollCallback](#scrollonscrollcallback))
...
| Name | Type                                                     | Mandatory | Description                  |
| ------- | --------------------------------------------------------- | ---- | ---------------------- |
| handler | [ScrollOnScrollCallback](#scrollonscrollcallback)  | Yes  | Callback triggered when the **Scroll** component scrolls. |
...
Represents the callback triggered before scrolling. 
...
| yOffset     | number                                                  | Yes  | Vertical offset per frame during scrolling. A positive offset indicates scrolling upward, and a negative offset indicates scrolling downward.<br>Unit: vp |
| scrollState | [ScrollState](ts-container-list.md#scrollstate) | Yes | Current scrolling state.                                              | 
| scrollSource | [ScrollSource](ts-container-list.md#scrollsource12) | Yes | Source of the current scrolling operation. |
...
Triggered when the **Scroll** component reaches the start position.
...
Triggered when the **Scroll** component reaches the end position.

When the component's edge effect is the spring effect, this event is triggered once when the component passes the end position and is triggered again when the component returns to the end position.
