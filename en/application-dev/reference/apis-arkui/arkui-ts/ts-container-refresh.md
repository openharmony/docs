...
struct ListRefreshLoad {
  ...
  refreshBuilder() {
    Stack({ alignContent: Alignment.Bottom }) {
      //The Progress component can be controlled by the refresh state.
      // The Progress component is present only when the refresh state is pulling or refreshing.
  ...
  footer() {
    ...
    // Hide the component when it is not in the loading state.
