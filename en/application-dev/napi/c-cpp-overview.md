...
The OpenHarmony NDK provides industry standard libraries [musl libc](../reference/native-lib/musl.md) and [C++ Standard Library](../reference/native-lib/cpp.md). This topic describes the mechanisms of these libraries in OpenHarmony. Understanding these mechanisms helps you avoid pitfalls during NDK development.
...
The namespace mechanism allows an application to load only the native libraries in its installation directory (for example, **libs/arm64** on the ARM64 platform). If an application needs to load multiple native libraries, the native library in a new directory cannot be loaded if multiple loading paths are created. In this case, you can use the rpath mechanism to specify the search path during compilation.
...
### dlclose
You can use **dlclose** to truly unload a dynamic library.
...
### Globalization
Since API version 12, **locale** in **newlocale()** and **setlocale()** can be set to **C**, **C.UTF-8**, **en_US**, **en_US.UTF-8**, **zh_CN**, or **zh_CN.UTF-8**. **strtod_l**, **wcstod_l**, and **localeconv** support the **locale** values **zh_CN** and **zh_CN.UTF-8**. Note that **strtod_l()** and **wcstod_l()** do not support conversion of hexadecimal numbers and hexadecimal floating-point numbers.

### fdsan
File descriptor sanitizer ([fdsan](./fdsan.md)) helps detect the user-after-close and double-close issues of FDs.

### Network Usage
Using **RTM_GETLINK** (the core message type for network interface information) through the **send**, **sendto**, **sendmsg**, **getifaddrs**, and **if_nameindex** APIs on sockets of the route netlink type poses security risks and is therefore not recommended. You are advised to use the C library **ioctl** API and the corresponding **cmd** values to query network interface information.

The meanings of HarmonyOS internal **ioctl** **cmd** values are as follows:

| Name | Description |
| --- | --- |
| SIOCGIFINDEX | Obtains the network interface index. |
| SIOCGIFFLAGS | Obtains the flags (status) of a network interface. |
| SIOCGIFADDR | Obtains the address of a network interface. |
| SIOCGIFBRDADDR | Obtains the broadcast address of a network interface. |
| SIOCGIFNAME | Obtains the network interface name by index. |
| SIOCGIFCONF | Obtains the list of all network interfaces. |
