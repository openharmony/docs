...
As a rendering component, the [XComponent](../reference/apis-arkui/arkui-ts/ts-basic-components-xcomponent.md) is usually used to meet relatively complex rendering customization requirements, for example, display of a camera preview stream and rendering of a game image.
...
- **libraryname**: name of the loaded module, which must be the same as the value of **nm_modname** used when the Napi module is registered on the native side.
  ...
  >
  >- Use the **import** mode of the N-API.
  >
  >   ```ts
