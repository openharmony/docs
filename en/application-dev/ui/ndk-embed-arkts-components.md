...
The capabilities provided by ArkUI on the Native side are a subset of ArkTS, and some capabilities will not be provided on the Native side, such as declarative UI syntax, custom struct components, and advanced UI components.

For scenarios that require the independent capabilities of the ArkTS side, the ArkUI development framework provides the ability to embed ArkTS components on the Native side. This capability relies on the [ComponentContent](../reference/apis-arkui/js-apis-arkui-ComponentContent.md) mechanism, which encapsulates ArkTS components through ComponentContent. The encapsulated object is then passed to the Native side and converted into an ArkUI_NodeHandle object for use in Native side component mounting via the Native side's [OH_ArkUI_GetNodeHandleFromNapiValue](../reference/apis-arkui/_ark_u_i___native_module.md#oh_arkui_getnodehandlefromnapivalue) interface.


> **NOTE**
>
> - The ArkUI_NodeHandle object obtained through the OH_ArkUI_GetNodeHandleFromNapiValue interface can only be used as a child component parameter, such as the second parameter of the [addChild](../reference/apis-arkui/_ark_u_i___native_node_a_p_i__1.md#addchild) interface. Using this object in other scenarios, such as setting attributes with [setAttribute](../reference/apis-arkui/_ark_u_i___native_node_a_p_i__1.md#setattribute), will not take effect and will return an error code.
>
> - For scenarios where the Native side modifies ArkTS components, the update data on the ArkTS side must be constructed through the Node-API approach on the Native side, and then updated through the [update](../reference/apis-arkui/js-apis-arkui-ComponentContent.md#update) interface of ConmponnetContent.
>
> - In [custom scenarios](ndk-build-custom-components.md), functions such as measureNode cannot be called on components inside the ArkTS module.

The following example code embeds the ArkTS Refresh component based on the [accessed ArkTS page](ndk-access-the-arkts-page.md).


**Figure 1** Refresh component with a text list mounted

![refresh_text_list](figures/refresh_text_list.gif)

1. Register the ArkTS component creation function to the Native side for calling, and the creation function uses the ComponentContent capability for encapsulation.
   ```ts
   // MixedModule.ets
   ...
   // Define the parameter format for the @Builder function.
   interface RefreshAttribute {
     isRefreshing: boolean;
     // Attribute settings are optimized for performance with Modifier
   ...
   // ComponentContent encapsulation of ArkTS components relies on the global @Builder function, which can be used to nest @Component custom components in complex custom component scenarios.
   // The @Builder function provides a parameter input method, facilitating subsequent parameter updates through the update interface of ComponentContent.
   @Builder
   ...
   interface MixedModuleResult {
     // Define the encapsulated object for the Refresh construction function, used for conversion to ArkUI_NodeHandle objects on the Native side.
     content?: ComponentContent<RefreshAttribute>;
     // As a container component, Refresh needs to use the ContentSlot mechanism to mount child components on the Native side.
   ...
   export function createMixedRefresh(value: NativeRefreshAttribute): MixedModuleResult {
     ...
     // Maintain the UI context object at the start of the Ability using the AppStorage object.
     ...
     // Create a Refresh component through ComponentContent and encapsulate it.
   ...
   // Define the update function for the Refresh component for updates on the Native side.
   // In the update scenario, it is necessary to return the encapsulated object of the Refresh component and its child component slot object to prevent the component from being recreated.
   export function updateMixedRefresh(refresh: ComponentContent<RefreshAttribute>, childSlot: NodeContent,
     ...
     // Call the update interface of ComponentContent to perform the update.
     refresh.update({
       isRefreshing: value.isRefreshing,
       modifier: modifier,
       slot: childSlot,
...
2. Register the creation and update functions with the Native side.
   ...
   struct Index {
     ...
     changeNativeFlag(): void {
       if (this.showNative) {
         // Create a NativeModule component for mounting.
     ...
     build() {
       Column() {
         ...
         Row() {
           // ArkTS inserts Native components
...
3. On the Native side, save the creation and update functions through the Node-API for subsequent calls.
   ```c
   // ArkUIMixedRefresh.h
   ...
   napi_value ArkUIMixedRefresh::RegisterCreateRefresh(napi_env env, napi_callback_info info) {
       ...
       g_env = env;
       napi_ref refer;
       // Create a reference and save it to prevent it from being released.
...
4. Abstract the base class for mixed mode components.
   ```c
   // ArkUIMixedNode.h
   ...
   public:
       ...
       // In the base class destructor, the mixed mode object on the ArkTS side should be released.
...
5. Implement the mixed mode encapsulation object for the Refresh component.
   ```c
   // ArkUIMixedRefresh.h
   // Encapsulation object for Refresh in mixed mode on the Native side.
   ...
   // Define the data structure for interaction between the Native side and the ArkTS side.
   ...
   public:
       ...
       ~ArkUIMixedRefresh() override { napi_delete_reference(env_, nodeContent_); } // Release the child node placeholder component slot.
       ...
       // To avoid frequent cross-language calls, cache property events on the Native side and notify in batches.
   ...
   protected:
       void OnAddChild(const std::shared_ptr<ArkUIBaseNode> &child) override {
           ...
           // Mount the component (can be a pure Native component or an ArkTS component converted through ComponentContent on the Native side) under the ArkTS component using NodeContent.
       ...
       void OnRemoveChild(const std::shared_ptr<ArkUIBaseNode> &child) override {
           assert(contentHandle_);
           // Unmount the component using NodeContent.
       ...
       void OnInsertChild(const std::shared_ptr<ArkUIBaseNode> &child, int32_t index) override {
           assert(contentHandle_);
           // Insert the component using NodeContent.
           OH_ArkUI_NodeContent_InsertNode(contentHandle_, child->GetHandle(), index);
       }
   
   private:
       // Create the data structure on the ArkTS side using the NAPI.
   ...
   // Create the data structure for interaction with the ArkTS side using the NAPI, used for creating and updating the Refresh component.
   ...
   // Create the ArkTS side component and save it in the encapsulation object on the Native side.
   const std::shared_ptr<ArkUIMixedRefresh> ArkUIMixedRefresh::Create(const NativeRefreshAttribute &attribute) {
       ...
       // Get the ArkTS Refresh component.
       ...
       assert(handle);
       // Get the child component slot of the ArkTS Refresh component.
       ...
       // Save the ArkTS ComponentContent to prevent the release of the object on the ArkTS side and for subsequent updates.
       napi_ref componentContentRef;
       napi_create_reference(g_env, componentContent, 1, &componentContentRef);
       // Save the ArkTS NodeContent to prevent the release of the object on the ArkTS side and for subsequent updates.
   ...
   void ArkUIMixedRefresh::FlushMixedModeCmd() {
       napi_handle_scope scope;
       napi_open_handle_scope(g_env, &scope);
       // Create the input parameter for the call to the ArkTS interface.
       auto refreshAttribute = CreateRefreshAttribute(attribute_, this);
       if (refreshAttribute == nullptr) {
           napi_close_handle_scope(g_env, scope);
           return;
       }
      // Get the remaining two interface parameters for the update interface.
       napi_value componentContent = nullptr;
...
6. Using the page structure from the [Integrating with ArkTS Pages](ndk-access-the-arkts-page.md) section and continuing with the [Timer Module Simple Implementation](ndk-loading-long-list.md), make the Refresh component the parent component of the text list.
   ```c
   // MixedRefreshExample.h
   // Example code for mixed mode.
   ...
   std::shared_ptr<ArkUIBaseNode> CreateMixedRefreshList(napi_env env) {
       ...
       refresh->SetRefreshCallback([refreshPtr = refresh.get(), env]() {
           ...
           CreateNativeTimer(env, refreshPtr, 1, [](void *userData, int32_t count) {
               // Close the refresh after data acquisition.
       ...
       // Update the events to the ArkTS side.
       refresh->FlushMixedModeCmd();
       return refresh;
   }
...
Replace the entry component creation with a pull-to-refresh text list.
