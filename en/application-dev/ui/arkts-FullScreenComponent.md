...
<!-- md-trans-meta sourceCommit=871010847291a8d5221a0bd1c554d4d2cba1ab41 translatedAt=2026-08-01T00:25:36.665Z pushedAt=2026-08-01T02:23:13.870Z -->
...
FullScreenLaunchComponent provides a capability for full-screen launch of an atomic service. When an atomic service needs to be launched, the launching party queries AMS to check whether the launched party has authorized the consumer to run the atomic service in embedded mode. If authorized, the consumer runs the atomic service in full-screen embedded mode; if unauthorized, the consumer launches the atomic service via redirect.
...
| [FormLink](../reference/apis-arkui/arkui-ts/ts-container-formlink.md) | Not supported  | The **FormLink** component is provided for interactions between the static widgets and provider application. It supports three types of events: router, message, and call.| This component facilitates cross-process component calls. However, nesting it within the **FullScreenLaunchComponent** can complicate the process relationships, potentially leading to functional and performance issues.|
...
| [Action Sheet (ActionSheet)](../reference/apis-arkui/arkui-ts/ts-methods-action-sheet.md)| Partially supported| An action sheet is a dialog box that displays actions a user can take. If **showInSubWindow** is set to **true** in the **FullScreenLaunchComponent**, the action sheet is aligned with the host window based on the **FullScreenLaunchComponent**.| The action sheet achieves alignment with the host application's window by utilizing data obtained from the **FullScreenLaunchComponent** regarding the application's window. Yet, only window alignment is supported.|
...
The Native APIs provide capabilities that, in the **FullScreenLaunchComponent** context, also need to consider whether they extend beyond the current component, as well as interactions with the launcher (host) component and the application process context. This mainly includes the following scenarios:
...
| <!--DelRow-->[@ohos.uiAppearance (User Interface Appearance) (System API)](../reference/apis-arkui/js-apis-uiappearance-sys.md) | Not supported   | User Interface Appearance provides basic capabilities for managing the system appearance, currently including only dark/light mode configuration. | The provider cannot affect the consumer through this capability.                             |
...
- Synchronous event handling principles: Events that can be processed synchronously should have a low triggering frequency and minor performance impact, and should meet the actual scenarios of the application as much as possible.
...
When developing the provider application, you (provider) need to consider the following design constraints:
...
You can mitigate the flickering white screen issue in the following ways:
...
- Modifying and switching the atomic service EmbeddableUIAbility launched from the background is not supported during runtime.
