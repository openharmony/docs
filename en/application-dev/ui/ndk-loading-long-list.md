...
For List, Grid, WaterFlow, and Swiper components, the [NodeAdapter](../reference/apis-arkui/_ark_u_i___native_module.md#arkui_nodeadapterhandle) object is provided as an alternative to the LazyForEach function on the ArkTS side. It is used to generate child components on demand. The property enumeration values for the List component are NODE_LIST_NODE_ADAPTER, for the Grid component are NODE_GRID_NODE_ADAPTER, for the WaterFlow component are NODE_WATER_FLOW_NODE_ADAPTER, and for the Swiper component are NODE_SWIPER_NODE_ADAPTER.

Although both are used for on-demand component generation, the NodeAdapter object differs from ArkTS's LazyForEach in the following specifications:

- Nodes that have set the NodeAdapter property no longer support direct child component addition interfaces such as addChild. Child components are entirely managed by NodeAdapter. When setting the NodeAdapter using the property method, it checks whether the parent component already has child nodes. If the parent component already has child nodes, the setting of NodeAdapter fails and returns an error code.

- NodeApdater notifies developers to generate components on demand through related events, similar to component event mechanisms. Developers need to register [event listeners](../reference/apis-arkui/_ark_u_i___native_module.md#oh_arkui_nodeadapter_registereventreceiver) when using NodeAdapter, and handle the logic in the listener events. The related events are defined through [ArkUI_NodeAdapterEventType](../reference/apis-arkui/_ark_u_i___native_module.md#arkui_nodeadaptereventtype). Additionally, NodeAdapter does not actively release component objects that are not displayed on the screen. Developers need to release or cache and reuse component objects in the [NODE_ADAPTER_EVENT_ON_REMOVE_NODE_FROM_ADAPTER](../reference/apis-arkui/_ark_u_i___native_module.md#arkui_nodeadaptereventtype) event. The following figure shows the event triggering mechanism in a typical list scrolling scenario:
  ![en-us_image_0000001949769409](figures/en-us_image_0000001949769409.png)


The following example code optimizes the code from the [Integrating with ArkTS Pages](ndk-access-the-arkts-page.md) section by introducing a lazy loading mechanism to implement a text list:


1. Integrate with ArkTS. For details, see  [Integrating with ArkTS Pages](ndk-access-the-arkts-page.md).

2. Implement lazy loading adapter features.
   ```
   // ArkUIListItemAdapter
   // Code for lazy loading feature of the text list.
   ...
   public:
       ArkUIListItemAdapter()
           : module_(NativeModuleInstance::GetInstance()->GetNativeNodeAPI()), handle_(OH_ArkUI_NodeAdapter_Create()) { // Call the create function of NodeAdapter.
       ...
       ~ArkUIListItemAdapter() {
           ...
           items_.clear();
           // Release Adapter-related resources.
       ...
       void RemoveItem(int32_t index) {
           ...
           // If index will cause visibility changes of elements in the visible area, it will call back NODE_ADAPTER_EVENT_ON_REMOVE_NODE_FROM_ADAPTER event to delete elements.
           // Depending on whether there are new elements, call back NODE_ADAPTER_EVENT_ON_GET_NODE_ID and NODE_ADAPTER_EVENT_ON_ADD_NODE_TO_ADAPTER events.
       ...
       void InsertItem(int32_t index, const std::string &value) {
           ...
           // If index will cause visibility changes of elements in the visible area, it will call back NODE_ADAPTER_EVENT_ON_GET_NODE_ID and NODE_ADAPTER_EVENT_ON_ADD_NODE_TO_ADAPTER events,
           // Depending on whether there are deleted elements, call back NODE_ADAPTER_EVENT_ON_REMOVE_NODE_FROM_ADAPTER event.
       ...
       void MoveItem(int32_t oldIndex, int32_t newIndex) {
           ...
           // If the move position does not cause visibility changes of elements in the visible area, no event will be called back; otherwise, call back the corresponding event according to the addition and deletion scenarios.
           OH_ArkUI_NodeAdapter_MoveItem(handle_, oldIndex, newIndex);
       }
   
       void ReloadItem(int32_t index, const std::string &value) {
           data_[index] = value;
           // If index is located in the visible area, first call back NODE_ADAPTER_EVENT_ON_REMOVE_NODE_FROM_ADAPTER to delete the old element,
           // Then call back NODE_ADAPTER_EVENT_ON_GET_NODE_ID and NODE_ADAPTER_EVENT_ON_ADD_NODE_TO_ADAPTER events.
           OH_ArkUI_NodeAdapter_ReloadItem(handle_, index, 1);
       }
   
       void ReloadAllItem() {
           std::reverse(data_.begin(), data_.end());
           // In the scenario of reloading all items, it will call back NODE_ADAPTER_EVENT_ON_GET_NODE_ID to obtain new component IDs,
           // Compare based on new component IDs, reuse those with unchanged IDs,
           // For elements with new IDs, call NODE_ADAPTER_EVENT_ON_ADD_NODE_TO_ADAPTER event to create new components,
           // Then determine the unused IDs left in the old data, and call NODE_ADAPTER_EVENT_ON_REMOVE_NODE_FROM_ADAPTER to delete old elements.
           OH_ArkUI_NodeAdapter_ReloadAllItems(handle_);
       }
   
   private:
       static void OnStaticAdapterEvent(ArkUI_NodeAdapterEvent *event) {
           // Get the instance object, call back the instance event.
       ...
       // Assign IDs to items for element diff in the ReloadAllItems scenario.
       ...
       // Set a new list item to display in the visible area.
       void OnNewItemAttached(ArkUI_NodeAdapterEvent *event) {
           ...
           if (!cachedItems_.empty()) {
               ...
               // Release the cache pool reference.
               cachedItems_.pop();
           } else {
               // Create a list item.
       ...
       void OnItemDetached(ArkUI_NodeAdapterEvent *event) {
           ...
           // Place it in the cache pool for recycling.
       ...
       // Manage elements generated by NodeAdapter.
       std::unordered_map<ArkUI_NodeHandle, std::shared_ptr<ArkUIListItemNode>> items_;
   
       // Manage the recycled component pool.
...
3. Enhance the list encapsulation class object used in the [Integrating with ArkTS Pages](ndk-access-the-arkts-page.md) section with additional lazy loading capabilities.
   ```
   // ArkUIListNode.h
   ...
   public:
       ...
       ~ArkUIListNode() override {
           ...
           if (adapter_) {
               // Unload UI components under the adapter when destructing.
...
4. Create a list with lazy loading.
   ```
   // ArkUILazyTextListExample
   // Example code for the lazy loading list.
   ...
   std::shared_ptr<ArkUIBaseNode> CreateLazyTextListExample(napi_env env) {
       ...
       // 2: Create ListItem lazy loading component and mount it on the List.
       auto adapter = std::make_shared<ArkUIListItemAdapter>();
       list->SetLazyAdapter(adapter);
   
       // 3: Simulate related lazy loading operations.
...
5. Simple tmplementation of the Timer module:
   ```
   // UITimer.h
   // Timer module.
...
6. Mount lazy loading example code to ContentSlot for display based on the [Integrating with ArkTS Pages](ndk-access-the-arkts-page.md) topic.
   ```
   // NDK interface entry mounting file.
   ...
   napi_value CreateNativeRoot(napi_env env, napi_callback_info info) {
       ...
       // Create lazy loading text list
       auto node = CreateLazyTextListExample(env);
   
       // Keep the Native side object in the management class to maintain the lifecycle.
