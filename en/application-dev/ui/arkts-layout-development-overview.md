...
- Component content area (yellow block): size of the component area minus the [paddings](../reference/arkui-ts/ts-universal-attributes-size.md) of the component. It serves as the layout constraint for calculating the size of the component content (or child component).

- Component content (green block): size of the component content, for example, size of the text content in the component. The component content does not necessarily match the component content area. For example, if a component is set to a fixed width and height, the size of its content area is calculated based on the width and height minus the paddings, while the size for its text content is calculated by the text layout engine. In this case, the component content may be less than the component content area. When the component content and component content area do not match, the[align](../reference/arkui-ts/ts-universal-attributes-location.md) attribute takes effect, which defines the alignment mode of the component content in the component content area, for example, center aligned.
