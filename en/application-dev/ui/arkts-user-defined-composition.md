...
<!-- md-trans-meta sourceCommit=2fbcb1617e6bb792403a6beeb4fb79f7ab2b5a4f translatedAt=2026-08-05T01:23:53.910Z pushedAt=2026-08-05T01:50:07.061Z -->

## Overview

User-defined composition provides customization capabilities at the component granularity. You can combine existing components through custom components to build specific advanced components that align with business logic. During component composition, you can use layout containers and common layout attributes to arrange child components in custom positions, use the [Canvas](arkts-drawing-customization-on-canvas.md) component for pure custom drawing, or use [Shape](../reference/apis-arkui/arkui-ts/ts-drawing-components-shape.md)-type components to construct geometric graphics through shape combinations, enabling interactive graphic animations. The animation module can implement flexible animation effects for different types of attributes.
...
| Use the @Builder decorator to build reusable UI structures | Compared with the [@Component](../ui/state-management/arkts-create-custom-components.md#component) decorator, the [@Builder](../ui/state-management/arkts-builder.md) decorator is more lightweight. It can encapsulate pure UI structures as methods and invoke them in the build method of each custom component, thereby achieving UI structure reuse. This is equivalent to extracting the build portion from [@Component](../ui/state-management/arkts-create-custom-components.md#component) separately, without containing state variables itself. |
...
| Using the @AnimatableExtend decorator for animation effects|For non-animatable attributes, use the [@AnimatableExtend](../ui/state-management/arkts-animatable-extend.md) decorator instead of [property animation](./arkts-attribute-animation-apis.md). For numeric attributes, simply place them in globally defined @AnimatableExtend methods to make them animatable. For non-numeric attributes, implement **plus**, **subtract**, **multiply**, and **equals** APIs by creating a child class of [AnimatableArithmetic](../ui/state-management/arkts-animatable-extend.md), then use @AnimatableExtend for animation effects.|
