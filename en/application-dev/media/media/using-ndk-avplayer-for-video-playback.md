...
<!-- md-trans-meta sourceCommit=425e79ed59a841b19860caacc0b050f68405d43e translatedAt=2026-08-11T01:54:47.809Z pushedAt=2026-08-12T03:05:09.586Z -->
...
For details about the states, see [AVPlayerState](../../reference/apis-media-kit/capi-avplayer-base-h.md#avplayerstate). When playback is in the AV_PREPARED, AV_PLAYING, AV_PAUSED, or AV_COMPLETED state, the playback engine is in working state, which consumes considerable system memory. When you do not need the player temporarily, call `reset()` or `release()` to reclaim memory resources and properly manage resource utilization.
...
2. Set callback listener functions: Call [OH_AVPlayer_SetOnInfoCallback()](../../reference/apis-media-kit/capi-avplayer-h.md#oh_avplayer_setoninfocallback) and [OH_AVPlayer_SetOnErrorCallback()](../../reference/apis-media-kit/capi-avplayer-h.md#oh_avplayer_setonerrorcallback) to set the information listener callback function and error listener callback function for use in full-process scenarios. The supported listener callback functions include:

   | Callback Type | Description |
   | -------- | -------- |
   | OH_AVPlayerOnInfoCallback | Mandatory callback function; used to listen for player process information.<br>The listener must be set when the player is in the AV_IDLE state and before any resource setting API is called. If the listener is set after calling a resource setting API, information events reported during resource setting may be missed. |
   | OH_AVPlayerOnErrorCallback | Mandatory callback function; used to listen for player error information.<br>The listener must be set when the player is in the AV_IDLE state and before any resource setting API is called. If the listener is set after calling a resource setting API, information events reported during resource setting may be missed. |
...
6. Set the playback window: Call [OH_AVPlayer_SetVideoSurface()](../../reference/apis-media-kit/capi-avplayer-h.md#oh_avplayer_setvideosurface) to set the playback window. This function must be called after `OH_AVPlayer_SetURLSource()` and before `OH_AVPlayer_Prepare()`.
