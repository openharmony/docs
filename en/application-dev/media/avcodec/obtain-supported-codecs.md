...
3. Obtain the audio/video codec capability instance.
   ...
   Method 1: Call **OH_AVCodec_GetCapability** to obtain the codec capability instance recommended by the framework. The recommendation policy is the same as that of the **OH_XXX_CreateByMime** series APIs.
   ```c++
   ...
   ```
    The framework automatically recycles the instance when it is no longer needed.
