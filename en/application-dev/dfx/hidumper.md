...
| --mem-jsheap pid [-T tid] [--gc] |  Triggers the Garbage Collection (GC) and exports the snapshot of all threads. **pid** is mandatory. If **tid** is specified, triggers the GC and exports the snapshot of the specified thread.|
...
14. Run the **hidumper --mem-jsheap pid [-T tid] [--gc]** command to trigger GC and export snapshot of all threads. **pid** is mandatory. Specify **tid** to trigger GC and export snapshot of the specified thread. Specify **--gc** to trigger GC but not to export snapshot. This command is only available in the Debug version.
