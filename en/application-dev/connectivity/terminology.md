...
Core data unit of a [GATT](#gatt) [service](#service). It supports data read/write and is uniquely identified by a [UUID](#uuid).
...
In the HID protocol, data transmission channels are divided into two types: the interrupt channel and the control channel. The interrupt channel is used for transmitting unidirectional, low-latency real-time data; the control channel is used for transmitting bidirectional, reliable real-time data, including the following three types of requests:
...
Out of Band, which refers to the use of a channel independent of the primary data channel for information transmission. For example, during the pairing process, Bluetooth devices can exchange security keys via non-Bluetooth channels such as a Wi-Fi network or NFC, thereby enhancing pairing security.
...
Bluetooth [GATT](#gatt) service, which is a data structure containing multiple characteristics and dependent services. It represents a capability of the [BLE](#ble) device and is identified by a [UUID](#uuid).
