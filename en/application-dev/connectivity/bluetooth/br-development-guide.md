...
You can subscribe to the Bluetooth status change events to obtain the real-time Bluetooth status. Bluetooth status transition occurs when Bluetooth is enabled or disabled. [STATE_ON](../../reference/apis-connectivity-kit/js-apis-bluetooth-access.md#bluetoothstate) indicates that Bluetooth is enabled while [STATE_OFF](../../reference/apis-connectivity-kit/js-apis-bluetooth-access.md#bluetoothstate) indicates the opposite. An application can use Bluetooth-dependent functions only if Bluetooth is enabled.
```ts
...
### Enabling Bluetooth
If the Bluetooth switch is turned off, you need to enable Bluetooth manually to use Bluetooth-dependent functions. You can subscribe to the Bluetooth status change events to determine whether Bluetooth is successfully enabled.
