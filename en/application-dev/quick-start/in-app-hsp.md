...
- An HSP must be installed and run with the HAP that depends on it. It cannot be installed or run independently on a device. The HSP version must be the same as the HAP version.
  ...
  build() {
    Column() {
      List() {
        ...
        .padding({ left: 12, right: 12, top: 4, bottom: 4 })
        .onClick(() => {
          // Obtain the context of the HSP module through the current context and the resourceManager object of the HSP module, and call the API of resourceManager to obtain resources.
