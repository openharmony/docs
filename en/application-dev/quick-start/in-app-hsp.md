...
- An HSP must be installed and run with the HAP that depends on it. It cannot be installed or run independently on a device. The HSP version must be later than or equal to the HAP version.
- HSP does not support the declaration of the [ExtensionAbility](../application-models/extensionability-overview.md) but [UIAbility](../application-models/uiability-overview.md) components (except for the entry ability) in the configuration file.
