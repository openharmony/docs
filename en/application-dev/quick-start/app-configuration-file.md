...
| hwasanEnabled | Whether HWAsan detection is enabled for an application. HWAsan(HardWare-assisted AddressSanitizer) is an enhanced Asan improved by the Top-Byte-Ignore feature. Compared with Asan, HWAsan has a lower memory overhead and a larger range of detection for memory errors.<br>-&nbsp;**true**: HWAsan is enabled.<br>-&nbsp;**false**: HWAsan is disabled.| Boolean| Yes (initial value: **false**)|
