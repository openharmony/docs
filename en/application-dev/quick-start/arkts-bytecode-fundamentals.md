# Basic Principles of Ark Bytecode
...
|   -------   |  -------  |  ----------  |   ----------   |   --------   |
|  0x00	|  NONE	|  ldundefined	|   |  	Load the **undefined** file to **acc**.  |
|  0x01	|  NONE	|  ldnull	|   |  	Load the **null** file to **acc**.  |
|  0x02	|  NONE	|  ldtrue	|   |  	Load the **true** file to **acc**.  |
|  0x03	|  NONE	|  ldfalse	|   |  	Load the **false** file to **acc**.  |
|  0x04	|  NONE	|  createemptyobject	|   |  	Create an empty object and store it in **acc**.  |
|  0x05	|  IMM8|  	createemptyarray RR	|  R: 8-bit reserved number used internally during Ark running.|  Create an empty array and store it in **acc**.  |
|  0x06	|  IMM8_ID16	|  createarraywithbuffer RR, @AAAA	|  R: 8-bit reserved number used internally during Ark running.<br>A: 16-bit literal id.|  Use the literal array corresponding to index A to create an array object and store it in **acc**.  |
|  0x07	|  IMM8_ID16	|  createobjectwithbuffer RR, @AAAA	|  R: 8-bit reserved number used internally during Ark running.<br>A: 16-bit literal id.|  Use the literal array corresponding to index A to create an object and store it in **acc**.  |
|  0x08	|  IMM8_IMM8_V8	|  newobjrange RR, +AA, vBB	|  R: 8-bit reserved number used internally during Ark running.<br>A: number of parameters<br>B: class object<br>B + 1, ..., B + A - 1: parameter passed to the constructor|  	Use **B + 1, ..., B + A - 1** as a parameter to create an instance of class B and store it in **acc**.  |
...
|  0x0a	|  IMM8_V8	|  add2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A + acc** and save the calculation result to **acc**.  |
|  0x0b	|  IMM8_V8	|  sub2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A - acc** and save the calculation result to **acc**.  |
|  0x0c	|  IMM8_V8	|  mul2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A * acc** and save the calculation result to **acc**.  |
|  0x0d	|  IMM8_V8	|  div2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A / acc** and save the calculation result to **acc**.  |
|  0x0e	|  IMM8_V8	|  mod2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A % acc** and save the calculation result to **acc**.  |
|  0x0f	|  IMM8_V8	|  eq RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A == acc** and save the calculation result to **acc**.  |
|  0x10	|  IMM8_V8	|  noteq RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A != acc** and save the calculation result to **acc**.  |
|  0x11	|  IMM8_V8	|  less RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A < acc** and save the calculation result to **acc**.  |
|  0x12	|  IMM8_V8	|  lesseq RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A <= acc** and save the calculation result to **acc**.  |
|  0x13	|  IMM8_V8	|  greater RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A > acc** and save the calculation result to **acc**.  |
|  0x14	|  IMM8_V8	|  greatereq RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A >= acc** and save the calculation result to **acc**.  |
|  0x15	|  IMM8_V8	|  shl2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A << acc** and save the calculation result to **acc**.  |
|  0x16	|  IMM8_V8	|  shr2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A >>> acc** and save the calculation result to **acc**.  |
|  0x17	|  IMM8_V8	|  ashr2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A >> acc** and save the calculation result to **acc**.  |
|  0x18	|  IMM8_V8	|  and2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A & acc** and save the calculation result to **acc**.  |
|  0x19	|  IMM8_V8	|  or2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate `A\| acc` and save the calculation result to **acc**.  |
|  0x1a	|  IMM8_V8	|  xor2 RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the **A ^ acc** and save the calculation result to **acc**. |
|  0x1b	|  IMM8_V8	|  exp RR, vAA	|  Default input parameter: acc: operand<br>R: 8-bit reserved number used internally during Ark running.<br>A: operand|  Calculate the `A ** acc` and save the calculation result to **acc**.  |
|  0x1c	|  IMM8	|  typeof RR	|  Default input parameter: acc: object<br>R: 8-bit reserved number used internally during Ark running.|  Calculate the **typeof acc** and save the calculation result to **acc**.  |
...
|  0x5f	|  V8_IMM8	|  jnstricteq vAA, +BB	|  Default input parameter: acc: object<br>A: object<br>B: signed branch offset|  Calculates **acc !== A** and jumps to branch B if it is true.<br>The command function is not enabled and is unavailable currently.  |
...
|  0x7a	|  IMM8_ID16_V8	|  stownbyname RR, @AAAA, vBB	|  Default input parameter: acc: value<br>R: 8-bit reserved number used internally during Ark running<br>A: string id<br>B: object|  Store the value in **acc** to the attribute whose key is the string corresponding to index A of object B.  |
...
|  0x91	|  IMM16_ID16_V8	|  stobjbyname RRRR, @AAAA, vBB	|  Default input parameter: acc: value<br>R: 16-bit reserved number used internally during Ark running<br>A: string id<br>B: object|  Store the value in **acc** to the attribute whose key is the string corresponding to index A of object B.  |
...
|  0xa8	|  V8_IMM16	|  jne vAA, +BBBB	|  Default input parameter: acc: value<br>A: value<br>B: signed branch offset|  Calculates **acc != A** and jumps to branch B if it is true.<br>The command function is not enabled and is unavailable currently.  |
...
|  0x03fd	|  PREF_IMM16_ID16	|  wide.newlexenvwithname +AAAA, @BBBB	|  A: Number of slots in the lexical environment<br>B: literal id|  Use the lexical variable name stored in the literal array corresponding to index B to create a lexical environment with A slots, store the lexical environment in **acc**, and enter the lexical environment.  |
...
|  0x0bfd	|  PREF_IMM16	|  wide.copyrestargs +AAAA	|  A: start position of the remaining parameters in the formal parameter list|  Copy the rest parameters and save the copied parameter array copy to **acc**.  |
