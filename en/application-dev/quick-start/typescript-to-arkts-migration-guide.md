...
// What is `res`? Is it a number, a string, or an object?
// How to handle it?
// Supported
class CallResult {
...
Currently, only a few projects allow object layout changes at runtime, and some common code linters have added corresponding constraints. This recipe only leads to a small amount of code refactoring, but improves performance.
...
ArkTS does not support declaring class fields in constructors. Declare class fields inside the **class** declaration instead.
...
ArkTS allows unary operators to work on numeric types only. A compile-time error occurs if these operators are applied to a non-numeric type. Unlike in TypeScript, implicit casting of strings in this context is not supported in ArkTS and casting must be done explicitly.
...
function f() {
  ...
  let e2: ESObject = 1;  // Compile-time error: Cannot initialize an ESObject with non-dynamic values.
  let e3: ESObject = {}; // Compile-time error: Cannot initialize an ESObject with non-dynamic values.
  let e4: ESObject = []; // Compile-time error: Cannot initialize an ESObject with non-dynamic values.
  let e5: ESObject = ''; // Compile-time error: Cannot initialize an ESObject with non-dynamic values.
  e5['prop'];            // Compile-time error: Cannot access properties of 'ESObject'.
  e5[1];                 // Compile-time error: Cannot access properties of 'ESObject'.
  e5.prop;               // Compile-time error: Cannot access properties of 'ESObject'.
